...
void print_usage(const char* prog_name) {
    std::clog << "Usage: " << prog_name << " [wheel_radius_m] [wheel_base_m] [ticks_per_rev] [port_name] [calibration_file]\n";
    std::clog << "  wheel_radius_m    : nominal wheel radius in meters (default: 0.0325)\n";
    std::clog << "  wheel_base_m      : distance between left and right wheels in meters (default: 0.16)\n";
    std::clog << "  ticks_per_rev     : encoder ticks per wheel revolution (default: 4320.0)\n";
    std::clog << "  port_name         : serial port (default: /dev/ttyACM0)\n";
    std::clog << "  calibration_file  : output file path (default: data/robot_calibration.txt)\n";
    std::clog << "\nExample:\n";
    std::clog << "  " << prog_name << " 0.033 0.16 4320.0 /dev/ttyACM0 data/robot_calibration.txt\n";
}
...
bool invoke_control(const std::string& control_bin, const std::string& port_name,
                    float left_rad_sec, float right_rad_sec) {
    std::ostringstream cmd;
    cmd << control_bin << " " << std::fixed << std::setprecision(2)
        << left_rad_sec << " " << right_rad_sec
        << " " << port_name;
    std::clog << "  " << cmd.str() << "\n";
...
int main(int argc, char* argv[]) {
    ...
    std::clog << "=======================================================================\n";
    std::clog << "This tool drives the robot straight for a fixed time and uses the\n";
    std::clog << "measured travel distance to compute the effective wheel radius.\n";
    std::clog << "Configuration:\n";
    std::clog << "  Wheel radius   : " << std::fixed << std::setprecision(4) << config.wheel_radius << " m\n";
    std::clog << "  Wheel base     : " << std::setprecision(4) << config.wheel_base << " m\n";
    std::clog << "  Ticks per rev  : " << std::setprecision(1) << config.ticks_per_rev << "\n";
    std::clog << "  Serial port    : " << port_name << "\n";
    std::clog << "  Output file    : " << calib_file << "\n";
    std::clog << "=======================================================================\n\n";

    std::clog << "STEP 1: Place the robot on a flat surface and mark its starting\n";
    std::clog << "        position as 0, 0, 0. Make sure it has room to drive forward.\n";
    ...
    const float wheel_travel_rad = std::abs(left_vel) * drive_duration_sec; // 8 rad

    std::clog << "\nSTEP 2: Driving straight for " << drive_duration_sec << " seconds...\n";
    ...
    std::clog << "  Measured distance  : " << std::setprecision(4) << actual_distance << " m\n";
    std::clog << "  Expected distance  : " << std::setprecision(4) << expected_distance << " m\n";
    std::clog << "  Wheel rotation     : " << std::setprecision(1) << wheel_travel_rad << " rad\n";
    std::clog << "  Scale factor       : " << std::setprecision(6) << config.scale_factor << "\n";
    ...
    std::clog << "  bazel run --config=arm64 //pico_interface:run_state_estimator -- user@host\n";
