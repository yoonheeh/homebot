...
TEST(TelemetryTest, VerifySpanExported) {
    ...
    auto memory_exporter = std::make_unique<opentelemetry::exporter::memory::InMemorySpanExporter>();
...
TEST(TelemetryTest, VerifyStateEstimatorSpans) {
    ...
    // 2. Setup the Processor and Provider with shared_ptr (required by SetTracerProvider)
    auto processor = std::unique_ptr<sdktrace::SpanProcessor>(
        new sdktrace::SimpleSpanProcessor(std::move(memory_exporter)));
    ...
    // 6. Assert that the spans are in the memory buffer and verify their attributes
    auto span_data_collection = exporter_ptr->GetData();
    auto spans = span_data_collection->GetSpans();
