...
public:
    TeleopController(IInputProvider& input, IMotorDriver& motors, ITelemetryPublisher& telemetry)
    ...
    void step() {
        ...
        if (w_pressed) { left_target = LINEAR_SPEED; right_target = LINEAR_SPEED; } 
        else if (s_pressed) { left_target = -LINEAR_SPEED; right_target = -LINEAR_SPEED; }

        if (d_pressed) { left_target += TURN_SPEED; right_target -= TURN_SPEED; } 
        else if (a_pressed) { left_target -= TURN_SPEED; right_target += TURN_SPEED; }
    ...
    void stopRobot() {
        motors_.sendVelocityCommand(0.0f, 0.0f);
    }
