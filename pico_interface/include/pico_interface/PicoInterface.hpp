        ...
        while (run_reader_) {
            ...
            if (bytes_read > 0) {
                if (c == 0x00) {
                    ...
                    size_t decoded_len = cobs_decode(rx_buffer, rx_index, decoded, sizeof(decoded));
                    ...
                    if (decoded_len == sizeof(EncoderIMUTelemetry)) {
                        ...
                        std::memcpy(&received_crc,
                                  ...
                                  sizeof(uint16_t));
                        uint16_t check_crc = calculate_crc16(decoded, sizeof(EncoderIMUTelemetry) - sizeof(uint16_t));
                ...
                // Short sleep in microseconds to reduce CPU usage when no characters are ready
                usleep(100);
            }
        }
    }
...
public:
    PicoInterface(const std::string& port_name, TelemetryQueue<EncoderIMUTelemetry>& rx_queue)
