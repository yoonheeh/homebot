...
void test_queue_threading() {
  ...
  TelemetryQueue<EncoderIMUTelemetry> queue(
      50);  // Small size to force evictions
...
void test_ekf_straight_line() {
  ...
  std::clog << "  - Final Pose: X=" << pose.x << ", Y=" << pose.y
            << ", Theta=" << pose.theta << "\n";
  TEST_ASSERT(std::abs(pose.x - expected_dist) < 0.001,
              "X displacement is wrong");
...
void test_ekf_pure_rotation() {
  ...
  // For 1 full wheel rotation difference: Left -1 rotation (-1440 ticks), Right
  // +1 rotation (+1440 ticks) Delta left ticks = -1440, Delta right ticks =
  // +1440 d_s_L = -0.2073, d_s_R = +0.2073 d_s = 0.0 (No translation!) d_theta
  // = (d_s_R - d_s_L) / Wheelbase = (0.2073 - (-0.2073)) / 0.16 = 0.41469 /
  // 0.16 = 2.5918 rad
  EncoderIMUTelemetry t2{};
  ...
  t2.gyro_z = 25.918;  // Make IMU yaw rate match exactly to avoid innovation
                       // corrections
  ...
  TEST_ASSERT(std::abs(pose.theta - expected_theta) < 0.001,
              "Rotation heading is wrong");
...
void test_ekf_sensor_fusion() {
  ...
  // d_theta = (d_s_R - d_s_L) / Wheelbase = 0.5 => d_s_R - d_s_L = 0.5 * 0.16 =
  // 0.08 m Let d_s_R = 0.04 m, d_s_L = -0.04 m (d_s = 0.0) Ticks = d_s / (2 *
  // pi * R) * TICKS_PER_REV = 0.04 / (2 * pi * 0.033) * 1440 = 277.78 ticks
  EncoderIMUTelemetry t2{};
  t2.timestamp_us = 1100000;  // dt = 0.1s
  ...
  // Encoder heading prediction: d_theta_enc = (2 * (278/1440) * 2 * pi * 0.033)
  // / 0.16 = 0.5003 rad IMU integrated heading: imu_theta = 1.0 rad Since
  // Kalman Gain is non-zero, the posterior heading should be strictly between
  // 0.5003 and 1.0.
  std::clog << "  - Encoder Only: ~0.5003 rad, IMU Only: 1.0000 rad\n";
  std::clog << "  - EKF Fused Heading: " << pose.theta << " rad\n";

  TEST_ASSERT(
      pose.theta > 0.5003 && pose.theta < 1.0,
      "Fused heading must lie between encoder prediction and IMU update");
