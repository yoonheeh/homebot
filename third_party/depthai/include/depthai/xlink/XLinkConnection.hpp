...
struct DeviceInfo {
  DeviceInfo() = default;
  DeviceInfo(std::string name, std::string deviceId, XLinkDeviceState_t state,
             XLinkProtocol_t protocol, XLinkPlatform_t platform,
             XLinkError_t status);
  /**
   * Creates a DeviceInfo by checking whether supplied parameter is a DeviceID
   * or IP/USB name
...
class XLinkConnection {
 public:
   ...
   * @param skipInvalidDevices whether or not to skip over devices that cannot
   * be successfully communicated with
   ...
   */
  static std::tuple<bool, DeviceInfo> getFirstDevice(
      XLinkDeviceState_t state = X_LINK_ANY_STATE,
      bool skipInvalidDevices = true);
   ...
   * @param skipInvalidDevices Whether or not to skip devices that cannot be
   * fully detected
   * @returns Tuple of bool and DeviceInfo. Bool specifies if device was found.
   * DeviceInfo specifies the found device
   */
  static std::tuple<bool, DeviceInfo> getDeviceById(
      std::string deviceId, XLinkDeviceState_t state = X_LINK_ANY_STATE,
      bool skipInvalidDevice = true);

  /**
   * Tries booting the given device into bootloader state
   *
   * @param devInfo Information of device which it should boot into bootloader
   * state
 ...
 private:
  ...
  // static
  static bool bootAvailableDevice(const deviceDesc_t &deviceToBoot,
