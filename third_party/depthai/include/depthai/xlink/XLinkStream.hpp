...
class XLinkStream {
 ...
 public:
  XLinkStream(const std::shared_ptr<XLinkConnection> conn,
  ...
  void read(std::vector<std::uint8_t> &data,
            XLinkTimespec &timestampReceived) const;
  ...
  bool read(std::vector<std::uint8_t> &data,
            std::chrono::milliseconds timeout) const;
  bool readMove(StreamPacketDesc &packet,
                const std::chrono::milliseconds timeout) const;
  // TODO optional<StreamPacketDesc> readMove(timeout) -or- tuple<bool,
  // StreamPacketDesc> readMove(timeout)

  // deprecated use readMove() instead; readRaw leads to memory violations
  // and/or memory leaks
  [[deprecated("use readMove()")]] streamPacketDesc_t *readRaw() const;
  // deprecated use readMove(packet, timeout) instead; readRaw leads to memory
  // violations and/or memory leaks
  [[deprecated("use readMove(packet, timeout)")]] bool readRaw(
      streamPacketDesc_t *&pPacket, std::chrono::milliseconds timeout) const;
...
struct XLinkError : public std::runtime_error {
  ...
  XLinkError(XLinkError_t statusID, std::string stream,
             const std::string &message)
      : runtime_error(message),
        status(statusID),
        streamName(std::move(stream)) {}
};
