...
/**
 * @brief RTABMap SLAM node. Performs SLAM on given odometry pose, rectified
 frame and depth frame.

*/
class RTABMapSLAM
    : public dai::NodeCRTP<dai::node::ThreadedHostNode, RTABMapSLAM> {
 public:
  ...
  Input odom{*this,
             {"odom",
              ...
              DEFAULT_BLOCKING,
              15,
              {{{dai::DatatypeEnum::TransformData, true}}}}};

  /**
   * Output transform.
   */
  Output transform{*this,
                   {"transform",
                    DEFAULT_GROUP,
                    {{{dai::DatatypeEnum::TransformData, true}}}}};
  /**
   ...
   */
  Output obstaclePCL{*this,
                     {"obstaclePCL",
                      DEFAULT_GROUP,
                      {{{dai::DatatypeEnum::PointCloudData, true}}}}};
  /**
   * Output ground point cloud.
   */
  Output groundPCL{*this,
                   {"groundPCL",
                    DEFAULT_GROUP,
                    {{{dai::DatatypeEnum::PointCloudData, true}}}}};
  /**
   ...
   */
  Output occupancyGridMap{*this,
                          {"occupancyGridMap",
                           DEFAULT_GROUP,
                           {{{dai::DatatypeEnum::MapData, true}}}}};

  /**
   * Output passthrough rectified image.
   */
  Output passthroughRect{*this,
                         {"passthroughRect",
                          DEFAULT_GROUP,
  ...
  Output passthroughFeatures{*this,
                             {"passthroughFeatures",
                              DEFAULT_GROUP,
                              {{{dai::DatatypeEnum::TrackedFeatures, true}}}}};
  /**
   * Output passthrough odometry pose.
   */
  Output passthroughOdom{*this,
                         {"passthroughOdom",
                          DEFAULT_GROUP,
                          {{{dai::DatatypeEnum::TransformData, true}}}}};
   ...
   */
  void setLoadDatabaseOnStart(bool load) { loadDatabaseOnStart = load; }

  /**
   * Whether to save the database on close. False by default.
   */
  void setSaveDatabaseOnClose(bool save) { saveDatabaseOnClose = save; }
 ...
 private:
  ...
  void initialize(dai::Pipeline &pipeline, int instanceNum, int width,
                  int height);
