...
class PipelineEventDispatcherInterface {
 ...
 public:
  class BlockPipelineEvent {
    ...
    std::optional<std::chrono::time_point<std::chrono::steady_clock>> endTs =
        std::nullopt;

   public:
    void start(std::optional<std::chrono::time_point<std::chrono::steady_clock>>
                   ts = std::nullopt) {
      PipelineEvent event;
      event.type = type;
      event.source = source;
    ...
    BlockPipelineEvent(
        PipelineEventDispatcherInterface &dispatcher, PipelineEvent::Type type,
    ...
    void end() {
      ...
      event.status = canceled ? PipelineEvent::Status::CANCELLED
                              : PipelineEvent::Status::SUCCESS;
      dispatcher.endTrackedEvent(event, endTs);
      canceled = true;
    }
    void cancel() { canceled = true; }
    void setQueueSize(uint32_t qs) { queueSize = qs; }
    void setEndTimestamp(
        std::chrono::time_point<std::chrono::steady_clock> ts) {
      endTs = ts;
    }
    ~BlockPipelineEvent() { end(); }
  };

  bool sendEvents = true;

  virtual ~PipelineEventDispatcherInterface();
  virtual void startEvent(PipelineEvent::Type type, const std::string &source,
                          std::optional<uint32_t> queueSize = std::nullopt) = 0;
  virtual void startInputEvent(
      const std::string &source,
      std::optional<uint32_t> queueSize = std::nullopt) = 0;
  virtual void startOutputEvent(const std::string &source) = 0;
  virtual void startCustomEvent(const std::string &source) = 0;
  virtual void endEvent(PipelineEvent::Type type, const std::string &source,
                        std::optional<uint32_t> queueSize = std::nullopt) = 0;
  virtual void endInputEvent(
      const std::string &source,
      std::optional<uint32_t> queueSize = std::nullopt) = 0;
  virtual void endOutputEvent(const std::string &source) = 0;
  virtual void endCustomEvent(const std::string &source) = 0;
  virtual void startTrackedEvent(
      PipelineEvent event,
      std::optional<std::chrono::time_point<std::chrono::steady_clock>> ts =
          std::nullopt) = 0;
  virtual void startTrackedEvent(
      PipelineEvent::Type type, const std::string &source, int64_t sequenceNum,
      std::optional<std::chrono::time_point<std::chrono::steady_clock>> ts =
          std::nullopt) = 0;
  virtual void endTrackedEvent(
      PipelineEvent event,
      std::optional<std::chrono::time_point<std::chrono::steady_clock>> ts =
          std::nullopt) = 0;
  virtual void endTrackedEvent(
      ...
      std::optional<std::chrono::time_point<std::chrono::steady_clock>> ts =
          std::nullopt) = 0;
  virtual void pingEvent(PipelineEvent::Type type,
                         const std::string &source) = 0;
  virtual void pingMainLoopEvent() = 0;
  virtual void pingCustomEvent(const std::string &source) = 0;
  virtual void pingInputEvent(
      const std::string &source, PipelineEvent::Status status,
      std::optional<uint32_t> queueSize = std::nullopt) = 0;
  virtual BlockPipelineEvent blockEvent(
      PipelineEvent::Type type, const std::string &source,
      std::optional<std::chrono::time_point<std::chrono::steady_clock>> ts =
          std::nullopt,
