 ...
 */
std::vector<std::string> filenamesInArchive(
 ...
 */
std::vector<uint8_t> readFileInArchive(const std::filesystem::path &archivePath,
 ...
 * @param filesOnDisk Vector of paths to file on the host filesystem to include
 * in the archive
 ...
 * @param filesInArchive Vector of paths for the files within the archive
 */
void archiveFilesCompressed(
    const std::filesystem::path &archivePath,
    const std::vector<std::filesystem::path> &filesOnDisk,
    const std::vector<std::string> &filesInArchive);

/**
 * Extracts files from an archive, supports both compressed (e.g. tar.gz) and
 * uncompressed (e.g. tar) archives.
 * @param archivePath Path to the archive file to extract from
 * @param filesInArchive Vector of paths for the files within the archive to
 * extract
 * @param filesOnDisk Vector of paths where the extracted files should be
 * written
 */
