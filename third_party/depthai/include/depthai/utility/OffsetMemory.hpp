...
/**
 * Memory interface that exposes a logical offset into an underlying byte
 * buffer.
 */
class OffsetMemory : public Memory {
 public:
  ...
  /**
   * Gets the number of bytes available from the current offset to the end of
   * the buffer.
   *
   * @return Remaining buffer size after the offset.
   */
  std::size_t getOffsetSize() const { return getOffsetData().size(); };
};

/**
 * Adapter that exposes any `Memory` instance through the `OffsetMemory`
 * interface.
 */
class ConvertedOffsetMemory : public OffsetMemory {
 ...
 public:
   ...
   */
  explicit ConvertedOffsetMemory(std::shared_ptr<Memory> memory)
      : memory(std::move(memory)) {
    if (this->memory == nullptr)
      throw std::invalid_argument(
          "ConvertedOffsetMemory requires non-null memory");
  }

  span<std::uint8_t> getData() override { return memory->getData(); }
  span<const std::uint8_t> getData() const override {
    return memory->getData();
  }
  std::size_t getMaxSize() const override { return memory->getMaxSize(); }
  std::size_t getOffset() const override { return dataOffset; }
   ...
   */
  std::shared_ptr<Memory> getInternal() { return memory; }

  /**
   * Converts a generic memory object into an `OffsetMemory`.
   *
   * Existing `OffsetMemory` instances are returned unchanged, while other
   * memory implementations are wrapped in `ConvertedOffsetMemory`.
   *
   * @param memory Memory object to convert.
   * @return Offset-aware view of the provided memory, or `nullptr` if `memory`
   * is null.
   */
  static std::shared_ptr<OffsetMemory> convert(
   ...
   *
   * If the offset-aware instance is a `ConvertedOffsetMemory`, the cast is
   * performed against its wrapped memory object.
   *
