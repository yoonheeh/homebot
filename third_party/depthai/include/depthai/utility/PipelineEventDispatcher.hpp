...
class PipelineEventDispatcher : public PipelineEventDispatcherInterface {
 ...
 public:
  ...
  PipelineEventDispatcher(int64_t &nodeId, Node::Output *output)
      : nodeId(nodeId), out(output) {}

  void startEvent(PipelineEvent::Type type, const std::string &source,
  ...
  void endEvent(PipelineEvent::Type type, const std::string &source,
                std::optional<uint32_t> queueSize = std::nullopt) override;
  void endInputEvent(const std::string &source,
                     std::optional<uint32_t> queueSize = std::nullopt) override;
  void endOutputEvent(const std::string &source) override;
  void endCustomEvent(const std::string &source) override;
  // Timestamp in event is not used. If ts is specified ts is used, else curent
  // time is applied
  void startTrackedEvent(
      PipelineEvent event,
  ...
  BlockPipelineEvent customBlockEvent(const std::string &source,
                                      bool startNow = true) override;
};
