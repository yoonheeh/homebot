...
template <
    SerializationType TYPE, typename T,
...
template <SerializationType TYPE, typename T,
          std::enable_if_t<TYPE == SerializationType::JSON, bool> = true>
inline bool serialize(const T &obj, std::vector<std::uint8_t> &data) {
  nlohmann::json j = obj;
  auto json = j.dump();
...
class VectorWriter {
 public:
  ...
  nop::Status<void> Write(const void *begin, const void *end) {
    vector.insert(vector.end(), static_cast<const std::uint8_t *>(begin),
...
template <SerializationType TYPE, typename T,
          std::enable_if_t<TYPE == SerializationType::LIBNOP, bool> = true>
inline bool serialize(const T &obj, std::vector<std::uint8_t> &data) {
...
std::string jsonDisplay(const nlohmann::json &json, int level = 0,
                        int indent = 4);
...
#define DEPTHAI_NLOHMANN_JSON_OPTIONAL_FROM(v1) \
  if (nlohmann_json_j.contains(#v1))            \
    nlohmann_json_j[#v1].get_to(nlohmann_json_t.v1);
#define DEPTHAI_NLOHMANN_DEFINE_TYPE_OPTIONAL_NON_INTRUSIVE(Type, ...) \
  inline void to_json(nlohmann::json &nlohmann_json_j,                 \
                      const Type &nlohmann_json_t) {                   \
    DEPTHAI_NLOHMANN_JSON_EXPAND(DEPTHAI_NLOHMANN_JSON_PASTE(          \
        DEPTHAI_NLOHMANN_JSON_OPTIONAL_TO, __VA_ARGS__))               \
  }                                                                    \
  inline void from_json(const nlohmann::json &nlohmann_json_j,         \
                        Type &nlohmann_json_t) {                       \
    DEPTHAI_NLOHMANN_JSON_EXPAND(DEPTHAI_NLOHMANN_JSON_PASTE(          \
        DEPTHAI_NLOHMANN_JSON_OPTIONAL_FROM, __VA_ARGS__))             \
  }
...
#define DEPTHAI_SERIALIZE_OPTIONAL_EXT(...)                             \
  DEPTHAI_DEFERRED_EXPAND(                                              \
      DEPTHAI_NLOHMANN_DEFINE_TYPE_OPTIONAL_NON_INTRUSIVE(__VA_ARGS__)) \
  DEPTHAI_DEFERRED_EXPAND(NOP_EXTERNAL_STRUCTURE(__VA_ARGS__))

#define DEPTHAI_SERIALIZE_OPTIONAL(...)                             \
  DEPTHAI_DEFERRED_EXPAND(                                          \
      DEPTHAI_NLOHMANN_DEFINE_TYPE_OPTIONAL_INTRUSIVE(__VA_ARGS__)) \
  DEPTHAI_DEFERRED_EXPAND(NOP_STRUCTURE(__VA_ARGS__))

#define DEPTHAI_SERIALIZE_EXT(...)                             \
  DEPTHAI_DEFERRED_EXPAND(                                     \
      DEPTHAI_NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(__VA_ARGS__)) \
  DEPTHAI_DEFERRED_EXPAND(NOP_EXTERNAL_STRUCTURE(__VA_ARGS__))

#define DEPTHAI_SERIALIZE(Type, ...)                             \
  DEPTHAI_DEFERRED_EXPAND(                                       \
      DEPTHAI_NLOHMANN_DEFINE_TYPE_INTRUSIVE(Type, __VA_ARGS__)) \
