...
class FileData {
 public:
  ...
  explicit FileData(const std::shared_ptr<ImgFrame> &imgFrame,
                    std::string fileTag);
  explicit FileData(const std::shared_ptr<EncodedFrame> &encodedFrame,
                    std::string fileTag);
  // explicit FileData(const std::shared_ptr<NNData>& nnData, std::string
  // fileTag);
  explicit FileData(const std::shared_ptr<ImgDetections> &imgDetections,
                    std::string fileTag);
...
class FileGroup {
 public:
  ...
  void addFile(const std::optional<std::string> &fileTag,
  ...
  void addImageDetectionsPair(
      ...
      const std::shared_ptr<EncodedFrame> &encodedFrame,
      const std::shared_ptr<ImgDetections> &imgDetections);
  // void addImageNNDataPair(std::string fileTag, const
  // std::shared_ptr<ImgFrame>& imgFrame, const std::shared_ptr<NNData>&
  // imgDetections); void addImageNNDataPair(std::string fileTag, const
  // std::shared_ptr<EncodedFrame>& encodedFrame, const std::shared_ptr<NNData>&
  // imgDetections);
...
class EventsManager {
 public:
  explicit EventsManager(std::string apiKey = "",
                         bool uploadCachedOnStart = false);
  explicit EventsManager(bool uploadCachedOnStart)
      : EventsManager("", uploadCachedOnStart) {}
  ~EventsManager();
   ...
   */
  std::optional<std::string> sendEvent(
      const std::string &name, const std::vector<std::string> &tags = {},
      const std::unordered_map<std::string, std::string> &extras = {},
      const std::vector<std::string> &associateFiles = {});
  /**
   * Send a snap to the events service. Snaps should be used for sending images
   * and other files.
   * @param name Name of the snap
   * @param fileGroup FileGroup containing FileData objects to send
   * @param tags List of tags to send
   * @param extras Extra data to send
   * @param successCallback Callback to be called when the snap is successfully
   * uploaded to the hub
   * @param failureCallback Callback to be called if the snap upload is
   * unsuccessful
   * @return LocalID of the sent Snap
   */
  std::optional<std::string> sendSnap(
      const std::string &name, const std::shared_ptr<FileGroup> fileGroup,
      ...
      const std::function<void(SendSnapCallbackResult)> failureCallback =
          nullptr);
  /**
   * Send a snap to the events service, with an ImgFrame and ImgDetections pair
   * as files
   * @param name Name of the snap
   ...
   */
  std::optional<std::string> sendSnap(
      const std::string &name, const std::optional<std::string> &fileTag,
      const std::shared_ptr<ImgFrame> imgFrame,
      ...
      const std::function<void(SendSnapCallbackResult)> failureCallback =
          nullptr);
  /**
   * Wait for pending snaps/events to be processed by the background upload
   * thread
   * @param timeoutMs Timeout in milliseconds. 0 means wait until uploads are
   * finished, connection is dropped, or manager is stopped
   * @return true if the pending data was uploaded before timeout, false if not
   * - either because of timeout, dropped connection, or shutdown
   */
  bool waitForPendingUploads(uint64_t timeoutMs = 0);
  /**
   * Set whether to log the responses from the server. By default, logResponse
   * is set to false
  ...
  /**
   * Set whether to verify the SSL certificate. By default, verifySsl is set to
   * false
  ...
  /**
   * Set the cache directory for storing cached data. By default, the cache
   * directory is set to /internal/private
   * @param cacheDir Cache directory
   * @return void
   */
  void setCacheDir(const std::string &cacheDir);
  /**
   * Set whether to cache data if it cannot be sent. By default,
   * cacheIfCannotSend is set to false
 ...
 private:
   ...
   * @param retryOnFail Retry fetching on failure; when true, keeps retrying
   * until successful (except if the apiKey is empty)
   * @return bool
   */
   ...
   */
  bool uploadGroup(std::shared_ptr<SnapData> snapData,
                   dai::proto::event::FileUploadGroupResult prepareGroupResult);
  /**
  ...
  /**
   * Validate the input event by checking that its fields adhere to defined
   * limitations
