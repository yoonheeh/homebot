...
namespace impl {
template <typename N>
void loop(N &node, const ImageManipConfig &initialConfig,
          std::shared_ptr<spdlog::async_logger> logger,
          const std::function<size_t(const ImageManipConfig &,
                                     const ImgFrame &)> &build,
          const std::function<std::shared_ptr<ImgFrame>(size_t)> &getFrame,
          const std::function<bool(const std::shared_ptr<OffsetMemory> &,
                                   std::shared_ptr<OffsetMemory>)> &apply,
  ...
  while (node.mainLoop()) {
    ...
    bool skipImage = false;
    {
      ...
      if (node.inputConfig.getWaitForMessage()) {
        ...
        hasConfig = true;
        if (inImage != nullptr && hasConfig &&
            pConfig->getReusePreviousImage()) {
          needsImage = false;
        }
      ...
      if (!node.inputConfig.getWaitForMessage() &&
          config.getReusePreviousImage()) {
        logger->warn(
            "reusePreviousImage is only taken into account when "
            "inputConfig is synchronous");
      }
    }
    ...
    // Check the output image size requirements, and check whether pool has the
    // size required
    if (outputSize == 0) {
      ...
      bool success = true;
      {
        ...
        success = apply(ConvertedOffsetMemory::convert(inImage->data),
                        ConvertedOffsetMemory::convert(outImage->data));
        auto t4 = steady_clock::now();

        setFrame(*inImage, *outImage);

        logger->trace(
            "Build time: {}us, Process time: {}us, Total time: {}us, "
            "image manip id: {}",
            duration_cast<microseconds>(t2 - t1).count(),
            duration_cast<microseconds>(t4 - t3).count(),
            duration_cast<microseconds>(t4 - t1).count(), node.id);
      }
      if (!success) {
      ...
      logger->error(
          "Output image is bigger ({}B) than maximum frame size "
          "specified in properties ({}B) - skipping frame.\nPlease "
          "use the setMaxOutputFrameSize "
          "API to explicitly config the [maximum] output size.",
          outputSize, node.properties.outputFrameSize);
    }

    // Update previousConfig of preprocessor, to be able to check if it needs to
    // be updated
    auto loopNanos = std::chrono::duration_cast<std::chrono::nanoseconds>(
                         std::chrono::steady_clock::now() - startP)
                         .count();
    logger->trace("ImageManip | total process took {}ns ({}ms)", loopNanos,
                  (double)loopNanos / 1e6);
  }
}
...
class _ImageManipMemory : public OffsetMemory {
  ...
  uint8_t *data() { return _span.data() + _offset; }
  const uint8_t *data() const { return _span.data() + _offset; }
  size_t size() const { return _span.size() - _offset; }
 ...
 public:
  ...
  span<uint8_t> getData() override { return _span; }
  span<const uint8_t> getData() const override { return _span; }
  ...
  size_t getMaxSize() const override { return _span.size(); }
...
class _ImageManipBuffer {
 ...
 public:
  ...
  span<T> getData() { return span(data(), data() + size()); }
  span<const T> getData() const { return span(data(), data() + size()); }
};
...
class UndistortOpenCvImpl {
 ...
 private:
  ...
  void initMaps(std::array<float, 9> cameraMatrix,
                std::array<float, 9> newCameraMatrix,
                std::vector<float> distCoeffs, dai::ImgFrame::Type type,
...
class Warp {
 ...
 public:
  ...
  ImageManipOpsBase<Container>::Background background =
      ImageManipOpsBase<Container>::Background::COLOR;
  ...
  virtual void build(
      const FrameSpecs srcFrameSpecs, const FrameSpecs dstFrameSpecs,
  ...
  virtual void buildUndistort(bool enable,
                              ...
                              const ImgFrame::Type type,
                              const uint32_t srcWidth, const uint32_t srcHeight,
                              const uint32_t dstWidth,
                              const uint32_t dstHeight) = 0;

  virtual void apply(const std::shared_ptr<OffsetMemory> &src,
                     std::shared_ptr<OffsetMemory> dst) = 0;
...
class WarpH : public Warp {
  ...
  void transform(const std::shared_ptr<OffsetMemory> &srcData,
                 std::shared_ptr<OffsetMemory> dstData, const size_t srcWidth,
                 const size_t srcHeight, const size_t srcStride,
                 const size_t dstWidth, const size_t dstHeight,
                 const size_t dstStride, const uint16_t numChannels,
                 const uint16_t bpp,
                 const std::array<std::array<float, 3>, 3> matrix,
 ...
 public:
  ...
  void buildUndistort(bool enable, const std::array<float, 9> &cameraMatrix,
                      const std::array<float, 9> &newCameraMatrix,
                      const std::vector<float> &distCoeffs,
                      const ImgFrame::Type type, const uint32_t srcWidth,
                      const uint32_t srcHeight, const uint32_t dstWidth,
                      const uint32_t dstHeight) override;

  void apply(const std::shared_ptr<OffsetMemory> &src,
             std::shared_ptr<OffsetMemory> dst) override;
};
...
class ColorChangeH : public ColorChange {
 ...
 public:
  void build(const FrameSpecs srcFrameSpecs, const FrameSpecs dstFrameSpecs,
             const ImgFrame::Type typeFrom, const ImgFrame::Type typeTo);

  void apply(const std::shared_ptr<OffsetMemory> &src,
             std::shared_ptr<OffsetMemory> dst);
};

template <typename ImageManipData, typename ColorChangeBackend,
          typename WarpBackend>
class ImageManipOperations {
  static_assert(std::is_base_of<OffsetMemory, ImageManipData>::value,
                "ImageManipData must be derived from OffsetMemory");
  static_assert(std::is_base_of<Warp, WarpBackend>::value,
                "WarpBackend must be derived from Warp");
  static_assert(std::is_base_of<ColorChange, ColorChangeBackend>::value,
                "ColorChangeBackend must be derived from ColorChange");
  using Container = std::vector<ManipOp>;
  ...
  std::array<std::array<float, 3>, 3> matrixInv{
      {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}}};
 ...
 public:
  ImageManipOperations(ImageManipProperties props,
  ...
  ImageManipOperations &build(const ImageManipOpsBase<Container> &base,
                              ImgFrame::Type outputFrameType,
                              FrameSpecs srcFrameSpecs, ImgFrame::Type type);
  ImageManipOperations &buildUndistort(
      bool enable, const std::array<float, 9> &cameraMatrix,
      const std::array<float, 9> &newCameraMatrix,
      const std::vector<float> &distCoeffs, const ImgFrame::Type type,
      const uint32_t srcWidth, const uint32_t srcHeight,
      const uint32_t dstWidth, const uint32_t dstHeight);

  bool apply(const std::shared_ptr<OffsetMemory> &src,
             std::shared_ptr<OffsetMemory> dst);
  ...
  ImgFrame::Type getOutputFrameType() const { return outputFrameType; }
  std::vector<RotatedRect> getSrcCrops() const;
  std::array<std::array<float, 3>, 3> getMatrix() const;
  bool undistortEnabled() const { return warpEngine.enableUndistort; }
...
FrameSpecs getCcDstFrameSpecs(FrameSpecs srcSpecs, dai::ImgFrame::Type from,
                              dai::ImgFrame::Type to);
FrameSpecs getDstFrameSpecs(size_t width, size_t height,
                            dai::ImgFrame::Type type);
size_t getAlignedOutputFrameSize(ImgFrame::Type type, size_t width,
                                 size_t height);
...
bool getFrameTypeInfo(dai::ImgFrame::Type outFrameType, int &outNumPlanes,
                      float &outBpp);
...
std::tuple<float, float, float, float> getOuterRect(
    const std::vector<std::array<float, 2>> points);
dai::RotatedRect getOuterRotatedRect(
    const std::vector<std::array<float, 2>> &points);
std::array<std::array<float, 3>, 3> getResizeMat(Resize o, float width,
                                                 float height,
                                                 uint32_t outputWidth,
                                                 uint32_t outputHeight);
void getOutputSizeFromCorners(
    const std::array<std::array<float, 2>, 4> &corners, const bool center,
    const std::array<std::array<float, 3>, 3> transformInv,
    const uint32_t srcWidth, const uint32_t srcHeight, uint32_t &outputWidth,
    uint32_t &outputHeight);

void getTransformImpl(
    const ManipOp &op, std::array<std::array<float, 3>, 3> &transform,
...
inline bool isSingleChannelu8(const std::shared_ptr<dai::ImgFrame> img) {
  return img->getType() == dai::ImgFrame::Type::GRAY8 ||
         img->getType() == dai::ImgFrame::Type::RAW8;
}
inline bool isSingleChannelu8(const dai::ImgFrame::Type type) {
  return type == dai::ImgFrame::Type::GRAY8 ||
...
std::string getConfigString(const dai::ImageManipOpsBase<C> &ops) {
  ...
  for (auto i = 0U; i < operations.size(); ++i) {
    configSS << std::visit([](auto &&op) { return getOpStr(op); },
                           operations[i].op);
    if (i != operations.size() - 1) configSS << " ";
  }
  configSS << "| o=" << ops.outputWidth << "x" << ops.outputHeight
           << " c=" << ops.center << " rm=" << (int)ops.resizeMode
           << " b=" << (int)ops.background << " bc=" << ops.backgroundR << ","
           << ops.backgroundG << "," << ops.backgroundB
           << " c=" << (int)ops.colormap << " u=" << (int)ops.undistort;
  return configSS.str();
}

inline std::array<std::array<float, 3>, 3> matmul(
...
inline std::array<float, 2> matvecmul(std::array<std::array<float, 3>, 3> M,
...
inline std::array<float, 2> matvecmul(std::array<std::array<float, 2>, 2> M,
                                      std::array<float, 2> vec) {
...
template <typename C>
std::tuple<std::array<std::array<float, 3>, 3>,
           std::array<std::array<float, 2>, 4>,
           std::vector<std::array<std::array<float, 2>, 4>>>
getTransform(const C &ops, uint32_t inputWidth, uint32_t inputHeight,
             uint32_t outputWidth, uint32_t outputHeight) {
  ...
  for (const auto &op : ops) {
    getTransformImpl(op, transform, imageCorners, srcCorners, outputWidth,
                     outputHeight);
  }
...
std::tuple<std::array<std::array<float, 3>, 3>,
           std::array<std::array<float, 2>, 4>,
           std::vector<std::array<std::array<float, 2>, 4>>>
getFullTransform(dai::ImageManipOpsBase<C> &base, size_t inputWidth,
                 size_t inputHeight, dai::ImgFrame::Type type,
                 dai::ImgFrame::Type outputFrameType, C &outputOps) {
  using namespace dai;
  ...
  auto [matrix, imageCorners, srcCorners] = getTransform(
      operations, inputWidth, inputHeight, base.outputWidth, base.outputHeight);

  getOutputSizeFromCorners(imageCorners, base.center,
                           matrix::getMatrixInverse(matrix), inputWidth,
                           inputHeight, base.outputWidth, base.outputHeight);
  ...
  if (base.resizeMode != ImageManipOpsBase<C>::ResizeMode::NONE) {
    ...
    auto [minx, maxx, miny, maxy] =
        getOuterRect(std::vector(imageCorners.begin(), imageCorners.end()));
    auto mat = getResizeMat(res, maxx - minx, maxy - miny, base.outputWidth,
                            base.outputHeight);
    imageCorners = {{{matvecmul(mat, imageCorners[0])},
                     {matvecmul(mat, imageCorners[1])},
                     {matvecmul(mat, imageCorners[2])},
                     {matvecmul(mat, imageCorners[2])}}};
  ...
  if (type == ImgFrame::Type::NV12 || type == ImgFrame::Type::YUV420p ||
      outputFrameType == ImgFrame::Type::NV12 ||
  ...
  srcCorners.push_back({matvecmul(matrixInv, {0, 0}),
                        ...
                        matvecmul(matrixInv, {(float)base.outputWidth,
                                              (float)base.outputHeight}),
...
template <typename ImageManipData, typename ColorChangeBackend,
          typename WarpBackend>
ImageManipOperations<ImageManipData, ColorChangeBackend, WarpBackend> &
ImageManipOperations<ImageManipData, ColorChangeBackend, WarpBackend>::build(
    const ImageManipOpsBase<Container> &newBase, ImgFrame::Type outType,
    FrameSpecs srcFrameSpecs, ImgFrame::Type inFrameType) {
  ...
  if (newCfgStr == prevConfig && outType == outputFrameType &&
      srcFrameSpecs.width == srcSpecs.width &&
      srcFrameSpecs.height == srcSpecs.height && inFrameType == inType)
    return *this;
  ...
  if (newBase.hasWarp(srcFrameSpecs.width, srcFrameSpecs.height))
    mode = mode | MODE_WARP;
  if (newBase.colormap != Colormap::NONE && isSingleChannelu8(inFrameType))
    mode = mode | MODE_COLORMAP;
  ...
  if (!isSingleChannelu8(inType) && base.colormap != Colormap::NONE) {
    if (logger)
      logger->warn(
          "ImageManip | Colormap can only be applied to single "
          "channel images, ignoring colormap");
  ...
  if (mode & MODE_WARP) {
    auto [matrix, imageCorners, _srcCorners] = getFullTransform<Container>(
        base, inputWidth, inputHeight, type, outputFrameType, outputOps);
    ...
    if (logger) {
      ...
      logger->trace("|{} {}|{} {}|", imageCorners[0][0], imageCorners[0][1],
                    imageCorners[1][0], imageCorners[1][1]);
      logger->trace("-------------");
      logger->trace("|{} {}|{} {}|", imageCorners[3][0], imageCorners[3][1],
                    imageCorners[2][0], imageCorners[2][1]);
      ...
      logger->trace("|{} {} {}|", matrixInv[0][0], matrixInv[0][1],
                    matrixInv[0][2]);
      logger->trace("-------------");
      logger->trace("|{} {} {}|", matrixInv[1][0], matrixInv[1][1],
                    matrixInv[1][2]);
      logger->trace("-------------");
      logger->trace("|{} {} {}|", matrixInv[2][0], matrixInv[2][1],
                    matrixInv[2][2]);
    }

    warpEngine.build(srcSpecs, getOutputFrameSpecs(type), type, matrix,
                     srcCorners);
    warpEngine.setBackgroundColor(base.backgroundR, base.backgroundG,
                                  base.backgroundB);
    clrChange.build(getOutputFrameSpecs(type),
                    getOutputFrameSpecs(outputFrameType), type,
                    outputFrameType);
  } else {
    base.outputWidth = inputWidth;
    base.outputHeight = inputHeight;
  }
  size_t newConvertedSize =
      getAlignedOutputFrameSize(type, inputWidth, inputHeight);
  size_t newColormapSize =
      getAlignedOutputFrameSize(type, base.outputWidth, base.outputHeight);
  size_t newWarpedSize = getAlignedOutputFrameSize(
      isSingleChannelu8(type) && base.colormap != Colormap::NONE
          ? VALID_TYPE_COLOR
          : type,
      base.outputWidth, base.outputHeight);

  if (!convertedFrame || convertedFrame->getOffsetSize() < newConvertedSize)
    convertedFrame = std::make_shared<ImageManipData>(newConvertedSize);
  if (!colormapFrame || colormapFrame->getOffsetSize() < newColormapSize)
    colormapFrame = std::make_shared<ImageManipData>(newColormapSize);
  if (!warpedFrame || warpedFrame->getOffsetSize() < newWarpedSize)
    warpedFrame = std::make_shared<ImageManipData>(newWarpedSize);
...
ImageManipOperations<ImageManipData, ColorChangeBackend, WarpBackend> &
ImageManipOperations<ImageManipData, ColorChangeBackend, WarpBackend>::
    buildUndistort(bool enable, const std::array<float, 9> &cameraMatrix,
                   const std::array<float, 9> &newCameraMatrix,
                   const std::vector<float> &distCoeffs,
                   const ImgFrame::Type type, const uint32_t srcWidth,
                   const uint32_t srcHeight, const uint32_t dstWidth,
                   const uint32_t dstHeight) {
  warpEngine.buildUndistort(enable, cameraMatrix, newCameraMatrix, distCoeffs,
                            type, srcWidth, srcHeight, dstWidth, dstHeight);
  return *this;
}

template <typename ImageManipData, typename ColorChangeBackend,
          typename WarpBackend>
bool ImageManipOperations<ImageManipData, ColorChangeBackend, WarpBackend>::
    apply(const std::shared_ptr<OffsetMemory> &src,
          std::shared_ptr<OffsetMemory> dst) {
  size_t requiredSize = getFrameSize(inType, srcSpecs);
  if (src->getOffsetSize() < requiredSize)
    throw std::runtime_error(
        "ImageManip not built for the source image specs. "
        "Consider rebuilding with the new configuration.");
  if (mode == 0) {
    std::copy(src->getOffsetData().begin(), src->getOffsetData().end(),
              dst->getOffsetData().begin());
    return true;
  }

#ifdef DEPTHAI_HAVE_OPENCV_SUPPORT
  if (convertInput || mode == MODE_CONVERT)
    preprocCc.apply(src, mode == MODE_CONVERT ? dst : convertedFrame);
  if (mode != MODE_CONVERT) {
    if (mode & MODE_WARP) {
      warpEngine.apply(convertInput ? convertedFrame : src,
                       base.colormap != Colormap::NONE
                           ? colormapFrame
                           : (type == outputFrameType ? dst : warpedFrame));
    }
    if (mode & MODE_COLORMAP) {
      uint8_t *colormapDst = outputFrameType == VALID_TYPE_COLOR
                                 ? dst->getOffsetData().data()
                                 : warpedFrame->getOffsetData().data();
      size_t colormapDstStride =
          outputFrameType == VALID_TYPE_COLOR
              ? getOutputStride()
              : ALIGN_UP(base.outputWidth, DEPTHAI_STRIDE_ALIGNMENT);
      uint8_t *colormapSrc =
          mode & MODE_WARP
              ? colormapFrame->getOffsetData().data()
              : (convertInput ? convertedFrame->getOffsetData().data()
                              : src->getOffsetData().data());
      size_t colormapSrcStride =
          !(mode & MODE_WARP) && !convertInput
              ? srcSpecs.p1Stride
              : ALIGN_UP(base.outputWidth, DEPTHAI_STRIDE_ALIGNMENT);
      cv::Mat gray(base.outputWidth, base.outputHeight, CV_8UC1, colormapSrc,
                   colormapSrcStride);
    ...
    // Change color(format) if outputFrameType is not None / the same as the
    // current frame type and not (frame type is single channel + colormap is
    // applied
    // + output frame type is RGBi)
    if (type != outputFrameType &&
        !(isSingleChannelu8(type) && base.colormap != Colormap::NONE &&
          outputFrameType == VALID_TYPE_COLOR)) {
...
template <typename ImageManipData, typename ColorChangeBackend,
          typename WarpBackend>
size_t ImageManipOperations<ImageManipData, ColorChangeBackend,
                            WarpBackend>::getOutputWidth() const {
...
template <typename ImageManipData, typename ColorChangeBackend,
          typename WarpBackend>
size_t ImageManipOperations<ImageManipData, ColorChangeBackend,
                            WarpBackend>::getOutputHeight() const {
...
template <typename ImageManipData, typename ColorChangeBackend,
          typename WarpBackend>
size_t ImageManipOperations<ImageManipData, ColorChangeBackend,
                            WarpBackend>::getOutputStride(uint8_t plane) const {
  if (mode == 0)
    return plane == 0 ? srcSpecs.p1Stride
                      : (plane == 1 ? srcSpecs.p2Stride
                                    : (plane == 2 ? srcSpecs.p3Stride : 0));
...
template <typename ImageManipData, typename ColorChangeBackend,
          typename WarpBackend>
size_t ImageManipOperations<ImageManipData, ColorChangeBackend,
                            WarpBackend>::getOutputPlaneSize(uint8_t plane)
    const {
  if (mode == 0) return 0;
  size_t size = 0;
  switch (outputFrameType) {
    ...
    case ImgFrame::Type::BGR888i:
      size = getOutputStride() *
             getOutputHeight();  // Do not do stride for RGB/BGRi/p
      break;
    case ImgFrame::Type::GRAY8:
    case ImgFrame::Type::RAW8:
      size = getOutputStride() *
             ALIGN_UP(getOutputHeight(), DEPTHAI_HEIGHT_ALIGNMENT);
      break;
    case ImgFrame::Type::NV12:
      if (plane == 0) {
        size = getOutputStride(0) *
               ALIGN_UP(getOutputHeight(), DEPTHAI_HEIGHT_ALIGNMENT);
      } else if (plane == 1) {
        size = ALIGN_UP(
            getOutputStride(1) *
                ALIGN_UP(getOutputHeight() / 2, DEPTHAI_HEIGHT_ALIGNMENT / 2),
            DEPTHAI_PLANE_ALIGNMENT);
      }
      break;
  ...
  if (size == 0)
    throw std::runtime_error("Output size is 0 for plane " +
                             std::to_string(plane));
  return size;
}
...
template <typename ImageManipData, typename ColorChangeBackend,
          typename WarpBackend>
size_t ImageManipOperations<ImageManipData, ColorChangeBackend,
                            WarpBackend>::getOutputSize() const {
  if (mode == 0) return 0;
  size_t size = 0;
  switch (outputFrameType) {
    case ImgFrame::Type::RGB888p:
...
template <typename ImageManipData, typename ColorChangeBackend,
          typename WarpBackend>
FrameSpecs ImageManipOperations<
    ImageManipData, ColorChangeBackend,
    WarpBackend>::getOutputFrameSpecs(ImgFrame::Type type) const {
  if (mode == 0)
    return srcSpecs;
  else
...
template <typename ImageManipData, typename ColorChangeBackend,
          typename WarpBackend>
std::vector<RotatedRect> ImageManipOperations<
    ImageManipData, ColorChangeBackend, WarpBackend>::getSrcCrops() const {
  std::vector<RotatedRect> crops;
  for (const auto &corners : srcCorners) {
    auto rect =
        getOuterRotatedRect({corners[0], corners[1], corners[2], corners[3]});
...
template <typename ImageManipData, typename ColorChangeBackend,
          typename WarpBackend>
std::array<std::array<float, 3>, 3> ImageManipOperations<
    ImageManipData, ColorChangeBackend, WarpBackend>::getMatrix() const {
  return matrix;
}

template <typename ImageManipData, typename ColorChangeBackend,
          typename WarpBackend>
std::string ImageManipOperations<ImageManipData, ColorChangeBackend,
                                 WarpBackend>::toString() const {
  std::stringstream cStr;
  cStr << getConfigString(base);
  if (outputOps.size() > 0) {
    cStr << " | ";
    for (auto i = 0U; i < outputOps.size(); ++i) {
      cStr << std::visit([](auto &&op) { return getOpStr(op); },
                         outputOps[i].op);
