...
/**
 * @brief A simple circular buffer implementation with forward and reverse
 * iterators
...
class CircularBuffer {
  ...
  size_t start() const { return (buffer.size() < maxSize) ? 0 : index; }

  virtual void onSwap(T &, T &) {}
  virtual void onAdd(T &) {}

 public:
  CircularBuffer(size_t size) : maxSize(size) { buffer.reserve(size); }
  virtual ~CircularBuffer() = default;
  // Add a new element to the buffer after the last one or overwrite the oldest
  // one
  T &add(T value) {
  ...
  iterator end() { return iterator(this, buffer.size()); }
  ...
  class reverse_iterator {
   public:
    ...
    reference operator*() {
      ...
      size_t logicalIndex =
          (parent->start() + parent->buffer.size() - 1 - pos) %
          parent->buffer.size();
  ...
  reverse_iterator rend() { return reverse_iterator(this, buffer.size()); }
};

/**
 * @brief A circular buffer that maintains a running average of its elements
 * @tparam T The type of elements stored in the buffer (must support addition
 * and division)
 */
template <typename T>
class WindowedAverageBuffer : public CircularBuffer<T> {
 ...
 public:
  ...
  WindowedAverageBuffer(size_t size, T initialValue = T())
      : CircularBuffer<T>(size), sum(initialValue) {}

  void onSwap(T &oldValue, T &newValue) override {
    sum -= oldValue;
    sum += newValue;
  }
  void onAdd(T &newValue) override { sum += newValue; }
  T getAverage() const {
    if (this->size() == 0) {
