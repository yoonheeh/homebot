...
/**
 * @brief Basalt Visual Inertial Odometry node. Performs VIO on stereo images
 and IMU data.
...
class BasaltVIO : public NodeCRTP<ThreadedHostNode, BasaltVIO> {
 public:
  ...
  struct VioConfig {
    ...
    float
        optical_flow_image_safe_radius;  // Use to mask black corners in cameras
    ...
    bool optical_flow_recall_over_tracking;    // Always perform recall, even on
                                               // already tracked features
    bool optical_flow_recall_update_patch_viewpoint;  // Update feature patch
                                                      // when successfully
                                                      // recalled
    float optical_flow_recall_max_patch_dist;  // Maximum distance in % of width
                                               // to accept a recall, or 0
    std::vector<float>
        optical_flow_recall_max_patch_norms;  // Maximum patch residual norm to
                                              // accept a recall
  ...
  Input imu{
      *this,
      {"inIMU", DEFAULT_GROUP, false, 0, {{DatatypeEnum::IMUData, true}}}};
  ...
  Output passthrough{
      *this,
      {"imgPassthrough", DEFAULT_GROUP, {{DatatypeEnum::ImgFrame, true}}}};
