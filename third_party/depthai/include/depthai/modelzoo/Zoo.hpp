...
struct NNModelDescription {
   ...
   *        If modelName is a relative path (e.g. ./yolo.yaml), it is used as
   * is. If modelName is a full path (e.g. /home/user/models/yolo.yaml), it is
   * used as is. If modelName is a model name (e.g. yolo) or a model yaml file
   * (e.g. yolo.yaml), the function will use modelsPath if provided or the
   * DEPTHAI_ZOO_MODELS_PATH environment variable and use a path made by
   * combining the modelsPath and the model name to the yaml file. For instance,
   * yolo -> ./depthai_models/yolo.yaml (if modelsPath or
   * DEPTHAI_ZOO_MODELS_PATH are
   * ./depthai_models)
   *
   * @param modelName: model name or yaml file path (string is implicitly
   * converted to Path)
   * @param modelsPath: Path to the models folder, use environment variable
   * DEPTHAI_ZOO_MODELS_PATH if not provided
   * @return NNModelDescription
   */
  static NNModelDescription fromYamlFile(const fs::path &modelName,
  ...
  /**
   * @brief Check if the model description is valid (contains all required
   * fields)
   *
  ...
  /**
   * @brief Convert NNModelDescription to string for printing purposes. This can
 ...
 * @param cacheDirectory: Cache directory where the cached models are stored,
 * default is "". If cacheDirectory is set to "", this function checks the
 * DEPTHAI_ZOO_CACHE_PATH environment variable and uses that if set, otherwise
 * the default value is used (see getDefaultCachePath).
 * @param apiKey: API key for the model zoo, default is "". If apiKey is set to
 * "", this function checks the DEPTHAI_ZOO_API_KEY environment variable and
 * uses that if set. Otherwise, no API key is used.
 * @param progressFormat: Format to use for progress output (possible values:
 * pretty, json, none), default is "pretty"
...
/**
 * @brief Helper function allowing one to download all models specified in yaml
 * files in the given path and store them in the cache directory
 *
 * @param path: Path to the directory containing yaml files
 * @param cacheDirectory: Cache directory where the cached models are stored,
 * default is "". If cacheDirectory is set to "", this function checks the
 * DEPTHAI_ZOO_CACHE_PATH environment variable and uses that if set, otherwise
 * the default is used (see getDefaultCachePath).
 * @param apiKey: API key for the model zoo, default is "". If apiKey is set to
 * "", this function checks the DEPTHAI_ZOO_API_KEY environment variable and
 * uses that if set. Otherwise, no API key is used.
 * @param progressFormat: Format to use for progress output (possible values:
 * pretty, json, none), default is "pretty"
 * @return bool: True if all models were downloaded successfully, false
 * otherwise
 */
