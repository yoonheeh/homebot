...
/// Support for basic OpenVINO related actions like version identification of
/// neural network blobs,...
class OpenVINO {
 public:
  ...
  /**
   * @brief A superblob is an efficient way of storing generated blobs for all
   * different number of shaves.
   */
  class SuperBlob {
   public:
     ...
     * @brief Construct a new SuperBlob object
     *
     * @param pathToSuperBlobFile: Path to the superblob file (.superblob
     * suffix)
     */
     ...
     *
     * @param numShaves: Number of shaves to generate the blob for. Must be
     * between 1 and NUMBER_OF_PATCHES.
     * @return dai::OpenVINO::Blob: Blob compiled for the specified number of
     * shaves
     */
   ...
   private:
    ...
    struct SuperBlobHeader {
      static constexpr size_t HEADER_SIZE =
          1 * sizeof(uint64_t) + NUMBER_OF_PATCHES * sizeof(uint64_t);
    ...
    // Get a pointer to the first byte of the patch data for a specific number
    // of shaves
  ...
  /**
   * Returns a list of potentially supported versions for a specified blob major
   * and minor versions.
   ...
   */
  static std::vector<Version> getBlobSupportedVersions(
   ...
   */
  static Version getBlobLatestSupportedVersion(std::uint32_t majorVersion,
   ...
   * @returns Latest potentially supported version
   */
  static Version getBlobVersion(std::uint32_t majorVersion,
                                std::uint32_t minorVersion);
 ...
 private:
  static const std::map<std::pair<std::uint32_t, std::uint32_t>, Version>
      blobVersionToOpenvinoGuessMapping;
  static const std::map<std::pair<std::uint32_t, std::uint32_t>,
                        std::vector<Version>>
