...
DEPTHAI_SERIALIZE_EXT(TrackedFeature, position, id, age, harrisScore,
                      trackingError, descriptor);

/**
 * TrackedFeatures message. Carries position (X, Y) of tracked features and
 * their ID.
 */
class TrackedFeatures : public Buffer {
 public:
  ...
  DEPTHAI_SERIALIZE(TrackedFeatures, trackedFeatures, Buffer::sequenceNum,
                    Buffer::ts, Buffer::tsDevice);
};
