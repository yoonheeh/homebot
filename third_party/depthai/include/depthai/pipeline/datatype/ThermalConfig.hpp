...
class ThermalConfig : public Buffer {
 public:
  ...
  /**
   * Ambient factors that affect the temperature measurement of a Thermal
   * sensor.
   */
  struct ThermalAmbientParams {
    /// Distance to the measured object. unit:cnt(128cnt=1m),
    /// range:0-25600(0-200m)
    std::optional<uint16_t> distance;

    /// Reflection temperature. unit:K, range:230-500(high gain), 230-900(low
    /// gain)
    std::optional<uint16_t> reflectionTemperature;

    /// Atmospheric temperature. unit:K, range:230-500(high gain), 230-900(low
    /// gain)
    ...
    DEPTHAI_SERIALIZE(ThermalAmbientParams, distance, reflectionTemperature,
                      atmosphericTemperature, targetEmissivity,
                      atmosphericTransmittance, gainMode);
  };

  struct ThermalFFCParams {
    /// Auto Flat-Field-Correction. Controls wheather the shutter is controlled
    /// by the sensor module automatically or not.
    ...
    /// The condition for triggering the auto FFC is that the change of Vtemp
    /// value exceeds a certain threshold, which is called the Auto FFC trigger
    /// threshold.
    std::optional<uint16_t> autoFFCTempThreshold;

    /// The shutter blade may open/close abnormally during strong mechanical
    /// shock (such as fall), and a monitoring process is designed in the
    /// firmware to correct the abnormal shutter switch in time. Turn on or off
    /// the fall protect mechanism.
    std::optional<bool> fallProtection;

    /// Frequent FFC will cause shutter heating, resulting in abnormal FFC
    /// effect and abnormal temperature measurement. Regardless of which
    /// mechanism triggers FFC, the minimum trigger interval must be limited.
    std::optional<uint16_t> minShutterInterval;

    /// Set this to True/False to close/open the shutter when autoFFC is
    /// disabled.
    ...
    DEPTHAI_SERIALIZE(ThermalFFCParams, autoFFC, minFFCInterval, maxFFCInterval,
                      autoFFCTempThreshold, fallProtection, minShutterInterval,
                      closeManualShutter,
  ...
  struct ThermalImageParams {
    /// 0-3. Time noise filter level. Filters out the noise that appears over
    /// time.
    ...
    DEPTHAI_SERIALIZE(ThermalImageParams, timeNoiseFilterLevel,
                      spatialNoiseFilterLevel, digitalDetailEnhanceLevel,
