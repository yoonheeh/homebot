...
class ImgDetectionsT : public Buffer {
 ...
 public:
   ...
   */
  void setSegmentationMask(const std::vector<std::uint8_t> &mask, size_t width,
                           size_t height);
...
// Optional - OpenCV support
   ...
   * mask data is not set, returns std::nullopt.
   ...
   */
  std::optional<cv::Mat> getCvSegmentationMask(
      cv::MatAllocator *allocator = nullptr);

  /**
   * Returns a binary mask where pixels belonging to the instance index are set
   * to 1, others to 0. If mask data is not set, returns std::nullopt.
   * @param index Instance index
   * @param allocator Allows callers to supply a custom cv::MatAllocator for
   * zero-copy/custom memory management; nullptr uses OpenCV’s default.
   */
  std::optional<cv::Mat> getCvSegmentationMaskByIndex(
      uint8_t index, cv::MatAllocator *allocator = nullptr);
   ...
   * zero-copy/custom memory management; nullptr uses OpenCV’s default.
   */
  std::optional<cv::Mat> getCvSegmentationMaskByClass(
      uint8_t semanticClass, cv::MatAllocator *allocator = nullptr);
