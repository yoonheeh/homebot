...
class NNData : public Buffer {
 ...
 public:
   ...
   * @returns True if layer exists, false otherwise
   */
  bool getLayerDatatype(const std::string &name,
                        TensorInfo::DataType &datatype) const;
  ...
  /**
   * Emplace a tensor
   * This function allocates memory for the tensor and return over the said
   * memory. It is up to the caller to fill the memory out with meaningful data.
   ...
   * The provided array is stored as a 1xN tensor where N is the length of the
   * array.
   *
   * @param name: Name of the tensor
   * @param data: array
  ...
  NNData &addTensor(const std::string &name, const std::vector<_Ty> &data,
                    dai::TensorInfo::DataType dataType) {
    return addTensor<_Ty>(
        name, xt::adapt(data, std::vector<size_t>{1, data.size()}), dataType);
  };
  ...
  NNData &addTensor(const std::string &name, const std::vector<_Ty> &tensor) {
    if constexpr (std::is_same<_Ty, int>::value) {
      ...
      return addTensor<std::int8_t>(name, tensor,
                                    dai::TensorInfo::DataType::I8);
    } else if constexpr (std::is_same<_Ty, std::uint8_t>::value) {
      return addTensor<std::uint8_t>(name, tensor,
  ...
  NNData &addTensor(const std::string &name,
  ...
  NNData &addTensor(const std::string &name,
                    const std::vector<std::int8_t> &tensor) {
    return addTensor<std::int8_t>(name, tensor, dai::TensorInfo::DataType::I8);
  };
  NNData &addTensor(const std::string &name,
                    const std::vector<std::uint8_t> &tensor) {
    return addTensor<std::uint8_t>(name, tensor,
                                   dai::TensorInfo::DataType::U8F);
  };
  ...
  NNData &addTensor(const std::string &name,
                    const xt::xarray<uint16_t> &tensor) {
    return addTensor<uint16_t>(name, tensor, dai::TensorInfo::DataType::FP16);
  };
  ...
  NNData &addTensor(const std::string &name,
                    const xt::xarray<std::int8_t> &tensor) {
    return addTensor<std::int8_t>(name, tensor, dai::TensorInfo::DataType::I8);
  };
  NNData &addTensor(const std::string &name,
                    const xt::xarray<std::uint8_t> &tensor) {
    return addTensor<std::uint8_t>(name, tensor,
                                   dai::TensorInfo::DataType::U8F);
  };
  ...
  NNData &addTensor(const std::string &name, const std::vector<_Ty> &data,
    ...
    return addTensor<_Ty>(
        name, xt::adapt(data, std::vector<size_t>{1, data.size()}), order);
  };
  ...
  template <typename _Ty = double>
  NNData &addTensor(const std::string &name, const xt::xarray<_Ty> &data,
                    TensorInfo::StorageOrder order) {
    auto dataType = std::is_integral<_Ty>::value
                        ? dai::TensorInfo::DataType::U8F
  ...
  /**
   * @brief Add a tensor to this NNData object. The storage order is picked
   * based on the number of dimensions of the tensor. Float values are converted
   * to FP16 and integers are cast to bytes.
   *
   * @param name: Name of the tensor
   * @param tensor: tensor
   * @return NNData&: reference to this object
    ...
    switch (tensor.shape().size()) {
      ...
      default:
        throw std::runtime_error(
            "Unsupported tensor shape. Only 1D, 2D, 3D and "
            "4D tensors are supported");
    }
  ...
  NNData &addTensor(const std::string &name, const xt::xarray<_Ty> &tensor,
                    dai::TensorInfo::DataType dataType,
                    const TensorInfo::StorageOrder order) {
    static_assert(
        std::is_integral<_Ty>::value || std::is_floating_point<_Ty>::value,
        "Tensor type needs to be integral or floating point");
    ...
    if (std::dynamic_pointer_cast<VectorMemory>(data) == nullptr) {
      auto prev =
          std::vector<uint8_t>(data->getData().begin(), data->getData().end());
      data = std::make_shared<VectorMemory>(std::move(prev));
    }
    auto vecData = std::dynamic_pointer_cast<VectorMemory>(data);

    // Get size in bytes of the converted tensor data, u8 for integral and fp16
    // for floating point const size_t sConvertedData =
    // std::is_integral<_Ty>::value ? tensor.size() : 2 * tensor.size();
    ...
    size_t remainder =
        std::distance(vecData->begin(), vecData->end()) % DATA_ALIGNMENT;
    if (remainder > 0) {
    ...
    if (dataType == dai::TensorInfo::DataType::I8) {
      ...
      for (uint32_t i = 0; i < tensor.size(); i++) {
        *(uint16_t *)(&vecData->data()[2 * i + offset]) =
            (uint16_t)tensor.data()[i];
      }
    } else if (dataType == dai::TensorInfo::DataType::FP16) {
      for (uint32_t i = 0; i < tensor.size(); i++) {
        *(uint16_t *)(&vecData->data()[2 * i + offset]) =
            fp32_to_fp16(tensor.data()[i]);
      }
      ...
      for (uint32_t i = 0; i < tensor.size(); i++) {
        *(int32_t *)(&vecData->data()[4 * i + offset]) =
            (int32_t)tensor.data()[i];
      }
      ...
      for (uint32_t i = 0; i < tensor.size(); i++) {
        *(double *)(&vecData->data()[8 * i + offset]) =
            (double)tensor.data()[i];
      }
    }
    ...
    // Validate storage order - past this point, the tensor shape and storage
    // order should be correct
    try {
      info.validateStorageOrder();
    } catch (...) {
      vecData->resize(
          offset);  // Resize vector back to its size prior to adding tensor
      throw;
    }
  ...
  xt::xarray<_Ty> getTensor(const std::string &name, bool dequantize = false) {
    const auto it =
        std::find_if(tensors.begin(), tensors.end(),
                     [&name](const TensorInfo &ti) { return ti.name == name; });
    ...
    switch (it->dataType) {
      ...
      case TensorInfo::DataType::I8:
        for (uint32_t i = 0; i < tensor.size(); i++) {
          tensor.data()[i] = reinterpret_cast<int8_t *>(
              data->getData().data())[it->offset + i];
        }
        break;
      case TensorInfo::DataType::U16F:
        for (uint32_t i = 0; i < tensor.size(); i++) {
          tensor.data()[i] = reinterpret_cast<uint16_t *>(
              data->getData().data())[it->offset / sizeof(uint16_t) + i];
        }
        break;
      case TensorInfo::DataType::INT:
        for (uint32_t i = 0; i < tensor.size(); i++) {
          tensor.data()[i] = reinterpret_cast<int32_t *>(
              data->getData().data())[it->offset / sizeof(int32_t) + i];
        }
        break;
      case TensorInfo::DataType::FP16:
        for (uint32_t i = 0; i < tensor.size(); i++) {
          tensor.data()[i] = fp16_to_fp32(reinterpret_cast<uint16_t *>(
              data->getData().data())[it->offset / sizeof(uint16_t) + i]);
        }
        break;
      case TensorInfo::DataType::FP32:
        for (uint32_t i = 0; i < tensor.size(); i++) {
          tensor.data()[i] = reinterpret_cast<float_t *>(
              data->getData().data())[it->offset / sizeof(float_t) + i];
        }
        break;
      case TensorInfo::DataType::FP64:
        for (uint32_t i = 0; i < tensor.size(); i++) {
          tensor.data()[i] = reinterpret_cast<double_t *>(
              data->getData().data())[it->offset / sizeof(double_t) + i];
        }
        break;
    }
    if (dequantize) {
  ...
  template <typename _Ty>
  void changeStorageOrder(xt::xarray<_Ty> &array, TensorInfo::StorageOrder from,
                          TensorInfo::StorageOrder to) {
    ...
    if (fromOrder.size() == toOrder.size()) {
      ...
      for (size_t i = 0; i < toOrder.size(); ++i) {
        ...
        if (it == fromOrder.end()) {
          throw std::runtime_error(
              "Cannot change storage order. Dimension not found [Permute]");
        }
    ...
    if (fromOrder.size() < toOrder.size()) {
      ...
      std::vector<size_t> permute;
      for (size_t i = 0; i < toOrder.size(); ++i) {
        auto it = std::find(fromOrderExpanded.begin(), fromOrderExpanded.end(),
                            toOrder[i]);
        if (it == fromOrderExpanded.end()) {
          throw std::runtime_error(
              "Cannot change storage order. Dimension not "
              "found [Expand and Permute]");
        }
    ...
    if (fromOrder.size() > toOrder.size()) {
      ...
      for (size_t i = 0; i < fromOrder.size(); ++i) {
        ...
        if (it == toOrder.end()) {
          if (array.shape()[i] != 1) {
            throw std::runtime_error(
                "Cannot change storage order. Squeeze dimension is not 1");
          } else {
            squeeze.push_back(i);
          }
        } else {
      ...
      std::vector<size_t> permute;
      for (size_t i = 0; i < toOrder.size(); ++i) {
        auto it = std::find(fromOrderSqueezed.begin(), fromOrderSqueezed.end(),
                            toOrder[i]);
        if (it == fromOrderSqueezed.end()) {
          throw std::runtime_error(
              "Cannot change storage order. Dimension not "
              "found [Squeeze and Permute]");
        }
   ...
   */
  template <typename _Ty>
  xt::xarray<_Ty> getFirstTensor(TensorInfo::StorageOrder order,
                                 bool dequantize = false) {
    if (!tensors.empty()) {
  ...
  DEPTHAI_SERIALIZE(NNData, Buffer::sequenceNum, Buffer::ts, Buffer::tsDevice,
                    tensors, batchSize, transformation);
};
