...
struct Tracklet {
  enum class TrackingStatus : std::int32_t {
    ...
    LOST,    /**< The object gets lost now. The object can be tracked again
               automatically(long term tracking) or by specifying detected object
  ...
  DEPTHAI_SERIALIZE(Tracklet, roi, id, label, age, status, srcImgDetection,
                    spatialCoordinates, velocity, speed);
};
...
class Tracklets : public Buffer, public TransformableCRTP<Tracklets> {
 protected:
  /**
   * Internal transform hook used by transformTo() to apply Tracklets-specific
 ...
 public:
  ...
  /**
   * Returns a new Tracklets message with the tracklets transformed into the
   * target image transformation.
   *
   * For each tracklet, the bounding box is assumed to lie on a plane parallel
   * to the image plane at depth `tracklet.spatialCoordinates.z` (that is, all
  ...
  DEPTHAI_SERIALIZE(Tracklets, tracklets, transformation, ts, tsDevice,
                    sequenceNum, unit);
};
...
inline std::ostream &operator<<(std::ostream &out,
                                const dai::Tracklet::TrackingStatus &status) {
  switch (status) {
