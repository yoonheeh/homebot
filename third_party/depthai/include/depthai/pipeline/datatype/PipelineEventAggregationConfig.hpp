...
class NodeEventAggregationConfig {
 public:
  ...
  DEPTHAI_SERIALIZE(NodeEventAggregationConfig, nodeId, inputs, outputs, others,
                    events);
};
...
class PipelineEventAggregationConfig : public Buffer {
 public:
  ...
  std::optional<uint32_t> repeatIntervalSeconds =
      std::nullopt;  // Keep sending the aggregated state without waiting for
                     // new config
  ...
  DEPTHAI_SERIALIZE(PipelineEventAggregationConfig, Buffer::ts,
                    Buffer::tsDevice, Buffer::sequenceNum, nodes,
