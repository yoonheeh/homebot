...
class PointCloudData : public Buffer,
                       ...
                       public TransformableCRTP<PointCloudData> {
 protected:
  /**
   * Internal transform hook used by transformTo() to apply
   * PointCloudData-specific transformation logic.
   *
   * Point cloud remapping is currently not implemented here. Use ImageAlign on
   * the source images before generating the point cloud instead.
   */
  void transformToInternal(const ImgTransformation &target) override;

  unsigned int width = 0;  // width in pixels (for organized) or number of
                           // points (for unorganized)
  unsigned int height =
      0;  // height in pixels (for organized) or 1 (for unorganized)
 ...
 public:
   ...
   * represents the hight of the frame which was used to generate the point
   * cloud
   */
   ...
   */
  [[deprecated(
      "isSparse is deprecated, use isOrganized() instead (isSparse == "
      "!isOrganized())")]] bool
  isSparse() const;

  /**
   * Retrieves whether point cloud is organized (height > 1)
   * Organized point clouds have width x height structure from the original
   ...
   * @deprecated This setter is deprecated and will be removed in a future
   * release
   */
  [[deprecated(
      "setSparse is deprecated, width and height are set "
      "automatically based on organization")]] PointCloudData &
  setSparse(bool val);
   ...
   *
   * Due to efficiency reasons, point cloud remapping is currently not
   * implemented. Point clouds should be generated from already aligned source
   * messages instead of being transformed after the fact. Use ImageAlign on the
   * source inputs before creating the point cloud.
   *
   ...
   */
  std::vector<std::uint8_t> serializeProto(
  ...
  void getPclData() const {
    static_assert(dependent_false<T...>::value,
                  "Library not configured with PCL support");
  }
  template <typename... T>
  void setPclData(T...) {
    static_assert(dependent_false<T...>::value,
  ...
  DEPTHAI_SERIALIZE(PointCloudData, width, height, minx, miny, minz, maxx, maxy,
                    maxz, instanceNum, color, transformation, ts, tsDevice,
                    sequenceNum);
};
