...
class ImgFrame : public Buffer, public ProtoSerializable {
 public:
  ...
  getTimestamp(CameraExposureOffset offset) const;

  /**
   * Retrieves image timestamp (at the specified offset of exposure) directly
   * captured from device's monotonic clock, not synchronized to host time. Used
   * when monotonicity is required.
   */
  std::chrono::time_point<std::chrono::steady_clock,
                          std::chrono::steady_clock::duration>
  getTimestampDevice(CameraExposureOffset offset) const;
  ...
  /**
   * Retrieves sensor temperature in degrees Celsius. Returns an empty optional
   * if not available.
   */
   ...
   * Convenience function to initialize meta data from another frame
   * Copies over timestamps, transformations done on the image, etc.
   * @param sourceFrame shared pointer to source frame from which the metadata
   * is taken from
   */
   ...
   * @returns remapped point
   */
  static Point2f remapPointBetweenFrames(const Point2f &originPoint,
                                         const ImgFrame &originFrame,
                                         const ImgFrame &destFrame);
   ...
   * @returns remapped rectangle
   */
  static Rect remapRectBetweenFrames(const Rect &originRect,
                                     const ImgFrame &originFrame,
                                     const ImgFrame &destFrame);
...
// Optional - OpenCV support
   ...
   * ImgFrame is converted to color BGR interleaved or grayscale depending on
   * type.
   *
   ...
   *
   * @param frame Input cv::Mat BGR frame or single channel frame from which to
   * copy the data from.
   * @param type  Specifies the target image format for the internal buffer,
   * including color space, layout, and bit depth.
   */
  ...
  ImgFrame &setFrame(T...) {
    static_assert(dependent_false<T...>::value,
                  "Library not configured with OpenCV support");
    return *this;
  }
  template <typename... T>
  void getFrame(T...) {
 ...
 public:
  ...
  struct Specs {
    ...
    DEPTHAI_SERIALIZE(Specs, type, width, height, stride, bytesPP, p1Offset,
                      p2Offset, p3Offset);
  };
  struct CameraSettings {
    ...
    DEPTHAI_SERIALIZE(CameraSettings, exposureTimeUs, sensitivityIso,
                      lensPosition, wbColorTemp, lensPositionRaw, fsync,
                      sensorMode, fps, sensorTemperatureC);
  };
 ...
 public:
  DEPTHAI_SERIALIZE(ImgFrame, Buffer::ts, Buffer::tsDevice, Buffer::sequenceNum,
                    fb, sourceFb, cam, category, instanceNum, transformation);
};
