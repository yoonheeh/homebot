 ...
 * calibration algorithm. Defines parameters for periodic or one-time
 * recalibration of the camera sensors.
 */
struct AutoCalibrationConfig : public Buffer {
 public:
   ...
   * @param calibrationConfidence Threshold for accepting a new calibration
   * result.
   ...
   * @param maxImg Maximum number of images to collect for a single
   * recalibration.
   * @param valSize Number of images used to validate the calibration result.
   * @param flash Whether to save the resulting calibration to non-volatile
   * memory.
   */
  AutoCalibrationConfig(Mode mode, int sleepingTime,
                        double calibrationConfidence, double dataConfidence,
                        unsigned int maxIter, unsigned int maxImg, int valSize,
                        bool flash)
      : mode(mode),
  ...
  /**
   * @brief Minimum confidence score (0.0 to 1.0) required to apply a new
   * calibration.
   */
  double calibrationConfidenceThreshold = 0.9;

  /**
   * @brief Minimum quality threshold (0.0 to 1.0) for input features to be
   * used. A value below 0 selects the default based on the lens model.
   */
  ...
  /**
   * @brief If true, saves successful calibration to non-volatile memory
   * (EEPROM); otherwise, keeps in RAM only.
   */
