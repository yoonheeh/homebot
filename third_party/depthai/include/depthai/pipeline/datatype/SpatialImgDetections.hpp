...
struct SpatialImgDetection {
 private:
  void transformKeypointsFallback(const ImgTransformation &source,
                                  const ImgTransformation &target,
                                  const Point3f &spatialCoordinates);
 ...
 public:
  ...
  SpatialImgDetection(const RotatedRect &boundingBox,
                      Point3f spatialCoordinates, float confidence = 1.f,
                      uint32_t label = 0);
  SpatialImgDetection(const RotatedRect &boundingBox,
                      Point3f spatialCoordinates, std::string labelName,
                      float confidence, uint32_t label);
  SpatialImgDetection(const RotatedRect &boundingBox,
                      Point3f spatialCoordinates,
                      const SpatialKeypointsList &keypoints,
                      float confidence = 1.f, uint32_t label = 0);
  SpatialImgDetection(const RotatedRect &boundingBox,
                      Point3f spatialCoordinates,
                      const SpatialKeypointsList &keypoints,
  ...
  /**
   * Returns a list of spatial coordinates for each keypoint, or empty list if
   ...
   * @param lengthUnit Length unit to use for the transformation. By default,
  ...
  DEPTHAI_SERIALIZE(SpatialImgDetection, label, labelName, confidence, xmin,
                    ymin, xmax, ymax, boundingBox, keypoints,
 ...
 * location data
 */
class SpatialImgDetections : public ImgDetectionsT<SpatialImgDetection>,
                             public ProtoSerializable,
                             public TransformableCRTP<SpatialImgDetections> {
 protected:
  /**
   * Internal transform hook used by transformTo() to apply
   * SpatialImgDetections-specific transformation logic.
   */
 ...
 public:
   ...
   *
   * For each detection, the bounding box is assumed to lie on a plane parallel
   * to the image plane at depth `detection.spatialCoordinates.z` (that is, all
   * four bounding-box corners are projected using the same depth value). The
   * transformed corners are then fit with the smallest enclosing rotated
   * rectangle to preserve rectangularity.
   *
   * @param target Target image transformation.
