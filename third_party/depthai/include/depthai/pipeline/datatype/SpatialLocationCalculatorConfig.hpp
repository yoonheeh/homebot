...
DEPTHAI_SERIALIZE_EXT(SpatialLocationCalculatorConfigThresholds, lowerThreshold,
                      upperThreshold);
 ...
 */
enum class SpatialLocationCalculatorAlgorithm : uint32_t {
  AVERAGE = 0,
  MEAN = AVERAGE,
  MIN,
  MAX,
...
struct SpatialLocationCalculatorConfigData {
   ...
   * every second etc. Default value AUTO: for AVERAGE, MIN, MAX step size is 1;
   * for MODE/MEDIAN it's 2.
   */
  std::int32_t stepSize = AUTO;
};
DEPTHAI_SERIALIZE_EXT(SpatialLocationCalculatorConfigData, roi, depthThresholds,
                      calculationAlgorithm, stepSize);
...
class SpatialLocationCalculatorConfig : public Buffer {
 ...
 public:
   ...
   * @param ROIs Vector of configuration parameters for ROIs (region of
   interests)
   */
   ...
   * calculations.
   * @param lowerThreshold Lower threshold in depth units (millimeter by
   * default).
   * @param upperThreshold Upper threshold in depth units (millimeter by
   * default).
   */
  void setDepthThresholds(uint32_t lowerThreshold = MIN_LOWER_THRESHOLD,
                          uint32_t upperThreshold = MAX_UPPER_THRESHOLD);
   ...
   */
  void setCalculationAlgorithm(
      SpatialLocationCalculatorAlgorithm calculationAlgorithm);

  /**
   * Set step size for spatial location calculation.
   * Step size 1 means that every pixel is taken into calculation, size 2 means
   * every second etc. for AVERAGE, MIN, MAX step size is 1; for MODE/MEDIAN
   * it's 2.
   */
  ...
  /**
   * Specify whether to consider only segmented pixels within a detection
  ...
  /**
   * Specify whether to passthrough segmentation mask along with spatial
   * detections.
  ...
  /*
   * Retrieve the lower and upper depth value thresholds used in the spatial
   * calculations.
   * @returns Pair of lower and upper thresholds in depth units (millimeter by
   * default).
   */
  ...
  /**
   * Retrieve whether segmentation is passed through along with spatial
   * detections.
   * @warning Only applicable to ImgDetections with segmentation masks.
   */
  ...
  DEPTHAI_SERIALIZE(SpatialLocationCalculatorConfig, globalStepSize,
                    globalLowerThreshold, globalUpperThreshold,
                    globalCalculationAlgorithm, globalKeypointRadius,
                    calculateSpatialKeypoints, useSegmentation,
                    segmentationPassthrough, bBoxScaleFactor, config);
};
