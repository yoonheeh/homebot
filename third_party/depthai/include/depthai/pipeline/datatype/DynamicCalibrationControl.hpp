 ...
 *
 * This class encapsulates all commands that can be sent to the
 * DynamicCalibration node. A command is represented using the @ref Command
 * variant, which may contain one of the command structures defined in the @ref
 * Commands namespace.
 *
 * Factory methods (e.g., @ref calibrate, @ref startCalibration) allow
 * convenient creation of command objects.
 *
 * The message is serialized and transmitted to the device through an input
 * queue.
 */
class DynamicCalibrationControl : public Buffer {
 public:
  ...
  /**
   * @brief Namespace containing all command structures that may be used inside
   * @ref Command.
   *
   * These structs represent individual commands that the Dynamic Calibration
   * node can execute. Each command corresponds to a specific action or
  ...
  struct Commands {
    ...
    /**
     * @brief Command to compute calibration metrics (e.g.,
     * calibrationConfidence and dataConfidence).
     * * Evaluates the provided calibration data and computes relevant quality
     * metrics.
     */
    struct ComputeCalibrationMetrics {
      explicit ComputeCalibrationMetrics(dai::CalibrationHandler &calibration)
    ...
    struct StartCalibration {
      explicit StartCalibration(float loadImagePeriod = 0.5f,
                                float calibrationPeriod = 5.0f)
          : loadImagePeriod(loadImagePeriod),
    ...
    struct ApplyCalibration {
      ...
      explicit ApplyCalibration(const CalibrationHandler &calibration,
                                bool flash = false)
          : calibration(calibration), flash(flash) {}
    ...
    struct SetPerformanceMode {
      ...
      explicit SetPerformanceMode(PerformanceMode performanceMode)
   ...
   */
  using Command =
      std::variant<std::monostate,                ///< No command
                   Commands::Calibrate,           ///< Run calibration
                   Commands::CalibrationQuality,  ///< Run calibration quality
                                                  ///< check
                   ...
                   Commands::ComputeCalibrationMetrics  ///< Compute
                                                        ///< reprojection errot
                                                        ///< and confidence
                   >;
   ...
   */
  [[nodiscard]] static std::shared_ptr<DynamicCalibrationControl> calibrate(
      bool force = false) {
    return std::make_shared<DynamicCalibrationControl>(
        Commands::Calibrate{force});
  }
   ...
   * @return Shared pointer to a DynamicCalibrationControl command.
   */
  [[nodiscard]] static std::shared_ptr<DynamicCalibrationControl>
  calibrationQuality(bool force = false) {
    return std::make_shared<DynamicCalibrationControl>(
        Commands::CalibrationQuality{force});
  }
  ...
  computeCalibrationMetrics(dai::CalibrationHandler &calibration) {
    return std::make_shared<DynamicCalibrationControl>(
        Commands::ComputeCalibrationMetrics{calibration});
  }
   ...
   * @param calibrationPeriod How often a new calibration should be executed, in
   * seconds.
   * @return Shared pointer to a DynamicCalibrationControl command.
   */
  [[nodiscard]] static std::shared_ptr<DynamicCalibrationControl>
  startCalibration(float loadImagePeriod = 0.5f,
                   float calibrationPeriod = 5.0f) {
    return std::make_shared<DynamicCalibrationControl>(
        Commands::StartCalibration{loadImagePeriod, calibrationPeriod});
  }
   ...
   */
  [[nodiscard]] static std::shared_ptr<DynamicCalibrationControl>
  applyCalibration(const CalibrationHandler &calibration, bool flash = false) {
    return std::make_shared<DynamicCalibrationControl>(
        Commands::ApplyCalibration{calibration, flash});
  }
   ...
   */
  [[nodiscard]] static std::shared_ptr<DynamicCalibrationControl>
  setPerformanceMode(PerformanceMode mode = PerformanceMode::DEFAULT) {
    return std::make_shared<DynamicCalibrationControl>(
        Commands::SetPerformanceMode{mode});
  }
