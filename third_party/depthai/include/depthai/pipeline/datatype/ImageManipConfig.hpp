...
struct Translate : OpBase {
  ...
  Translate() = default;
  Translate(float offsetX, float offsetY, bool normalized = false)
      : offsetX(offsetX), offsetY(offsetY), normalized(normalized) {}
...
struct Rotate : OpBase {
  ...
  bool center;  // if true, rotation is around center of image, otherwise around
                // top-left corner
  float offsetX;
  float offsetY;
  bool normalized;
  ...
  Rotate() = default;
  explicit Rotate(float angle, bool center = true, float offsetX = 0,
                  float offsetY = 0, bool normalized = false)
      : angle(angle),
        center(center),
        offsetX(offsetX),
        offsetY(offsetY),
  ...
  std::string toStr() const override {
    std::stringstream ss;
    ss.imbue(std::locale(""));
    ss << "Rot:a=" << angle << ",c=" << center << ",x=" << offsetX << ",y"
       << offsetY << ",n=" << normalized;
    return ss.str();
  }
...
struct Resize : OpBase {
  ...
  Resize() = default;
  Resize(float width, float height, bool normalized = false)
      : width(width), height(height), normalized(normalized), mode(VALUE) {}
  ...
  std::string toStr() const override {
    std::stringstream ss;
    ss.imbue(std::locale(""));
    ss << "Res:w=" << width << ",h=" << height << ",n=" << normalized
       << ",m=" << mode;
    return ss.str();
  }
...
struct Flip : OpBase {
  ...
  bool center = false;  // if true, flip is around center of image, otherwise
                        // around top-left corner

  ~Flip() override;

  Flip() = default;
  explicit Flip(Direction direction, bool center = true)
...
struct Affine : OpBase {
  ...
  std::string toStr() const override {
    std::stringstream ss;
    ss.imbue(std::locale(""));
    ss << "A:m=" << matrix[0] << "," << matrix[1] << "," << matrix[2] << ","
       << matrix[3];
    return ss.str();
  }
...
struct Perspective : OpBase {
  ...
  std::string toStr() const override {
    std::stringstream ss;
    ss.imbue(std::locale(""));
    ss << "P:m=" << matrix[0] << "," << matrix[1] << "," << matrix[2] << ","
...
struct FourPoints : OpBase {
  std::array<dai::Point2f, 4> src{
      dai::Point2f(0.0, 0.0), dai::Point2f(1.0, 0.0), dai::Point2f(1.0, 1.0),
      dai::Point2f(0.0, 1.0)};
  ...
  FourPoints() = default;
  FourPoints(std::array<dai::Point2f, 4> src, std::array<dai::Point2f, 4> dst,
             bool normalized = false)
  ...
  std::string toStr() const override {
    std::stringstream ss;
    ss.imbue(std::locale(""));
    ss << "4P:s1=" << src[0].x << "," << src[0].y << ",s2=" << src[1].x << ","
       << src[1].y << ",s3=" << src[2].x << "," << src[2].y
       << ",s4=" << src[3].x << "," << src[3].y << "d1=" << dst[0].x << ","
       << dst[0].y << ",d2=" << dst[1].x << "," << dst[1].y
       << ",d3=" << dst[2].x << "," << dst[2].y << ",d4=" << dst[3].x << ","
       << dst[3].y << ",d=" << normalized;
    return ss.str();
  }
...
struct Crop : OpBase {
  ...
  Crop(float width, float height, bool normalized = false, bool center = false)
      : width(width), height(height), normalized(normalized), center(center) {}

  Crop clone() const { return *this; }

  std::string toStr() const override {
    std::stringstream ss;
    ss.imbue(std::locale(""));
    ss << "C:w=" << width << ",h=" << height << ",n=" << normalized
       << ",c=" << center;
    return ss.str();
  }
...
struct ManipOp {
  std::variant<Translate, Rotate, Resize, Flip, Affine, Perspective, FourPoints,
               Crop>
      op;
...
class ImageManipOpsEnums {
 public:
  enum class Background : uint8_t {
    COLOR /* , REPLICATE, MIRROR */
  };      // TODO(asahtik): replicate impl
...
class ImageManipOpsBase : public ImageManipOpsEnums {
 public:
  ...
  void cloneTo(ImageManipOpsBase<C2> &to) const {
    ...
    to.operations.clear();
    to.operations.insert(to.operations.end(), operations.begin(),
                         operations.end());
  }

  bool hasWarp(const size_t inputWidth, const size_t inputHeight) const {
    return operations.size() > 0 ||
           (outputWidth != 0 && outputWidth != inputWidth) ||
           (outputHeight != 0 && outputHeight != inputHeight) || undistort;
  }
  ...
  ImageManipOpsBase &transformFourPoints(std::array<dai::Point2f, 4> src,
                                         std::array<dai::Point2f, 4> dst,
  ...
  ImageManipOpsBase &resize(float width, float height,
                            bool normalized = false) {
    operations.emplace_back(Resize(width, height, normalized));
    return *this;
  }

  ImageManipOpsBase &crop(float x, float y, float w, float h,
                          bool normalized = false, bool center = false) {
  ...
  ImageManipOpsBase &resizeWidthKeepAspectRatio(float width,
                                                bool normalized = false) {
    operations.emplace_back(Resize(width, 0, normalized));
    return *this;
  }

  ImageManipOpsBase &resizeHeightKeepAspectRatio(float height,
                                                 bool normalized = false) {
  ...
  ImageManipOpsBase &rotateRadians(float angle, bool center = true,
    ...
    operations.emplace_back(
        Rotate(angle, center, offsetX, offsetY, normalized));
    return *this;
  }
  ...
  ImageManipOpsBase &rotateDegrees(float angle, bool center = true,
    ...
    return rotateRadians(angle * 3.14159265358979323846f / 180.0f, center,
                         offsetX, offsetY, normalized);
  }

  ImageManipOpsBase &translate(float offsetX, float offsetY,
                               bool normalizedCoords = false) {
  ...
  ImageManipOpsBase &setOutputResize(uint32_t width, uint32_t height,
                                     ResizeMode mode) {
    outputWidth = width;
    outputHeight = height;
    resizeMode = mode;
  ...
  ImageManipOpsBase &setBackgroundColor(uint32_t red, uint32_t green,
                                        uint32_t blue) {
  ...
  bool getUndistort() const { return undistort; }

  const C &getOperations() const { return this->operations; }
  ...
  DEPTHAI_SERIALIZE(ImageManipOpsBase, operations, outputWidth, outputHeight,
                    center, resizeMode, background, backgroundR, backgroundG,
                    backgroundB, colormap, undistort);
};
...
class ImageManipConfig : public Buffer {
 ...
 public:
   ...
   * @param normalizedCoords If true, the coordinates are normalized to range
   * [0, 1] where 1 maps to the width/height of the image
   */
   ...
   * [0, 1] where 1 maps to the width/height of the image
   */
  ImageManipConfig &addCropRotatedRect(dai::RotatedRect rotatedRect,
                                       bool normalizedCoords = false);
  /**
   * Rescales the image using the specified factors
   * @param scaleX Scale factor for the X axis
   * @param scaleY Scale factor for the Y axis. If not specified, scaleY is set
   * to the same value as scaleX
   */
  ...
  /**
   * Rotates the image around the specified point by the specified angle in
   * degrees
   * @param angle Angle in radians
   ...
   * @param matrix an array containing a 2x2 matrix representing the affine
   * transformation
   */
  ImageManipConfig &addTransformAffine(std::array<float, 4> matrix);
  /**
   * Applies a perspective transformation to the image
   * @param matrix an array containing a 3x3 matrix representing the perspective
   * transformation
   */
   ...
   * [0, 1] where 1 maps to the width/height of the image
   */
  ImageManipConfig &addTransformFourPoints(std::array<dai::Point2f, 4> src,
                                           std::array<dai::Point2f, 4> dst,
                                           bool normalizedCoords = false);
  /**
   ...
   * @param mode Resize mode. NONE - no resize, STRETCH - stretch to fit,
   * LETTERBOX - keep aspect ratio and pad with background color, CENTER_CROP -
   * keep aspect ratio and crop
   */
  ImageManipConfig &setOutputSize(uint32_t w, uint32_t h,
   ...
   */
  ImageManipConfig &setBackgroundColor(uint32_t red, uint32_t green,
                                       uint32_t blue);
  /**
  ...
  /**
   * Instruct ImageManip to not remove current image from its queue and use the
   * same for next message.
  ...
  DEPTHAI_SERIALIZE(ImageManipConfig, base, outputFrameType, reusePreviousImage,
                    skipCurrentImage);
