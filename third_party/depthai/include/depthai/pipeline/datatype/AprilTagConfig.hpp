...
class AprilTagConfig : public Buffer {
 public:
  ...
  /**
   * Max number of error bits that should be corrected. Accepting large numbers
   * of corrected errors leads to greatly increased false positive rates. As of
   * this implementation, the detector cannot detect tags with a hamming
  ...
  struct QuadThresholds {
    ...
    /**
     * Should the thresholded image be deglitched? Only useful for very noisy
     * images
     */
    bool deglitch = false;

    DEPTHAI_SERIALIZE(QuadThresholds, minClusterPixels, maxNmaxima,
                      criticalDegree, maxLineFitMse, minWhiteBlackDiff,
                      deglitch);
  };
  ...
  DEPTHAI_SERIALIZE(AprilTagConfig, Buffer::sequenceNum, Buffer::ts,
                    Buffer::tsDevice, family, quadDecimate, quadSigma,
                    refineEdges, decodeSharpening, maxHammingDistance,
                    quadThresholds);
};
