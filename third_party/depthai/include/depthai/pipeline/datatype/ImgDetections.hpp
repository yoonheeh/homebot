...
struct ImgDetection {
  ...
  ImgDetection(const RotatedRect &boundingBox, float confidence = 0.f,
               uint32_t label = 0);
  ImgDetection(const RotatedRect &boundingBox, std::string labelName,
               float confidence = 0.f, uint32_t label = 0);
  ImgDetection(const RotatedRect &boundingBox, const KeypointsList &keypoints,
               float confidence = 0.f, uint32_t label = 0);
  ImgDetection(const RotatedRect &boundingBox, const KeypointsList &keypoints,
               std::string labelName, float confidence = 0.f,
               uint32_t label = 0);
  ...
  /**
   * Returns bounding box if it was set, else it constructs a new one from the
   * legacy xmin, ymin, xmax, ymax values.
   */
  RotatedRect getBoundingBox() const;

  /**
   * Sets the bounding box and the legacy coordinates of the detection from the
   * top-left and bottom-right points.
   */
  void setOuterBoundingBox(const float xmin, const float ymin, const float xmax,
                           const float ymax);
  ...
  /**
   * Returns a list of edges, each edge is a pair of indices, or empty list if
   * no keypoints were set.
   */
   ...
   */
  void transform(const ImgTransformation &source,
                 const ImgTransformation &target);

  DEPTHAI_SERIALIZE(ImgDetection, label, labelName, confidence, xmin, ymin,
                    xmax, ymax, boundingBox, keypoints);
};

/**
 * ImgDetections message. Carries normalized detections and optional
 * segmentation mask. The segmentation mask is stored as a single-channel INT8
 * 2-d array, where the value represents the instance index in the list of
 * detections. The value 255 is treated as a background pixel (no instance).
 */
class ImgDetections : public ImgDetectionsT<ImgDetection>,
                      public ProtoSerializable,
                      public TransformableCRTP<ImgDetections> {
 protected:
  /**
   * Internal transform hook used by transformTo() to apply
   * ImgDetections-specific transformation logic.
   */
 ...
 public:
  ...
  /**
   * Returns a new ImgDetections message with the detections transformed into
   * the target image transformation.
   *
   * If the target transformation has a different source coordinate system (eg.
   * different camera socket) than the one the detections were originally
   * generated in, the remapping will be inaccurate due to the lack of depth
   * information.
   *
   * The segmentation mask is not transformed. Use ImageAlign node to transform
   * the segmentation mask to the target transformation if needed.
   *
   * @param target Target image transformation.
   */
  ...
  DEPTHAI_SERIALIZE(ImgDetections, sequenceNum, ts, tsDevice, detections,
                    transformation, segmentationMaskWidth,
