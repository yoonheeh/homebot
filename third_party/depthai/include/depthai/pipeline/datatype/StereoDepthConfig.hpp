...
class StereoDepthConfig : public Buffer {
 public:
  ...
  struct AlgorithmControl {
    /**
     * Align the disparity/depth to the perspective of a rectified output, or
     * center it
     */
    ...
    /**
     * Set the disparity/depth alignment to the perspective of a rectified
     * output, or center it
     */
    ...
    /**
     * Custom depth unit multiplier, if custom depth unit is enabled, relative
     * to 1 meter. A multiplier of 1000 effectively means depth unit in
     * millimeter.
     */
    float customDepthUnitMultiplier = 1000.f;

    /**
     * Computes and combines disparities in both L-R and R-L directions, and
     * combine them. For better occlusion handling
     */
    ...
    /**
     * Disparity range increased from 95 to 190, combined from full resolution
     * and downscaled images. Suitable for short range objects
     */
    ...
    /**
     * Left-right check threshold for left-right, right-left disparity map
     * combine, 0..128 Used only when left-right check mode is enabled. Defines
     * the maximum difference between the confidence of pixels from left-right
     * and right-left confidence maps
     */
     ...
     * For example shifting by 48 will change effective disparity search range
     * from (0,95] to [48,143]. An alternative approach to reducing the minZ. We
     * normally only recommend doing this when it is known that there will be no
     * objects farther away than MaxZ, such as having a depth camera mounted
     * above a table pointing down at the table surface.
     */
    std::int32_t disparityShift = 0;

    /*
     * Used only for debug purposes. centerAlignmentShiftFactor is set
     * automatically in firmware, from camera extrinsics when depth alignment to
     * camera is enabled. Center alignment is achieved by shifting the obtained
     * disparity map by a scale factor. It's used to align to a different camera
     * that is on the same horizontal baseline as the two stereo cameras. E.g.
     * if we have a device with 10 cm stereo baseline, and we have another
     * camera in between, 9cm from the LEFT camera and 1 cm from the RIGHT
     * camera we can align the obtained disparity map using a scale factor of
     * 0.9. Note that aligning disparity map to a different camera involves 2
     * steps:
     ...
     * For right and center alignment X pixels will be invalidated from the
     * right edge, for left alignment from the left edge.
     */
    std::int32_t numInvalidateEdgePixels = 0;

    DEPTHAI_SERIALIZE(AlgorithmControl, depthAlign, depthUnit,
                      customDepthUnitMultiplier, enableLeftRightCheck,
                      enableSwLeftRightCheck, enableExtended, enableSubpixel,
                      leftRightCheckThreshold, subpixelFractionalBits,
                      disparityShift, centerAlignmentShiftFactor,
  ...
  struct ConfidenceMetrics {
    ...
    /**
     * Threshold offset for MV variance in confidence generation. A value of 0
     * allows most variance. Valid range is [0,3].
     */
    ...
    /**
     * Flag to indicate whether final confidence value will be overidden by
     * flatness value. Valid range is {true,false}.
     */
    bool flatnessOverride = false;

    DEPTHAI_SERIALIZE(ConfidenceMetrics, occlusionConfidenceWeight,
                      motionVectorConfidenceWeight,
                      motionVectorConfidenceThreshold, flatnessConfidenceWeight,
                      flatnessConfidenceThreshold, flatnessOverride);
  };
  ...
  struct PostProcessing {
    enum class Filter : int32_t {
      ...
      SPATIAL,
      TEMPORAL,
      FILTER_COUNT = TEMPORAL
    };

    /**
     * Order of filters to be applied if filtering is enabled.
     */
    std::array<Filter, 5> filteringOrder = {Filter::MEDIAN, Filter::DECIMATION,
                                            Filter::SPECKLE, Filter::SPATIAL,
                                            Filter::TEMPORAL};
     ...
     * pixel neighborhood will be mixed together.
     */
    ...
    /**
     * Edge-preserving filtering: This type of filter will smooth the depth
     * noise while attempting to preserve edges.
     */
     ...
     * If input frame pixel is too dark or too bright, disparity will be
     * invalidated. The idea is that for too dark/too bright pixels we have low
     * confidence, since that area was under/over exposed and details were lost.
     */
    struct BrightnessFilter {
      /**
       * Minimum pixel brightness.
       * If input pixel is less or equal than this value the depth value is
       * invalidated.
       */
    ...
    /**
     * Decimation filter.
     * Reduces the depth scene complexity. The filter runs on kernel sizes [2x2]
     * to [8x8] pixels.
     */
    ...
    /**
     * Decimation filter.
     * Reduces disparity/depth map x/y complexity, reducing runtime complexity
     * for other filters.
     */
    ...
    struct HoleFilling {
      ...
      /**
       * Pixels with confidence higher than this value are used to calculate an
       * average disparity per superpixel. Valid range is [1,255]
       */
       ...
       *    above nHighConfThresh that are used to calculate average disparity
       * per superpixel, where 1 means 50% or half, 2 means 25% or a quarter and
       * 3 means 12.5% or an eighth. If the required number of pixels are not
      ...
      DEPTHAI_SERIALIZE(HoleFilling, enable, highConfidenceThreshold,
                        fillConfidenceThreshold, minValidDisparity,
    ...
    struct AdaptiveMedianFilter {
      /**
       * Flag to enable adaptive median filtering for a final pass of filtering
       * on low confidence pixels.
       */
      bool enable = true;
    ...
    DEPTHAI_SERIALIZE(PostProcessing, filteringOrder, median,
                      bilateralSigmaValue, spatialFilter, temporalFilter,
                      thresholdFilter, brightnessFilter, speckleFilter,
                      decimationFilter, holeFilling, adaptiveMedianFilter);
  };

  /**
   * The basic cost function used by the Stereo Accelerator for matching the
   * left and right images is the Census Transform. It works on a block of
   * pixels and computes a bit vector which represents the structure of the
   * image in that block.
   * There are two types of Census Transform based on how the middle pixel is
   * used: Classic Approach and Modified Census. The comparisons that are made
   * between pixels can be or not thresholded. In some cases a mask can be
   * applied to filter out only specific bits from the entire bit stream. All
   * these approaches are: Classic Approach: Uses middle pixel to compare
   * against all its neighbors over a defined window. Each comparison results in
   * a new bit, that is 0 if central pixel is smaller, or 1 if is it bigger than
   * its neighbor. Modified Census Transform: same as classic Census Transform,
   * but instead of comparing central pixel with its neighbors, the window mean
   * will be compared with each pixel over the window. Thresholding Census
   * Transform: same as classic Census Transform, but it is not enough that a
   * neighbor pixel to be bigger than the central pixel, it must be significant
   * bigger (based on a threshold). Census Transform with Mask: same as classic
   * Census Transform, but in this case not all of the pixel from the support
   * window are part of the binary descriptor. We use a ma sk “M” to define
   * which pixels are part of the binary descriptor (1), and which pixels should
   * be skipped (0).
   */
  struct CensusTransform {
    ...
    /**
     * Census transform mask, default - auto, mask is set based on resolution
     * and kernel size. Disabled for 400p input resolution. Enabled for 720p.
    ...
    /**
     * If enabled, each pixel in the window is compared with the mean window
    ...
    DEPTHAI_SERIALIZE(CensusTransform, kernelSize, kernelMask, enableMeanMode,
                      threshold, noiseThresholdOffset, noiseThresholdScale);
  };

  /**
   * The matching cost is way of measuring the similarity of image locations in
   * stereo correspondence algorithm. Based on the configuration parameters and
   * based on the descriptor type, a linear equation is applied to computing the
   * cost for each candidate disparity at each pixel.
   */
  struct CostMatching {
    /**
     ...
     * This way the search range is extended to 176 disparities, by sparse
     * matching. Note: when enabling this flag only depth map will be affected,
     * disparity map is not.
     */
    bool enableCompanding = false;

    /**
     * Used only for debug purposes, SW postprocessing handled only invalid
     * value of 0 properly.
     */
     ...
     * CTC is the Census Transform Cost between 2 pixels, based on Hamming
     * distance (xor). The α and β parameters are subject to fine tuning by the
     * user.
     */
    ...
    DEPTHAI_SERIALIZE(CostMatching, disparityWidth, enableCompanding,
                      invalidDisparityValue, confidenceThreshold,
                      enableSwConfidenceThresholding, linearEquationParameters);
  };

  /**
   * Cost Aggregation is based on Semi Global Block Matching (SGBM). This
   * algorithm uses a semi global technique to aggregate the cost map.
   * Ultimately the idea is to build inertia into the stereo algorithm. If a
   * pixel has very little texture information, then odds are the correct
   * disparity for this pixel is close to that of the previous pixel considered.
   * This means that we get improved results in areas with low texture.
   */
  struct CostAggregation {
    ...
    struct P1Config {
       ...
       * A bigger value enforces higher smoothness and reduced noise at the cost
       * of lower edge accuracy. This value must be smaller than P2 default
       * penalty. Valid range is [10,50].
       */
       ...
       * A bigger value permits higher neighboring feature dissimilarity
       * tolerance. This value is shared with P2 penalty configuration. Valid
       * range is [8,16].
       */
      ...
      DEPTHAI_SERIALIZE(P1Config, enableAdaptive, defaultValue, edgeValue,
                        smoothValue, edgeThreshold, smoothThreshold);
    };
    ...
    struct P2Config {
       ...
       * Used as the default penalty value when nAdapEnable is disabled.
       * A bigger value enforces higher smoothness and reduced noise at the cost
       * of lower edge accuracy. This value must be larger than P1 default
       * penalty. Valid range is [20,100].
       */
      ...
      DEPTHAI_SERIALIZE(P2Config, enableAdaptive, defaultValue, edgeValue,
                        smoothValue);
    };

    P2Config p2Config;

    DEPTHAI_SERIALIZE(CostAggregation, divisionFactor, horizontalPenaltyCostP1,
                      horizontalPenaltyCostP2, verticalPenaltyCostP1,
                      verticalPenaltyCostP2, p1Config, p2Config);
  };

  /**
   * @param align Set the disparity/depth alignment: centered (between the
   * 'left' and 'right' inputs), or from the perspective of a rectified output
   * stream
   */
  ...
  /**
   * @param median Set kernel size for disparity/depth median filtering, or
   * disable
   */
   ...
   * neighborhood will be mixed together, resulting in larger areas of
   * semi-equal color.
  ...
  /**
   * @param threshold Set threshold for left-right, right-left disparity map
   * combine, 0..255
   */
  ...
  /**
   * Computes disparity with sub-pixel interpolation (3 fractional bits by
   * default).
   *
   ...
   * from (0,95] to [48,143]. An alternative approach to reducing the minZ. We
   * normally only recommend doing this when it is known that there will be no
   * objects farther away than MaxZ, such as having a depth camera mounted above
   * a table pointing down at the table surface.
   */
  StereoDepthConfig &setDisparityShift(int disparityShift);

  /**
   * Invalidate X amount of pixels at the edge of disparity frame.
   * For right and center alignment X pixels will be invalidated from the right
   * edge, for left alignment from the left edge.
   */
  StereoDepthConfig &setNumInvalidateEdgePixels(
      int32_t numInvalidateEdgePixels);
  ...
  DEPTHAI_SERIALIZE(StereoDepthConfig, algorithmControl, postProcessing,
                    censusTransform, costMatching, costAggregation,
