...
class EdgeDetectorConfig : public Buffer {
 public:
  ...
  struct EdgeDetectorConfigData {
    ...
    DEPTHAI_SERIALIZE(EdgeDetectorConfigData, sobelFilterHorizontalKernel,
                      sobelFilterVerticalKernel);
  };

  /**
   * Set sobel filter horizontal and vertical 3x3 kernels
   * @param horizontalKernel Used for horizontal gradient computation in 3x3
   * Sobel filter
   * @param verticalKernel Used for vertical gradient computation in 3x3 Sobel
   * filter
   */
  void setSobelFilterKernels(
      const std::vector<std::vector<int>> &horizontalKernel,
      const std::vector<std::vector<int>> &verticalKernel);

  /**
   * Retrieve configuration data for EdgeDetector
   * @returns EdgeDetectorConfigData: sobel filter horizontal and vertical 3x3
   * kernels
   */
  ...
  DEPTHAI_SERIALIZE(EdgeDetectorConfig, Buffer::sequenceNum, Buffer::ts,
                    Buffer::tsDevice, config);
};
