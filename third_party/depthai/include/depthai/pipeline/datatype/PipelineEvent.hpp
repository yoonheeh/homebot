...
class PipelineEvent : public Buffer {
 public:
  ...
  int64_t nodeId = -1;
  Status status =
      Status::SUCCESS;  // Used for input status reporting (if it is blocked)
  std::optional<uint32_t> queueSize;
  Interval interval = Interval::NONE;
  Type type = Type::CUSTOM;
  std::string source;  // e.g., input/output/custom event name - used in
                       // conjunction with type to identify the event
  ...
  DEPTHAI_SERIALIZE(PipelineEvent, Buffer::ts, Buffer::tsDevice,
                    Buffer::sequenceNum, nodeId, status, queueSize, interval,
                    type, source);
};
