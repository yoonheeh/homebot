...
class NodeState {
 public:
  ...
  struct DurationStats {
    ...
    DEPTHAI_SERIALIZE(DurationStats, minMicros, maxMicros, averageMicrosRecent,
                      stdDevMicrosRecent, minMicrosRecent, maxMicrosRecent,
                      medianMicrosRecent);
  };
  struct Timing {
    float fps = 0.0f;
  ...
  struct QueueStats {
    ...
    DEPTHAI_SERIALIZE(QueueStats, maxQueued, minQueuedRecent, maxQueuedRecent,
                      medianQueuedRecent);
  };
  struct InputQueueState {
    ...
    enum class State : std::int32_t {
      ...
      BLOCKED = 2   // An output attempted to send to this input, but the input
                    // queue was full
    } state = State::IDLE;
    ...
    bool isValid() const { return timing.isValid(); }

    DEPTHAI_SERIALIZE(InputQueueState, state, numQueued, timing, queueStats);
  };
  struct OutputQueueState {
    // Current state of the output queue. Send should ideally be instant. This
    // is not the case when the input queue is full. In that case, the state
    // will be SENDING until there is space in the input queue (unless trySend
    // is used).
    enum class State : std::int32_t {
      IDLE = 0,
      SENDING = 1
  ...
  DEPTHAI_SERIALIZE(NodeState, state, events, outputStates, inputStates,
                    inputsGetTiming, outputsSendTiming, mainLoopTiming,
                    otherTimings);
};
...
class PipelineState : public Buffer {
 public:
  ...
  DEPTHAI_SERIALIZE(PipelineState, Buffer::ts, Buffer::tsDevice,
                    Buffer::sequenceNum, nodeStates, configSequenceNum);
};
