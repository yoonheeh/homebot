...
DEPTHAI_SERIALIZE_EXT(AprilTag, id, hamming, decisionMargin, topLeft, topRight,
...
class AprilTags : public Buffer, public TransformableCRTP<AprilTags> {
 protected:
  /**
   * Internal transform hook used by transformTo() to apply AprilTags-specific
 ...
 public:
  ...
  /**
   * Returns a new AprilTags message with the tags transformed into the target
   * image transformation.
   *
   * If the target transformation has a different coordinate system source (eg.
   * different camera socket) then the remapping will be inaccurate due to the
