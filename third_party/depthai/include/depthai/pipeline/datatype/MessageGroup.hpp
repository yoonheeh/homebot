...
class MessageGroup : public Buffer {
 public:
  ...
  // TODO(Morato) - this API is dangerous, when T is a base reference to a
  // derived class template <typename T> void add(const std::string& name, const
  // T& value) {
  //     static_assert(std::is_base_of<ADatatype, T>::value, "T must derive from
  //     ADatatype"); group[name] = std::make_shared<T>(value);
  // }
  ...
  DEPTHAI_SERIALIZE(MessageGroup, group, Buffer::ts, Buffer::tsDevice,
                    Buffer::sequenceNum);
};
