...
class SegmentationParserConfig : public Buffer {
 public:
  ...
  /**
   * Add a confidence threshold to the argmax operation over the segmentation
   * tensor. Pixels with confidence values below this threshold will be assigned
   * the background class (255).
   * @param threshold Confidence threshold for segmentation parsing
   * @note Default is -1.0f, which means no thresholding is applied.
   * @note Only applicable if output classes are not in a single layer (eg.
   ...
   * A step size of 1 means every pixel is processed, a step size of 2 means
   * every second pixel is processed, and so on. This can be used to speed up
   * processing at the cost of lower resolution masks.
