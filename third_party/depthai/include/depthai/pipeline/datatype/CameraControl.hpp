 ...
 * mode, auto-white-balance in `AUTO` mode, and auto-exposure with anti-banding
...
class CameraControl : public Buffer {
 public:
  ...
  enum class AutoFocusMode : uint8_t {
    ...
    OFF = 0,
    /**
     * Basic automatic focus mode. In this mode, the lens does not move unless
     * the autofocus trigger action is called.
     */
    AUTO,
    /**
     * Close-up focusing mode - this mode is optimized for focusing on objects
     ...
     * this means slower focus movement and no overshoots.
     */
    CONTINUOUS_VIDEO,
    /**
     * In this mode, the AF algorithm modifies the lens position continually to
     * attempt to provide a constantly-in-focus image stream. The focusing
     * behavior should be suitable for still image capture; typically this means
     * focusing as fast as possible
     */
    CONTINUOUS_PICTURE,
    /**
     * Extended depth of field (digital focus) mode. The camera device will
     * produce images with an extended depth of field automatically. AF triggers
     * are ignored.
     */
    EDOF,
  };
  ...
  enum class AutoWhiteBalanceMode : uint8_t {
     ...
     * device uses incandescent light as the assumed scene illumination for
     * white balance.
     */
    INCANDESCENT,
    /**
  ...
  enum class SceneMode : uint8_t {
    ...
    UNSUPPORTED = 0,
    /**
     * If face detection support exists, use face detection data for auto-focus,
     * auto-white balance, and auto-exposure routines.
     */
    FACE_PRIORITY,
    /**
     * Optimized for photos of quickly moving objects. Similar to SPORTS scene
     * mode.
     */
    ACTION,
    /**
    ...
    SUNSET,
    /**
     * Optimized to avoid blurry photos due to small amounts of device motion
     * (for example: due to hand shake).
     */
    STEADYPHOTO,
    /**
    ...
    CANDLELIGHT,
    /**
     * Optimized for accurately capturing a photo of barcode for use by camera
     * applications that wish to read the barcode value.
     */
    BARCODE,
  };
  ...
  enum class AntiBandingMode : uint8_t {
    ...
    OFF = 0,
    /**
     * The camera device will adjust exposure duration to avoid banding problems
    ...
    MAINS_60_HZ,
    /**
     * The camera device will automatically adapt its antibanding routine to the
     * current illumination condition. This is the default mode if AUTO is
  ...
  enum class CaptureIntent : uint8_t {
    /**
     * The goal of this request doesn't fall into the other categories. The
     * camera device will default to preview-like behavior.
     */
    CUSTOM = 0,
    /**
     ...
     * use case. The camera device should take the highest-quality image
     * possible (given the other settings) without disrupting the frame rate of
     * video recording.
     */
    VIDEO_SNAPSHOT,
    /**
     * This request is for a ZSL usecase; the application will stream
     * full-resolution images and reprocess one or several later for a final
     * capture.
     */
    ZERO_SHUTTER_LAG,
  };
  ...
  enum class ControlMode : uint8_t {
    /**
     * Full application control of pipeline. All control by the device's
     * metering and focusing (3A) routines is disabled.
     */
    OFF = 0,
    /**
    ...
    AUTO,
    /**
     * Use a specific scene mode. Enabling this disables Auto-Exposure, AWB and
     * AF controls;
     */
    USE_SCENE_MODE,
  };
  ...
  enum class EffectMode : uint8_t {
    ...
    OFF = 0,
    /**
     * A "monocolor" effect where the image is mapped into a single color. This
     * will typically be grayscale.
     */
    MONO,
    /**
    ...
    NEGATIVE,
    /**
     * A "solarisation" effect (Sabattier effect) where the image is wholly or
     * partially reversed in tone.
     */
    SOLARIZE,
    /**
     * A "sepia" effect where the image is mapped into warm gray, red, and brown
     * tones.
     */
    SEPIA,
    /**
     * A "posterization" effect where the image uses discrete regions of tone
     * rather than a continuous gradient of tones.
     */
    POSTERIZE,
    /**
     * A "whiteboard" effect where the image is typically displayed as regions
     * of white, with black or grey details.
     */
    WHITEBOARD,
    /**
     * A "blackboard" effect where the image is typically displayed as regions
     * of black, with white or grey details.
     */
    BLACKBOARD,
    /**
  ...
  struct ManualExposureParams {
    ...
    DEPTHAI_SERIALIZE(ManualExposureParams, exposureTimeUs, sensitivityIso,
                      frameDurationUs);
  };
  ...
  struct StrobeTimings {
    ...
    DEPTHAI_SERIALIZE(StrobeTimings, exposureBeginOffsetUs, exposureEndOffsetUs,
                      durationUs);
  };
   ...
   * translating to how the camera pin FSIN/FSYNC is used:
   ...
   * polarity This normally requires a FSIN/FSYNC/trigger input for MyriadX
   * (usually GPIO 41), to generate timings
   */
  CameraControl &setStrobeExternal(int gpioNumber, int activeLevel = 1);

  // TODO API to set strobe line directly high/low (not following the exposure
  // window)
  // TODO API to set strobe timings, as offsets in relation to exposure window,
  // or fixed duration
   ...
   * Note: the region should be mapped to the configured sensor resolution,
   * before ISP scaling
   ...
   */
  CameraControl &setAutoFocusRegion(uint16_t startX, uint16_t startY,
   ...
   * @param height Region height
   */
  CameraControl &setAutoExposureRegion(uint16_t startX, uint16_t startY,
                                       uint16_t width, uint16_t height);

  /**
   * Set a command to specify auto exposure compensation. This modifies the
   * brightness target with positive numbers making the image brighter and
   * negative numbers making the image darker.
   * @param compensation Compensation value between -9..9, default 0
   */
  CameraControl &setAutoExposureCompensation(int compensation);

  /**
   * Set a command to specify the maximum exposure time limit for auto-exposure.
   * By default the AE algorithm prioritizes increasing exposure over ISO, up to
   * around frame-time (subject to further limits imposed by anti-banding)
   ...
   */
  CameraControl &setAutoExposureLimit(
      std::chrono::microseconds maxExposureTime);
   ...
   * (10ms in the example, with ISO at a minimum of 100), anti-banding is not
   * effective.
   *
   ...
   */
  CameraControl &setManualExposure(uint32_t exposureTimeUs,
                                   uint32_t sensitivityIso);
   ...
   * @param sensitivityIso Sensitivity as ISO value, usual range 100..1600
   */
  CameraControl &setManualExposure(std::chrono::microseconds exposureTime,
                                   uint32_t sensitivityIso);
   ...
   * @param colorTemperatureK Light source color temperature in kelvins, range
   * 1000..12000
   */
  ...
  void clearCommand(Command cmd) { setCommand(cmd, false); }
  ...
  DEPTHAI_SERIALIZE(CameraControl, cmdMask, autoFocusMode, lensPosition,
                    lensPositionRaw, lensPosAutoInfinity, lensPosAutoMacro,
                    expManual, aeRegion, afRegion, awbMode, sceneMode,
                    antiBandingMode, aeLockMode, awbLockMode, captureIntent,
                    controlMode, effectMode, frameSyncMode, strobeConfig,
                    strobeTimings, aeMaxExposureTimeUs, expCompensation,
                    brightness, contrast, saturation, sharpness, lumaDenoise,
                    chromaDenoise, wbColorTemp, lowPowerNumFramesBurst,
                    lowPowerNumFramesDiscard, enableHdr, miscControls);
  /**
