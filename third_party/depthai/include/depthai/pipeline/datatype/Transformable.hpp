...
class Transformable {
 protected:
  /**
   * Internal function to perform the transformation, to be implemented by
   * derived classes.
   */
...
class TransformableCRTP : public Transformable {
 public:
  /*
   * Returns a new message that has been transformed into the target image
   * transformation. The source transformation is taken from the message itself,
   * and an error is thrown if it is not set.
   *
   * The Derived class is expected to override the transformToInternal function,
   * which performs the actual transformation of the message data.
   */
 ...
 *
 * This class combines Buffer storage with the Transformable interface and
 * provides a virtual transformTo() entry point for host-side polymorphic
 * dispatch. Python users should inherit from TransformableBuffer, not
 * Transformable, when implementing custom messages that need transformation
 * support.
 */
class TransformableBuffer : public Buffer, public Transformable {
 ...
 public:
   ...
   * implement custom transformation logic. Generic callers use this entry point
   * when they need to transform a custom message without knowing its concrete
   * type. The base implementation throws, because it cannot preserve Python
   * subclass state automatically.
   *
   * @param target Target image transformation.
   */
  virtual std::shared_ptr<TransformableBuffer> transformTo(
      const ImgTransformation &target) const;

  void serialize(std::vector<std::uint8_t> &metadata,
                 DatatypeEnum &datatype) const override;

  DEPTHAI_SERIALIZE(TransformableBuffer, sequenceNum, ts, tsDevice,
