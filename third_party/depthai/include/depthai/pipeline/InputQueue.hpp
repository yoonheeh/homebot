...
class InputQueue {
 ...
 private:
  /**
   * @brief Construct a new Input Queue object. The constructor is private as we
   * only want to expose the relevant methods - only send for now
   *
  ...
  class InputQueueNode : public node::ThreadedHostNode {
   public:
    ...
    Node::Output output{
        *this, {"output", DEFAULT_GROUP, {{{DatatypeEnum::Buffer, true}}}}};
  };

  // Helper access functions
  inline Node::Output &getNodeOutput() { return inputQueueNode->output; }
  inline std::shared_ptr<Node> getNode() { return inputQueueNode; }

  /** Pointer to InputQueueNode that does the actual communication between host
   * and device */
