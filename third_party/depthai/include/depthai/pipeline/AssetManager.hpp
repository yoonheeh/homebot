...
/**
 * @brief Asset is identified with string key and can store arbitrary binary
 * data
 */
struct Asset {
  Asset() = default;
...
class AssetsMutable : public Assets {
 public:
  void set(std::string, std::uint32_t offset, std::uint32_t size,
...
class AssetManager /*: public Assets*/ {
 ...
 public:
   ...
   * @param alignment [Optional] alignment of asset data in asset storage.
   * Default is 64B
   */
  std::shared_ptr<dai::Asset> set(const std::string &key,
  ...
  /// Serializes
  void serialize(AssetsMutable &assets, std::vector<std::uint8_t> &assetStorage,
