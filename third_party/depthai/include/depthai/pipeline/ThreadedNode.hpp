...
class ThreadedNode : public Node {
 ...
 public:
  Output pipelineEventOutput{*this,
                             {"pipelineEventOutput",
                              DEFAULT_GROUP,
                              {{{DatatypeEnum::PipelineEvent, false}}}}};
   ...
   * prior to starting this node's main thread.
   */
  ...
  /**
   * @brief Creates a scoped event that sends start and end events for the
   * getting inputs state
   */
  utility::PipelineEventDispatcherInterface::BlockPipelineEvent inputBlockEvent(
      bool startNow = true);
  /**
   * @brief Creates a scoped event that sends start and end events for the
   * sending outputs state
   */
  utility::PipelineEventDispatcherInterface::BlockPipelineEvent
  outputBlockEvent(bool startNow = true);
  /**
   * @brief Creates a scoped event that sends start and end events for a custom
   * block event
   * @param type Type of the event
   * @param source Source name of the event
   */
  utility::PipelineEventDispatcherInterface::BlockPipelineEvent blockEvent(
