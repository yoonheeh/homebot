 ...
 */
class [[deprecated("Use Camera node instead")]] MonoCamera
    : public DeviceNodeCRTP<DeviceNode, MonoCamera, MonoCameraProperties>,
      public SourceNode {
 public:
 ...
 public:
  ...
  /**
   * Input for CameraControl message, which can modify camera parameters in
   * runtime Default queue is blocking with size 8
   */
  // Input inputControl{*this, "inputControl", Input::Type::SReceiver, true, 8,
  // {{DatatypeEnum::CameraControl, false}}};
  Input inputControl{*this,
                     {"inputControl",
                      DEFAULT_GROUP,
                      DEFAULT_BLOCKING,
                      DEFAULT_QUEUE_SIZE,
  ...
  // false}}};
  Output out{*this,
             {"out", DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};

  /**
   * Outputs ImgFrame message that carries RAW10-packed (MIPI CSI-2 format)
   * frame data.
   *
   * Captured directly from the camera sensor
   */
  // Output raw{*this, "raw", Output::Type::MSender, {{DatatypeEnum::ImgFrame,
  // false}}};
  Output raw{*this,
             {"raw", DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};

  /**
   * Outputs metadata-only ImgFrame message as an early indicator of an incoming
   * frame.
   *
   * It's sent on the MIPI SoF (start-of-frame) event, just after the exposure
   * of the current frame has finished and before the exposure for next frame
   * starts. Could be used to synchronize various processes with camera capture.
   * Fields populated: camera id, sequence number, timestamp
   */
  // Output frameEvent{*this, "frameEvent", Output::Type::MSender,
  // {{DatatypeEnum::ImgFrame, false}}};
  Output frameEvent{
      *this,
      {"frameEvent", DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};
   ...
   * Isp 3A rate (auto focus, auto exposure, auto white balance, camera controls
   etc.).
   * Default (0) matches the camera FPS, meaning that 3A is running on each
   frame.
   * Reducing the rate of 3A reduces the CPU usage on CSS, but also increases
   the convergence rate of 3A.
   * Note that camera controls will be processed at this rate. E.g. if camera is
   running at 30 fps, and camera control is sent at every frame,
   * but 3A fps is set to 15, the camera control messages will be processed at
  ...
  /**
   * Configures whether the camera `raw` frames are saved as MIPI-packed to
   * memory. The packed format is more efficient, consuming less memory on
   * device, and less data to send to host: RAW10: 4 pixels saved on 5 bytes,
   * RAW12: 2 pixels saved on 3 bytes. When packing is disabled (`false`), data
   * is saved lsb-aligned, e.g. a RAW10 pixel will be stored as uint16, on
   * bits 9..0: 0b0000'00pp'pppp'pppp. Default is auto: enabled for standard
   * color/monochrome cameras where ISP can work with both packed/unpacked, but
   * disabled for other cameras like ToF.
   */
