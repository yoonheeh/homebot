...
/**
 * @brief ObjectTracker node. Performs object tracking using Kalman filter and
 * hungarian algorithm.
 */
class ObjectTracker
    : public DeviceNodeCRTP<DeviceNode, ObjectTracker, ObjectTrackerProperties>,
      public HostRunnable {
 private:
 ...
 public:
  ...
  /**
   * Input ImgFrame message on which tracking will be performed. RGBp, BGRp,
   * NV12, YUV420p types are supported. Default queue is non-blocking with
   * size 4.
   */
  Input inputTrackerFrame{*this,
                          {"inputTrackerFrame",
                           DEFAULT_GROUP,
                           false,
                           4,
   ...
   */
  Input inputDetectionFrame{*this,
                            {"inputDetectionFrame",
                             DEFAULT_GROUP,
                             false,
                             4,
  ...
  Input inputDetections{*this,
                        {"inputDetections",
                         ...
                         false,
                         4,
                         {{{DatatypeEnum::ImgDetections, true}}},
                         true}};

  /**
   * Input ObjectTrackerConfig message with ability to modify parameters at
   * runtime. Default queue is non-blocking with size 4.
   */
  Input inputConfig{*this,
                    {"inputConfig",
                     ...
                     false,
                     4,
                     {{DatatypeEnum::ObjectTrackerConfig, false}}},
                    true};
   ...
   */
  Output out{*this,
             {"out", DEFAULT_GROUP, {{{DatatypeEnum::Tracklets, false}}}}};

  /**
   * Passthrough ImgFrame message on which tracking was performed.
   * Suitable for when input queue is set to non-blocking behavior.
   */
  Output passthroughTrackerFrame{*this,
   ...
   * Suitable for when input queue is set to non-blocking behavior.
   */
  Output passthroughDetectionFrame{*this,
   ...
   * Suitable for when input queue is set to non-blocking behavior.
   */
  Output passthroughDetections{*this,
                               {"passthroughDetections",
                                DEFAULT_GROUP,
                                {{{DatatypeEnum::ImgDetections, true}}}}};
  /**
   * Specify tracker threshold.
   * @param threshold Above this threshold the detected objects will be tracked.
   * Default 0, all image detections are tracked.
   */
  void setTrackerThreshold(float threshold);

  /**
   * Specify maximum number of object to track.
   * @param maxObjectsToTrack Maximum number of object to track. Maximum 60 in
   * case of SHORT_TERM_KCF, otherwise 1000.
   */
  void setMaxObjectsToTrack(std::int32_t maxObjectsToTrack);

  /**
   * Specify detection labels to track.
   * @param labels Detection labels to track. Default every label is tracked
   * from image detection network output.
   */
  ...
  /**
   * Set the occlusion ratio threshold. Used to filter out overlapping
   * tracklets.
  ...
  /**
   * Set the tracklet lifespan in number of frames. Number of frames after which
   * a LOST tracklet is removed.
   * @param trackletMaxLifespan Tracklet lifespan in number of frames. Default
   * 120.
   */
  void setTrackletMaxLifespan(uint32_t trackletMaxLifespan);

  /**
   * Set the tracklet birth threshold. Minimum consecutive tracked frames
   * required to consider a tracklet as a new (TRACKED) instance.
  ...
  /**
   * Enable or disable spatially-aware association. If disabled, only 2D
   * association is used.
   * @param enabled `true` enables spatially-aware association, `false` uses
   * 2D-only association. Default is false.
   */
  void setSpatialAssociation(bool enabled);

  /**
   * Set spatial association weight in [0,1].
   * @param weight Spatial association weight in [0,1] used to blend 2D and
   * spatial association scores (0 = 2D-only scoring, 1 = spatial-only scoring).
   *               This weight affects candidate scoring only; final acceptance
   ...
   * @param thresholdMeters Base spatial gating distance in meters. Default
   * is 1.5m.
   */
