...
class SPIOut : public DeviceNodeCRTP<DeviceNode, SPIOut, SPIOutProperties> {
 public:
   ...
   */
  void setStreamName(std::string name) { properties.streamName = name; }

  /**
   * Specifies SPI Bus number to use
   * @param id SPI Bus id
   */
  void setBusId(int busId) { properties.busId = busId; }

  void buildInternal() override { properties.busId = 0; }
};
