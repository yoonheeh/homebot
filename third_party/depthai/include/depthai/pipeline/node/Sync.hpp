...
class Sync : public DeviceNodeCRTP<DeviceNode, Sync, SyncProperties>,
 ...
 public:
   ...
   */
  // Output out{*this, "out", Output::Type::MSender,
  // {{DatatypeEnum::MessageGroup, false}}};
  Output out{*this,
             {"out", DEFAULT_GROUP, {{{DatatypeEnum::MessageGroup, false}}}}};
  ...
  /**
   * Set the number of attempts to get the specified max interval between
   * messages in the group
   * @param syncAttempts Number of attempts to get the specified max interval
   * between messages in the group:
   *   - if syncAttempts = 0 then the node sends a message as soon at the group
   * is filled
   *   - if syncAttempts > 0 then the node will make syncAttemts attempts to
   * synchronize before sending out a message
   *   - if syncAttempts = -1 (default) then the node will only send a message
