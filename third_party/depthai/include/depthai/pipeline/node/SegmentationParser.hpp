...
/**
 * @brief SegmentationParser node. Parses raw segmentation output from
 * segmentation neural networks into a dai::SegmentationMask datatype. The
 * parser supports two output model types:
 * 1. Single-channel output where the model argmaxes the class probabilities
 * internally and outputs a single channel mask with class indices.
 * 2. Multi-channel output where each channel corresponds to the probability map
 * for a specific class. The parser will perform argmax across channels to
 * generate the final mask. The parser can be configured to treat the first
 * class (index 0) as the background class, which will be ignored in the final
 * segmentation mask.
 *
 * @warning Only OAK4 supports running SegmentationParser on device. On other
 * platforms, the node will automatically switch to host execution.
 */
class SegmentationParser : public DeviceNodeCRTP<DeviceNode, SegmentationParser,
 ...
 public:
  ...
  std::shared_ptr<SegmentationParserConfig> initialConfig =
      std::make_shared<SegmentationParserConfig>();
  ...
  Input inputConfig{*this,
                    {"inputConfig",
                     ...
                     false,
                     4,
                     {{{DatatypeEnum::SegmentationParserConfig, false}}},
  ...
  Output out{
      *this,
      {"out", DEFAULT_GROUP, {{{DatatypeEnum::SegmentationMask, false}}}}};
   ...
   */
  std::shared_ptr<SegmentationParser> build(Node::Output &nnInput,
                                            const Model &model);

  /**
   * @brief Build SegmentationParser node with the specific head from NNArchive.
   * Useful when model outputs multiple segmentation heads.
   * @param nnInput: Output to link
   * @param head: Specific head from NNArchive to use for this parser
   */
  std::shared_ptr<SegmentationParser> build(
      Node::Output &nnInput, const dai::nn_archive::v1::Head &head);
   ...
   * @param backgroundClass Boolean indicating if the first class is the
   * background class
   *
   * @note Only applicable if the number of classes is greater than 1 and the
   * output classes are not in a single layer (eg. classesInOneLayer = false).
   */
 ...
 private:
  ...
  // TODO (aljazkonec1): common functions that are shared with NeuralNetwork,
  // DetectionNetwork, etc. should be moved to a common base class
