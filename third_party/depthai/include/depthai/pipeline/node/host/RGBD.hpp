...
class RGBD : public NodeCRTP<ThreadedHostNode, RGBD> {
 public:
   ...
   */
  Output pcl{*this,
             {"pcl", DEFAULT_GROUP, {{DatatypeEnum::PointCloudData, true}}}};
  /**
   * Output RGBD frames.
   */
  ...
  /**
   * @brief Build RGBD node with specified size. Note that this API is global
   * and if used autocreated cameras can't be reused.
   * @param autocreate If true, will create color and depth nodes if they don't
   * exist.
   ...
   */
  std::shared_ptr<RGBD> build(
      const std::shared_ptr<Camera> &camera, const DepthSource &depthSource,
      std::pair<int, int> frameSize = std::make_pair(640, 400),
      std::optional<float> fps = std::nullopt);
 ...
 private:
  ...
  void alignDepth(const DepthSource &depthSource,
                  ...
                  std::pair<int, int> frameSize, std::optional<float> fps);
