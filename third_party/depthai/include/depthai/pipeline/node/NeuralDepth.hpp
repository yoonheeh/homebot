...
/**
 * @brief NeuralDepth node. Compute depth from left-right image pair using
 * neural network.
 */
class NeuralDepth
    : public DeviceNodeCRTP<DeviceNode, NeuralDepth, NeuralDepthProperties> {
 protected:
 ...
 public:
  ...
  std::shared_ptr<NeuralDepthConfig> initialConfig =
      std::make_shared<NeuralDepthConfig>();
  std::shared_ptr<NeuralDepth> build(
      Output &left, Output &right,
      DeviceModelZoo model = DeviceModelZoo::NEURAL_DEPTH_480X300);
  ...
  Input inputConfig{*this,
                    {"inputConfig",
                     ...
                     true,
                     5,
                     {{{DatatypeEnum::NeuralDepthConfig, false}}}}};

  /**
   * Input NNData to parse
   */
  Input nnDataInput{*this,
                    {"nnDataInput",
                     DEFAULT_GROUP,
                     true,
                     5,
                     {{{DatatypeEnum::NNData, false}}}}};
   ...
   */
  Output edge{*this,
              {"edge", DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};
  ...
  Output confidence{
      *this,
      {"confidence", DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};
