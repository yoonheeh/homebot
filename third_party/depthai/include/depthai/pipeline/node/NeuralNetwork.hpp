 ...
 */
class NeuralNetwork : public DeviceNodeCRTP<DeviceNode, NeuralNetwork,
 ...
 public:
  ...
  /**
   * @brief Build NeuralNetwork node. Connect output to this node's input and
   * sets up the NNArchive.
   * @param output: Output to link
   * @param nnArchive: Neural network archive
   * @returns Shared pointer to NeuralNetwork node
   */
  std::shared_ptr<NeuralNetwork> build(Node::Output &input,
                                       const NNArchive &nnArchive);

  /**
   * @brief Build NeuralNetwork node. Connect Camera output to this node's input
   * and configure the inference model.
  ...
  std::shared_ptr<NeuralNetwork> build(
      const std::shared_ptr<Camera> &input, const Model &model,
...
#ifdef DEPTHAI_HAVE_OPENCV_SUPPORT
  /**
   * @brief Build NeuralNetwork node. Connect ReplayVideo output to this node's
   * input and configure the inference model.
   ...
   */
  std::shared_ptr<NeuralNetwork> build(
      const std::shared_ptr<ReplayVideo> &input, const Model &model,
      std::optional<float> fps = std::nullopt);
#endif
   ...
   */
  Output passthrough{
      *this, {"passthrough", DEFAULT_GROUP, {{{DatatypeEnum::Buffer, true}}}}};

  /**
   * Inputs mapped to network inputs. Useful for inferring from separate data
   * sources Default input is non-blocking with queue size 1 and waits for
   * messages
   */
  InputMap inputs{*this,
                  "inputs",
                  {DEFAULT_NAME,
                   DEFAULT_GROUP,
                   false,
   ...
   */
  OutputMap passthroughs{*this,
                         "passthroughs",
                         {"", DEFAULT_GROUP, {{{DatatypeEnum::Buffer, true}}}}};
 ...
 private:
  ...
  ImgFrameCapability getFrameCapability(
      const NNArchive &nnArchive,
      std::optional<ImgFrameCapability> expectedCapability = std::nullopt);
