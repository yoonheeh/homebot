...
class ImageFilters
    : public DeviceNodeCRTP<DeviceNode, ImageFilters, ImageFiltersProperties>,
      public HostRunnable {
 protected:
 ...
 public:
  ...
  std::shared_ptr<ImageFiltersConfig> initialConfig =
      std::make_shared<ImageFiltersConfig>();
   ...
   */
  std::shared_ptr<ImageFilters> build(
      Node::Output &input, ImageFiltersPresetMode presetMode =
                               ImageFiltersPresetMode::TOF_MID_RANGE);
   ...
   */
  std::shared_ptr<ImageFilters> build(
      ImageFiltersPresetMode presetMode =
          ImageFiltersPresetMode::TOF_MID_RANGE);

  /**
   * Input for image frames to be filtered
   */
  Node::Input input{*this,
                    {"input",
                     Node::DEFAULT_GROUP,
  ...
  Node::Output output{
      *this,
      {"output", Node::DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};
...
class ToFDepthConfidenceFilter
    : public DeviceNodeCRTP<DeviceNode, ToFDepthConfidenceFilter,
 ...
 public:
  ...
  std::shared_ptr<ToFDepthConfidenceFilterConfig> initialConfig =
      std::make_shared<ToFDepthConfidenceFilterConfig>();

  /**
   * Build the node.
   * @param depth Depth frame image, expected ImgFrame type is RAW8 or RAW16.
   * @param amplitude Amplitude frame image, expected ImgFrame type is RAW8 or
   * RAW16.
   * @param presetMode Preset mode for ToF depth confidence filter
   * @return Shared pointer to the node
   */
  std::shared_ptr<ToFDepthConfidenceFilter> build(
      Node::Output &depth, Node::Output &amplitude,
      ImageFiltersPresetMode presetMode =
          ImageFiltersPresetMode::TOF_MID_RANGE);
   ...
   */
  Node::Output filteredDepth{*this,
                             {"filteredDepth",
                              Node::DEFAULT_GROUP,
  ...
  Node::Output confidence{
      *this,
      {"confidence", Node::DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};
