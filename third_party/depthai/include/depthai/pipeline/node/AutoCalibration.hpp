...
class AutoCalibration : public DeviceNodeCRTP<DeviceNode, AutoCalibration,
                                              AutoCalibrationProperties>,
                        public HostRunnable {
 private:
  Subnode<node::DynamicCalibration> dynamicCalibration{*this,
                                                       "dynamicCalibration"};
 ...
 public:
  ...
  std::shared_ptr<AutoCalibration> build(
      const std::shared_ptr<Camera> &cameraLeft,
      const std::shared_ptr<Camera> &cameraRight);
  ...
  std::shared_ptr<AutoCalibrationConfig> initialConfig =
      std::make_shared<AutoCalibrationConfig>();
  ...
  Output output{*this,
                {"output",
                 DEFAULT_GROUP,
                 {{{DatatypeEnum::AutoCalibrationResult, false}}}}};
 ...
 private:
  ...
  bool updateCalibrationProcess(
      std::shared_ptr<dai::CalibrationHandler> calibration);

  void logReport(const Report &report, unsigned int iteration) const;

  void logConfig() const;

  std::shared_ptr<dai::CalibrationHandler> getNewCalibration(
      unsigned int maxNumIteration, Report &report);

  void loadData(unsigned int numImages);

  std::shared_ptr<dai::CalibrationMetrics> getMetrics(
      std::shared_ptr<dai::CalibrationHandler> calibration);

  AutoCalibrationProperties properties;

  // internal queues
  Output gateControlQueue{*this,
                          {"gateControlQueue",
                           DEFAULT_GROUP,
                           {{{DatatypeEnum::GateControl, false}}}}};

  Output dynamicCalibrationCommandQueue{
      *this,
      {"dynamicCalibrationCommandQueue",
       DEFAULT_GROUP,
       {{{DatatypeEnum::DynamicCalibrationControl, false}}}}};

  Input dynamicCalibrationQueue{
      *this,
      {"dynamicCalibrationQueue",
       DEFAULT_GROUP,
       false,
       1,
       {{DatatypeEnum::DynamicCalibrationResult, true}}}};

  Input metricsQueue{*this,
                     {"metricsQueue",
                      DEFAULT_GROUP,
                      false,
                      1,
                      {{DatatypeEnum::CalibrationMetrics, true}}}};

  Input coverageQueue{*this,
                      {"coverageQueue",
                       DEFAULT_GROUP,
                       false,
                       1,
                       {{DatatypeEnum::CoverageData, true}}}};

  Input gateOutput{
      *this,
      {"gateOutput", DEFAULT_GROUP, false, 1, {{DatatypeEnum::Buffer, true}}}};
