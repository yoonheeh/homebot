...
class XLinkInHost : public NodeCRTP<ThreadedHostNode, XLinkInHost> {
 private:
  ...
  std::shared_ptr<ADatatype> parsePacketizedData(
      const std::shared_ptr<PacketizedData> &packetizedData) const;
  void parseMessageGroup(
      const std::shared_ptr<MessageGroup> &messageGroup) const;
