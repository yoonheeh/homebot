 ...
 */
class PointCloud
    : public DeviceNodeCRTP<DeviceNode, PointCloud, PointCloudProperties>,
      public HostRunnable {
 public:
  ...
  class Impl {
   public:
    ...
    // Compute DENSE point cloud (width * height points, includes invalid z=0 or
    // negative)
    void computePointCloudDense(const uint8_t *depthData,
                                std::vector<Point3f> &points);

    // Compute DENSE colored point cloud from aligned depth+color (like RGBD
    // node)
    ...
    template <typename PointT>
    std::vector<PointT> filterValidPoints(
        const std::vector<PointT> &densePoints);
   ...
   private:
    ...
    void calcPointsChunkDense(const uint8_t *depthData,
                              std::vector<Point3f> &points,
                              unsigned int startRow, unsigned int endRow);
    void calcPointsChunkDenseColored(const uint8_t *depthData,
                                     const uint8_t *colorData,
                                     std::vector<Point3fRGBA> &points,
                                     unsigned int startRow,
                                     unsigned int endRow);
    void computePointCloudDenseCPU(const uint8_t *depthData,
                                   std::vector<Point3f> &points);
    void computePointCloudDenseCPUMT(const uint8_t *depthData,
                                     std::vector<Point3f> &points);
    void computePointCloudDenseGPU(const uint8_t *depthData,
                                   std::vector<Point3f> &points);
    void computePointCloudDenseColoredCPU(const uint8_t *depthData,
                                          const uint8_t *colorData,
                                          std::vector<Point3fRGBA> &points);
    void computePointCloudDenseColoredCPUMT(const uint8_t *depthData,
                                            const uint8_t *colorData,
                                            std::vector<Point3fRGBA> &points);
    ...
    static constexpr float DEFAULT_LENGTH_UNIT_MULTIPLIER =
        getLengthUnitMultiplier(LengthUnit::MILLIMETER);
    static constexpr float MM_MULTIPLIER =
        getLengthUnitMultiplier(LengthUnit::MILLIMETER);

    float scaleFactor = DEFAULT_LENGTH_UNIT_MULTIPLIER /
                        MM_MULTIPLIER;  // = 1.0 (mm to mm by default)
 ...
 public:
  ...
  std::shared_ptr<PointCloudConfig> initialConfig =
      std::make_shared<PointCloudConfig>();

  /**
   * Input PointCloudConfig message with ability to modify parameters in
  ...
  Input inputConfig{*this,
                    {"inputConfig",
                     ...
                     false,
                     4,
                     {{{DatatypeEnum::PointCloudConfig, false}}},
                     DEFAULT_WAIT_FOR_MESSAGE}};

  /**
   * Sync subnode for synchronized depth + color input.
   * When only depth is connected, Sync passes through single-item
   * MessageGroups. When both depth and color are connected, Sync pairs them by
   * timestamp.
   */
   ...
   */
  Output outputPointCloud{*this,
                          {"outputPointCloud",
                           DEFAULT_GROUP,
                           {{{DatatypeEnum::PointCloudData, false}}}}};
  ...
  Output passthroughDepth{
      *this,
      {"passthroughDepth", DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};
   ...
   * @param useSpecTranslation Use spec translation instead of calibration
   * (default: false)
   */
 ...
 private:
  ...
  void setCoordinateTransformation(const ImgFrame &depthFrame,
                                   const PointCloudConfig &config);

  // Processing methods for the two code paths
  void processDepthOnly(std::shared_ptr<ImgFrame> depthFrame,
                        std::shared_ptr<PointCloudData> pc, bool organized);
  void processColorized(std::shared_ptr<ImgFrame> depthFrame,
  ...
  // Extrinsics to set on the output PointCloudData after coordinate
  // transformation
  std::optional<Extrinsics> targetExtrinsics_;

  // Cached frame transformation — used to detect intrinsic/extrinsic/size
  // changes at runtime
