 ...
 *
 * Computes disparity and depth maps from a synchronized stereo camera pair
 * using OpenCL on the Adreno GPU. Supports both rectified and unrectified
 * inputs (controlled via @ref setRectification).
 */
class GPUStereo
    : public DeviceNodeCRTP<DeviceNode, GPUStereo, GPUStereoProperties> {
 protected:
 ...
 public:
   ...
   *
   * When enabled, the node rectifies the input images internally using
   * calibration data. When disabled, inputs are expected to be already
   * rectified.
   */
  ...
  Input leftInternal{*this,
                     {"leftFrameInternal",
                      DEFAULT_GROUP,
                      false,
                      1,
                      {{{DatatypeEnum::ImgFrame, false}}}}};
  Input rightInternal{*this,
                      {"rightFrameInternal",
                       DEFAULT_GROUP,
                       false,
                       1,
   ...
   * Note: postprocessing steps like LR-check/median filter are not applied to
   * confidence map.
   */
  ...
  std::shared_ptr<GPUStereoConfig> initialConfig =
      std::make_shared<GPUStereoConfig>();
