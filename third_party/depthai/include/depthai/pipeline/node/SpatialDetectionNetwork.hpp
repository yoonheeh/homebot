 ...
 */
using DepthSource =
    std::variant<std::shared_ptr<StereoDepth>, std::shared_ptr<NeuralDepth>,
                 std::shared_ptr<ToF>>;

/**
 * @brief SpatialDetectionNetwork node. Runs a neural inference on input image
 * and calculates spatial location data.
 */
class SpatialDetectionNetwork : public DeviceNodeGroup {
 public:
  ...
  static std::shared_ptr<SpatialDetectionNetwork> create(
  ...
  SpatialDetectionNetwork(const std::shared_ptr<Device> &device,
                          std::unique_ptr<Properties> props, bool confMode);
  ...
  /**
   * @brief Build SpatialDetectionNetwork node with specified depth source.
   * Connect Camera and depth source outputs to this node's inputs and configure
   * the inference model
   * @param inputRgb Camera node
   * @param depthSource Depth source node (StereoDepth, NeuralDepth, or ToF)
   * @param model: Neural network model description, NNArchive or HubAI model id
   * string
  ...
  std::shared_ptr<SpatialDetectionNetwork> build(
      const std::shared_ptr<Camera> &inputRgb, const DepthSource &depthSource,
      const Model &model, std::optional<float> fps = std::nullopt,
  ...
  Subnode<SpatialLocationCalculator> spatialLocationCalculator{
      *this, "spatialLocationCalculator"};
  ...
  // /**
  //  * Input SpatialLocationCalculatorConfig message with ability to modify
  ...
  /**
   * Passthrough message for depth frame on which the spatial location
   * calculation was performed. Suitable for when input queue is set to
  ...
  /**
   * @brief Set NNArchive for this Node. If the archive's type is SUPERBLOB, use
   * default number of shaves.
   *
  ...
  /**
   * @brief Set NNArchive for this Node, throws if the archive's type is not
   * SUPERBLOB
   *
   * @param nnArchive: NNArchive to set
  ...
  // Specify local filesystem path to load the blob (which gets loaded at
  // loadAssets)
  /**
  ...
  /**
   * Same functionality as the setBlobPath(). Load network blob into assets and
   * use once pipeline is started.
   *
   * @throws Error if file doesn't exist or isn't a valid network blob.
   * @param path Path to network blob
   */
  ...
  /**
   * Specifies confidence threshold at which to filter the rest of the
   * detections.
   * @param thresh Detection confidence must be greater than specified threshold
   ...
   * @param lowerThreshold LowerThreshold must be in the interval
   * [0,upperThreshold] and less than upperThreshold.
   */
   ...
   * values which will used to calculate spatial data
   * @param upperThreshold UpperThreshold must be in the interval
   ...
   */
  void setSpatialCalculationAlgorithm(
   ...
   * Step size 1 means that every pixel is taken into calculation, size 2 means
   * every second etc.
 ...
 private:
  ...
  void alignDepth(const DepthSource &depthSource,
  ...
  void alignDepthImpl(const std::shared_ptr<NeuralDepth> &neuralDepth,
                      const std::shared_ptr<Camera> &camera);
  void alignDepthImpl(const std::shared_ptr<ToF> &tof,
                      const std::shared_ptr<Camera> &camera);
};
