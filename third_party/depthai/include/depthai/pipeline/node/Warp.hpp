...
class Warp : public DeviceNodeCRTP<DeviceNode, Warp, WarpProperties> {
 ...
 private:
  void setWarpMesh(const float *meshData, int numMeshPoints, int width,
                   int height);
 ...
 public:
   ...
   * @param meshData 2D plane of mesh points, starting from top left to bottom
   * right
  ...
  void setWarpMesh(const std::vector<std::pair<float, float>> &meshData,
