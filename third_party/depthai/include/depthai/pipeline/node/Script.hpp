...
class Script : public DeviceNodeCRTP<DeviceNode, Script, ScriptProperties> {
 ...
 public:
  /**
   * Inputs to Script node. Can be accessed using subscript operator (Eg:
   * inputs['in1']) By default inputs are set to blocking with queue size 8
   */
  ...
  /**
   * Outputs from Script node. Can be accessed subscript operator (Eg:
   * outputs['out1'])
   */
  OutputMap outputs{
      *this,
      "io",
      {DEFAULT_NAME, DEFAULT_GROUP, {{{DatatypeEnum::Buffer, true}}}}};

  /**
   * Specify local filesystem path to load the script
   * @param path Filesystem path to load the script
   * @param name Optionally set a name of this script, otherwise the name
   ...
   * @param name Optionally set a name of this script
   */
  void setScript(const std::vector<std::uint8_t> &data,
                 const std::string &name = "");

  /**
   * @brief Get filesystem path from where script was loaded.
   *
   * @return std::filesystem::path from where script was loaded, otherwise
   * returns empty path
   */
   ...
   * When script loaded with setScriptPath() with name not provided, returns the
   * utf-8 string of that path. Otherwise, returns "<script>"
   *
