 ...
 */
class VideoEncoder
    : public DeviceNodeCRTP<DeviceNode, VideoEncoder, VideoEncoderProperties> {
 public:
  ...
  /**
   * Outputs ImgFrame message that carries BITSTREAM encoded (MJPEG, H264 or
   * H265) frame data. Mutually exclusive with out.
   */
  Output bitstream{
      *this, {"bitstream", DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};

  /**
   * Outputs EncodedFrame message that carries encoded (MJPEG, H264 or H265)
   * frame data. Mutually exclusive with bitstream.
   */
  Output out{*this,
             {"out", DEFAULT_GROUP, {{{DatatypeEnum::EncodedFrame, false}}}}};
  ...
  /// Set output bitrate in bps, for CBR rate control mode. 0 for auto (based on
  /// frame size and FPS)
  void setBitrate(int bitrate);
  /// Set output bitrate in kbps, for CBR rate control mode. 0 for auto (based
  /// on frame size and FPS)
   ...
   * Examples:
   *
   *  - 30 FPS video, keyframe frequency: 30. Every 1s a keyframe will be
   * inserted
   *
   *  - 60 FPS video, keyframe frequency: 180. Every 3s a keyframe will be
   * inserted
   *
   */
