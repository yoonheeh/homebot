 ...
 * Performs feature tracking and reidentification using motion estimation
 * between 2 consecutive frames.
 */
class FeatureTracker : public DeviceNodeCRTP<DeviceNode, FeatureTracker,
 ...
 public:
  ...
  std::shared_ptr<FeatureTrackerConfig> initialConfig =
      std::make_shared<FeatureTrackerConfig>();

  /**
   * Input FeatureTrackerConfig message with ability to modify parameters in
  ...
  Input inputConfig{*this,
                    {"inputConfig",
                     ...
                     false,
                     4,
                     {{{DatatypeEnum::FeatureTrackerConfig, false}}},
   ...
   */
  Output outputFeatures{*this,
                        {"outputFeatures",
                         DEFAULT_GROUP,
                         {{{DatatypeEnum::TrackedFeatures, false}}}}};

  /**
   * Passthrough message on which the calculation was performed.
   * Suitable for when input queue is set to non-blocking behavior.
   */
  Output passthroughInputImage{*this,
   ...
   * 2 shaves/memory slices are required for optical flow, 1 for corner
   * detection only.
