...
class DetectionNetwork : public DeviceNodeGroup {
 public:
  ...
  /**
   * @brief Build DetectionNetwork node. Connect output to this node's input.
   ...
   */
  std::shared_ptr<DetectionNetwork> build(Node::Output &input,
                                          const NNArchive &nnArchive);
  /**
   * @brief Build DetectionNetwork node. Connect Camera output to this node's
   ...
   * @returns Shared pointer to DetectionNetwork node
   */
  std::shared_ptr<DetectionNetwork> build(
  ...
  /*
   * @brief Build DetectionNetwork node. Connect ReplayVideo output to this
   * node's input and configure the inference model.
   ...
   * @param useCached: Use cached model if available
   */
  void setFromModelZoo(NNModelDescription description, int numShaves,
                       bool useCached = true);
  ...
  std::vector<std::pair<Input &, std::shared_ptr<Capability>>>
  getRequiredInputs() override;
