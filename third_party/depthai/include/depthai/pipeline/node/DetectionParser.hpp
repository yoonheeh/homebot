...
/**
 * @brief DetectionParser node. Parses detection results from Mobilenet-SSD or
 * YOLO neural networks.
 * @note If multiple detection heads are present in the NNArchive, only one type
 * is supported (either YOLO or Mobilenet-SSD) and the last one will be used.
 */
class DetectionParser : public DeviceNodeCRTP<DeviceNode, DetectionParser,
 ...
 public:
  ...
  /**
   * @brief Build DetectionParser node. Connect output to this node's input.
   * Also call setNNArchive() with provided NNArchive.
   * @param nnInput: Output to link
   * @param nnArchive: Neural network archive
   */
  std::shared_ptr<DetectionParser> build(Node::Output &nnInput,
  ...
  Input input{
      *this,
      {"in", DEFAULT_GROUP, true, 5, {{{DatatypeEnum::NNData, true}}}, true}};

  /**
   * Outputs image frame with detected edges
   */
  Output out{*this,
             {"out", DEFAULT_GROUP, {{{DatatypeEnum::ImgDetections, false}}}}};
   ...
   *
   * @warning If NN Family is set manually, user must ensure that it matches the
   ...
   * yolov6r1, yolov6r2 yolov8n, yolov6, yolov8, yolov10, yolov11, yolov3,
   * yolov3-tiny, yolov5, yolov7, yolo-p, yolov5-u
   */
  void setSubtype(const std::string &subtype);

  /**
   * Enable/disable keypoints decoding. If enabled, number of keypoints must
   * also be set.
   */
  ...
  /**
   * Set number of keypoints to decode. Automatically enables keypoints
   * decoding.
   */
   ...
   * @param edges Vector edges connections represented as pairs of keypoint
   * indices.
   ...
   */
  void decodeMobilenet(dai::NNData &nnData, dai::ImgDetections &outDetections,
                       float confidenceThr);
 ...
 private:
  ...
  void configureYOLONetworkParser(DetectionParserOptions &parser,
                                  const nn_archive::v1::Head &metadata);
  void checkKptExtraParams(DetectionParserOptions &parser,
