 ...
 * @brief Gate Node.
 *
 * This node acts as a valve for data pipelines. It controls the flow of
 * messages from the 'input' to the 'output' based on the state configured via
 * 'inputControl'. It can be configured to stay open indefinitely, stay closed,
 * or open for a specific number of messages.
 */
class Gate : public DeviceNodeCRTP<DeviceNode, Gate, GateProperties> {
 ...
 public:
   ...
   */
  Output output{*this,
                {"output", DEFAULT_GROUP, {{{DatatypeEnum::Buffer, true}}}}};

  /**
   * @brief Control input.
   * * Accepts 'GateControl' messages to dynamically change the Gate's state.
   * Use this to Open/Close the gate or set it to pass a specific number of
   * frames at runtime.
 ...
 private:
  ...
  const std::vector<std::reference_wrapper<MessageQueue>> inputs = {
      inputControl, input};
