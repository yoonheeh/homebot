...
class Camera : public DeviceNodeCRTP<DeviceNode, Camera, CameraProperties>,
 ...
 public:
  ...
  /**
   * Request output with advanced controls. Mainly to be used by custom node
   * writers.
   */
  Node::Output *requestOutput(const Capability &capability,
                              bool onHost) override;

  /**
   * Get a high resolution output with full FOV on the sensor.
   * By default the function will not use the resolutions higher than 5000x4000,
   * as those often need a lot of resources, making them hard to use in
   * combination with other nodes.
   * @param type Type of the output (NV12, BGR, ...) - by default it's
   * auto-selected for best performance
   * @param fps FPS of the output - by default it's auto-selected to highest
   * possible that a sensor config support or 30, whichever is lower
   * @param useHighestResolution If true, the function will use the highest
   * resolution available on the sensor, even if it's higher than 5000x4000
   */
  Node::Output *requestFullResolutionOutput(
      std::optional<ImgFrame::Type> type = std::nullopt,
      std::optional<float> fps = std::nullopt,
   ...
   * @param sensorResolution Sensor resolution to use - by default it's
   * auto-detected from the requested outputs
   * @param sensorFps Sensor FPS to use - by default it's auto-detected from the
   ...
   * @return Sensor type
   */
  inline CameraSensorType getSensorType() { return getProperties().sensorType; }
   ...
   */
  std::shared_ptr<Camera> setImageOrientation(
   ...
   */
  std::shared_ptr<Camera> build(dai::CameraBoardSocket boardSocket,
                                ReplayVideo &replay);
   ...
   * frame data.
   *
   * Captured directly from the camera sensor, and the source for the 'isp'
   * output.
   */
  Output raw{*this,
  ...
  /**
   * Set number of frames in raw pool (will be automatically reduced if the
   * maximum pool memory size is exceeded)
  ...
  /**
   * Set number of frames in isp pool (will be automatically reduced if the
   * maximum pool memory size is exceeded)
   * @param num Number of frames
   * @return Shared pointer to the camera node
   */
  ...
  /**
   * Set number of frames in all pools (will be automatically reduced if the
   * maximum pool memory size is exceeded)
 ...
 public:  // internal usage
  ...
  OutputMap dynamicOutputs{
      *this, "dynamicOutputs", {"", "", {{DatatypeEnum::ImgFrame, false}}}};
  Camera();
