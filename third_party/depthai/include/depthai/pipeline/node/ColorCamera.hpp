 ...
 */
class [[deprecated("Use Camera node instead")]] ColorCamera
    : public DeviceNodeCRTP<DeviceNode, ColorCamera, ColorCameraProperties>,
      public SourceNode {
 public:
 ...
 public:
   ...
   */
  Output video{*this,
               {"video", DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};

  /**
   * Outputs ImgFrame message that carries BGR/RGB planar/interleaved encoded
   * frame data.
   *
   * Suitable for use with NeuralNetwork node
   */
  Output preview{
      *this, {"preview", DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};

  /**
   * Outputs ImgFrame message that carries NV12 encoded (YUV420, UV plane
   * interleaved) frame data.
   *
   * The message is sent only when a CameraControl message arrives to
   * inputControl with captureStill command set.
   */
  Output still{*this,
               {"still", DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};
   ...
   */
  Output isp{*this,
             {"isp", DEFAULT_GROUP, {{{DatatypeEnum::ImgFrame, false}}}}};
  ...
  [[deprecated("Use 'setPreviewType()' instead")]] void setColorOrder(
      ColorCameraProperties::ColorOrder colorOrder);

  /// Get color order of preview output frames. RGB or BGR
  [[deprecated(
      "Use 'getPreviewType()' instead")]] ColorCameraProperties::ColorOrder
  getColorOrder() const;

  /// Set planar or interleaved data of preview output frames
  [[deprecated("Use 'setPreviewType()' instead")]] void setInterleaved(
      bool interleaved);
  ...
  /**
   * Set 'isp' output scaling (numerator/denominator), preserving the aspect
   * ratio. The fraction numerator/denominator is simplified first to a
   * irreducible form, then a set of hardware scaler constraints applies: max
   * numerator = 16, max denominator = 63
   */
  ...
  /**
   * Set 'isp' output scaling, per each direction. If the horizontal scaling
   * factor (horizNum/horizDen) is different than the vertical scaling factor
  ...
  /// Set 'isp' output scaling, per each direction, as <numerator, denominator>
  /// tuples
  void setIspScale(std::tuple<int, int> horizScale,
   ...
   * doesn't actually do on-sensor cropping (and MIPI-stream only that region),
   * but it does postprocessing on the ISP (on RVC).
   ...
   *
   * @param keep If true, a larger crop region will be considered to still be
   * able to create the final image in the specified aspect ratio. Otherwise
