 ...
 * pipeline.getState().nodes({nodeId1}).inputs() ->
 * std::unordered_map<std::string, QueueState>;
 * pipeline.getState().nodes({nodeId1}).inputs({inputName1}) ->
 * std::unordered_map<std::string, QueueState>;
...
class NodesStateApi {
 ...
 public:
  explicit NodesStateApi(std::vector<Node::Id> nodeIds,
                         std::shared_ptr<MessageQueue> pipelineStateOut,
                         std::shared_ptr<InputQueue> pipelineStateRequest)
      : nodeIds(std::move(nodeIds)),
        pipelineStateOut(pipelineStateOut),
        pipelineStateRequest(pipelineStateRequest) {}
  PipelineState summary();
  PipelineState detailed();
  std::unordered_map<
      Node::Id, std::unordered_map<std::string, NodeState::OutputQueueState>>
  outputs();
  std::unordered_map<
      Node::Id, std::unordered_map<std::string, NodeState::InputQueueState>>
  inputs();
  std::unordered_map<Node::Id,
                     std::unordered_map<std::string, NodeState::Timing>>
  otherTimings();
};
class NodeStateApi {
  Node::Id nodeId;
 ...
 public:
  explicit NodeStateApi(Node::Id nodeId,
                        std::shared_ptr<MessageQueue> pipelineStateOut,
                        std::shared_ptr<InputQueue> pipelineStateRequest)
      : nodeId(nodeId),
        pipelineStateOut(pipelineStateOut),
  ...
  std::unordered_map<std::string, NodeState::Timing> otherTimings(
      const std::vector<std::string> &timingNames);
...
class PipelineStateApi {
 ...
 public:
  PipelineStateApi(std::shared_ptr<MessageQueue> pipelineStateOut,
      ...
      : pipelineStateOut(std::move(pipelineStateOut)),
        pipelineStateRequest(std::move(pipelineStateRequest)) {
  ...
  void stateAsync(
      std::function<void(const PipelineState &)> callback,
      std::optional<PipelineEventAggregationConfig> config = std::nullopt);
};
