...
class MessageQueue : public std::enable_shared_from_this<MessageQueue> {
 ...
 private:
  ...
  std::unordered_map<CallbackId, std::shared_ptr<utility::WaitAnyNotifier>>
      notifiers;
 ...
 public:
  ...
  std::unordered_map<
      ...
      callbacks;  // Only public for the Python bindings
 ...
 public:
  ...
  explicit MessageQueue(std::string name, unsigned int maxSize = 16,
                        bool blocking = true,
                        utility::PipelineEventDispatcherInterface
                            *pipelineEventDispatcher = nullptr);
  ...
  /**
   * Blocks until any of the queues contains a message, all queues are closed,
   * or the timeout expires
   *
   * @param queues Queues to wait on
   * @param timeout Maximum time to wait, or no timeout if not specified
   * @returns True if at least one queue has a message available, false if all
   * queues are closed or the timeout expires first
   */
  static bool waitAny(
      const std::vector<std::reference_wrapper<MessageQueue>> &queues,
   ...
   * @param timeout Maximum time to wait, or no timeout if not specified
   * @returns Mapping containing one available message per queue key, or an
   * empty map if all queues are closed or the timeout expires first
   */
  static std::unordered_map<std::string, std::shared_ptr<ADatatype>> getAny(
      const std::unordered_map<std::string, MessageQueue &> &queues,
      std::optional<std::chrono::milliseconds> timeout = std::nullopt);
  template <typename T>

  /**
   * Waits for messages on any of the provided queues and returns all currently
   * available messages cast to the requested type
   *
   * @tparam T Requested message type
   * @param queues Mapping from output keys to queues to read from
   * @param timeout Maximum time to wait, or no timeout if not specified
   * @returns Mapping containing one available message per queue key that can be
   * cast to `T`, or an empty map if all queues are closed, the timeout expires
   * first, or no available messages match `T`
   */
  static std::unordered_map<std::string, std::shared_ptr<T>> getAny(
      const std::unordered_map<std::string, MessageQueue &> &queues,
      std::optional<std::chrono::milliseconds> timeout = std::nullopt) {
   ...
   *
   * @param blocking Specifies if block or overwrite the oldest message in the
   * queue
   */
   ...
   * @note If maxSize is smaller than size, queue will not be truncated
   * immediately, only after messages are popped
   */
  ...
  CallbackId addCallback(
      std::function<void(std::string, std::shared_ptr<ADatatype>)>);
   ...
   */
  CallbackId addCallback(
      const std::function<void(std::shared_ptr<ADatatype>)> &);
   ...
   * @returns True if queue isn't empty and the first element is of type T,
   * false otherwise
   */
  template <class T>
  bool has() {
  ...
  std::shared_ptr<T> tryGet() {
    ...
    if (pipelineEventDispatcher && pipelineEventDispatcher->sendEvents) {
      auto blockEvent =
          pipelineEventDispatcher->blockEvent(PipelineEvent::Type::INPUT, name);
  ...
  /**
   * Try to retrieve message from queue. If no message available, return
   * immediately with nullptr
   *
   * @returns Message or nullptr if no message available
   */
  std::shared_ptr<ADatatype> tryGet() { return tryGet<ADatatype>(); }
   ...
   * @returns Message or nullptr if no message available
   */
  std::shared_ptr<ADatatype> get() { return get<ADatatype>(); }
   ...
   * @returns Message or nullptr if no message available
   */
  std::shared_ptr<ADatatype> front() { return front<ADatatype>(); }
   ...
   * @returns Message of type T otherwise nullptr if message isn't type T or
   * timeout occurred
   */
  ...
  std::vector<std::shared_ptr<T>> getAll() {
    ...
    auto getInput = [this, &messages]() {
      bool notDestructed = this->queue.waitAndConsumeAll(
  ...
  template <class T, typename Rep, typename Period>
  std::vector<std::shared_ptr<T>> getAll(
      std::chrono::duration<Rep, Period> timeout, bool &hasTimedout) {
    if (queue.isDestroyed()) {
      throw QueueException(CLOSED_QUEUE_MESSAGE);
    }
    auto getInput = [&, this]() -> std::vector<std::shared_ptr<T>> {
  ...
  /**
   * Adds a message to the queue, which will be picked up and sent to the
   * device. Can either block if 'blocking' behavior is true or overwrite oldest
   ...
   * Can either block until timeout if 'blocking' behavior is true or overwrite
   * oldest
   *
   * @param msg Message to add to the queue
   * @param timeout Maximum duration to block in milliseconds
   */
  bool send(const std::shared_ptr<ADatatype> &msg,
