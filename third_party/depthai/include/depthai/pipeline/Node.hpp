...
#define DEFAULT_TYPES              \
 ...
 protected:
  ...
  std::unique_ptr<utility::PipelineEventDispatcherInterface>
  ...
  void setNodeRefs(
      std::initializer_list<std::pair<std::string, std::shared_ptr<Node> *>> l);
 ...
 public:
  ...
  class Output {
   ...
   private:
    ...
    utility::PipelineEventDispatcherInterface *pipelineEventDispatcher =
        nullptr;
   ...
   public:
    ...
    Output(Node &par, OutputDescription desc, bool ref = true)
        : parent(par),
          desc(std::move(desc)),
          pipelineEventDispatcher(par.pipelineEventDispatcher.get()) {
     ...
     */
    std::string getName() const { return desc.name; }

    /**
     * Get group of the output
     */
    std::string getGroup() const { return desc.group; }

    /**
     * Set group name for this output
     */
    void setGroup(std::string group) { desc.group = std::move(group); }

    /**
     * Set name for this output
     */
    void setName(std::string name) { desc.name = std::move(name); }
     ...
     *
     * @return std::shared_ptr<dai::MessageQueue>: shared pointer to an output
     * queue
     */
    std::shared_ptr<dai::MessageQueue> createOutputQueue(
        unsigned int maxSize = OUTPUT_QUEUE_DEFAULT_MAX_SIZE,
   ...
   private:
    ...
    void unlink(const std::shared_ptr<dai::MessageQueue> &queue) {
      connectedInputs.erase(std::remove(connectedInputs.begin(),
                                        connectedInputs.end(), queue.get()),
                            connectedInputs.end());
      queueConnections.erase(
          std::remove(queueConnections.begin(), queueConnections.end(),
                      QueueConnection{this, queue}),
          queueConnections.end());
    }
   ...
   public:
    ...
    /**
     * @brief Get XLink bridge associated with this output (only valid for
     * device outputs after pipeline build)
     *
     * @return std::shared_ptr<dai::node::internal::XLinkOutBridge>: pointer to
     * the XLink bridge or nullptr if not applicable
     */
   ...
   */
  class OutputMap
      : public std::unordered_map<std::pair<std::string, std::string>, Output,
                                  PairHash> {
   ...
   public:
    std::string name;
    OutputMap(Node &parent, std::string name, OutputDescription defaultOutput,
              bool ref = true);
  ...
  // Input extends the message queue with additional option that specifies
  // whether to wait for message or not
  struct InputDescription {
    std::string name = DEFAULT_NAME;    // Name of the input
    std::string group = DEFAULT_GROUP;  // Group of the input
    bool blocking{
        DEFAULT_BLOCKING};  // Whether to block when input queue is full
    int queueSize{DEFAULT_QUEUE_SIZE};  // Size of the queue
    std::vector<DatatypeHierarchy> types
        DEFAULT_TYPES;  // Possible datatypes that can be received
  ...
  class Input : public MessageQueue {
   ...
   public:
    enum class Type {
      SReceiver,
      MReceiver
    };  // TODO(Morato) - refactor, make the MReceiver a separate class
        // (shouldn't inherit from MessageQueue)
   ...
   public:
    ...
    explicit Input(Node &par, InputDescription desc, bool ref = true)
        : MessageQueue(
              desc.name.empty() ? par.createUniqueInputName() : desc.name,
              desc.queueSize, desc.blocking, par.pipelineEventDispatcher.get()),
          parent(par),
     ...
     * Specifies behavior whether to wait for this input when a Node processes
     * certain data or not.
     * @param waitForMessage Whether to wait for message to arrive to this input
     * or not
     */
    void setWaitForMessage(bool waitForMessage);

    /**
     * Get behavior whether to wait for this input when a Node processes certain
     * data or not
    ...
    /**
     * @brief Create an shared pointer to an input queue that can be used to
     * send messages to this input from onhost
     *
    ...
    std::shared_ptr<InputQueue> createInputQueue(
        unsigned int maxSize = INPUT_QUEUE_DEFAULT_MAX_SIZE,
        bool blocking = INPUT_QUEUE_DEFAULT_BLOCKING);

    /**
     * @brief Get XLink bridge associated with this input (only valid for device
     * inputs after pipeline build)
     *
     * @return std::shared_ptr<dai::node::internal::XLinkInBridge>: pointer to
     * the XLink bridge or nullptr if not applicable
     */
   ...
   */
  class InputMap
      : public std::unordered_map<std::pair<std::string, std::string>, Input,
                                  PairHash> {
    std::reference_wrapper<Node> parent;
 ...
 protected:
  ...
  // when Pipeline tries to serialize and construct on remote, it will check if
  // all connected nodes are on same pipeline
 ...
 protected:
  ...
  using SetConnectionInternal =
      std::unordered_set<ConnectionInternal, ConnectionInternal::Hash>;
  SetConnectionInternal connections;
  using ConnectionMap =
      std::unordered_map<std::shared_ptr<Node>, SetConnectionInternal>;
 ...
 public:
  ...
  virtual void link(std::shared_ptr<Node> in);
  virtual Node::Output *requestOutput(const Capability &capability,
                                      bool onHost);
  virtual std::vector<std::pair<Input &, std::shared_ptr<Capability>>>
  getRequiredInputs();
   ...
   */
  virtual bool isBuiltInNode() const { return false; }

  const NodeMap &getNodeMap() const { return nodeMap; }
...
class NodeCRTP : public Base {
 public:
  ...
  const char *getName() const override { return Derived::NAME; };

  bool isBuiltInNode() const override { return BuiltInNode; }
