...
struct ColorCameraProperties
    : PropertiesSerializable<Properties, ColorCameraProperties> {
  ...
  struct IspScale {
    ...
    DEPTHAI_SERIALIZE(IspScale, horizNumerator, horizDenominator, vertNumerator,
                      vertDenominator);
  };
  ...
  /**
   * Select the mock isp width. Overrides resolutionWidth/height if mockIsp is
   * connected.
   */
  int32_t mockIspWidth = AUTO;

  /**
   * Select the mock isp height. Overrides resolutionWidth/height if mockIsp is
   * connected.
   */
...
DEPTHAI_SERIALIZE_EXT(ColorCameraProperties, initialControl, boardSocket,
                      cameraName, imageOrientation, previewType, previewHeight,
                      previewWidth, videoWidth, videoHeight, stillWidth,
                      stillHeight, mockIspWidth, mockIspHeight, resolution, fps,
                      isp3aFps, sensorCropX, sensorCropY,
                      previewKeepAspectRatio, ispScale, numFramesPoolRaw,
                      numFramesPoolIsp, numFramesPoolVideo,
                      numFramesPoolPreview, numFramesPoolStill, warpMeshSource,
                      warpMeshUri, warpMeshWidth, warpMeshHeight, calibAlpha,
                      warpMeshStepWidth, warpMeshStepHeight, eventFilter,
                      rawPacked);
