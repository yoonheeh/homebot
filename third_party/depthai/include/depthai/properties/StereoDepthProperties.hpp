...
struct StereoDepthProperties
    : PropertiesSerializable<Properties, StereoDepthProperties> {
  ...
  struct RectificationMesh {
    ...
    DEPTHAI_SERIALIZE(RectificationMesh, meshLeftUri, meshRightUri, meshSize,
   ...
   * -1 means auto, memory will be allocated based on initial stereo settings
   * and number of shaves. 0 means that it will reuse the memory slices assigned
   * for main stereo algorithm. For optimal performance it's recommended to
   * allocate more than 0, so post processing will run in parallel with main
   * stereo algorithm. Minimum 1, maximum 6.
   */
  std::int32_t numPostProcessingMemorySlices = AUTO;

  /**
   * Whether to use horizontal focal length from calibration intrinsics (fx) or
   * calculate based on calibration FOV. Default value is true. If set to false
   * it's calculated from FOV and image resolution: focalLength = calib.width /
   * (2.f * tan(calib.fov / 2 / 180.f * pi));
   */
   ...
   * setMeshStep (Default: (16, 16)), can correct lens distortion.
   * Implementation for generating the mesh is same as opencv's
   ...
   * (design data) or from calibration. Suitable for debugging. Utilizes
   ...
   * from calibration. Suitable for debugging. Utilizes calibrated value as
   * default
   */
...
DEPTHAI_SERIALIZE_EXT(StereoDepthProperties, initialConfig, depthAlignCamera,
                      enableRectification, rectifyEdgeFillColor, width, height,
                      outWidth, outHeight, outKeepAspectRatio, mesh,
                      enableRuntimeStereoModeSwitch, numFramesPool,
                      numPostProcessingShaves, numPostProcessingMemorySlices,
                      focalLengthFromCalibration, useHomographyRectification,
                      enableFrameSync, baseline, focalLength,
                      disparityToDepthUseSpecTranslation,
                      rectificationUseSpecTranslation,
                      depthAlignmentUseSpecTranslation, alphaScaling);
}  // namespace dai
