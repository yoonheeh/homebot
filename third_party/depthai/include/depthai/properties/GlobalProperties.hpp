...
struct DeviceProperties : PropertiesSerializable<Properties, DeviceProperties> {
  ...
  /**
   * Set frequency of Leon OS - Increasing can improve performance, at the cost
   * of higher power draw
   */
  double leonCssFrequencyHz = 700 * 1000 * 1000;
  /**
   * Set frequency of Leon RT - Increasing can improve performance, at the cost
   * of higher power draw
   */
   ...
   */
  std::unordered_map<CameraBoardSocket, std::uint32_t>
  ...
  /**
   * Chunk size for splitting device-sent XLink packets, in bytes. A larger
...
DEPTHAI_SERIALIZE_EXT(DeviceProperties, leonCssFrequencyHz, leonMssFrequencyHz,
                      cameraTuningBlobSize, cameraTuningBlobUri,
                      cameraSocketTuningBlobSize, cameraSocketTuningBlobUri,
                      calibData, eepromId, xlinkChunkSize, sippBufferSize,
                      sippDmaBufferSize);
