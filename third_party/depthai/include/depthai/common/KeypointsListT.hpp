...
struct KeypointsListT {
 ...
 public:
  ...
  explicit KeypointsListT(std::vector<KeypointT> keypoints,
                          std::vector<Edge> edges)
      : keypoints(std::move(keypoints)), edges(std::move(edges)) {
    validateEdges();
  }
  explicit KeypointsListT(std::vector<KeypointT> keypoints)
  ...
  iterator begin() noexcept { return keypoints.begin(); }
  iterator end() noexcept { return keypoints.end(); }

  const_iterator begin() const noexcept { return keypoints.begin(); }
  const_iterator end() const noexcept { return keypoints.end(); }
  const_iterator cbegin() const noexcept { return keypoints.cbegin(); }
  const_iterator cend() const noexcept { return keypoints.cend(); }
  bool empty() const noexcept { return keypoints.empty(); }
  size_t size() const noexcept { return keypoints.size(); }
  value_type &operator[](size_t i) { return keypoints[i]; }
  const value_type &operator[](size_t i) const { return keypoints[i]; }
   ...
   */
  std::vector<KeypointT> getKeypoints() const { return keypoints; }
  ...
  std::vector<Point2f> getPoints2f() const {
    ...
    for (const auto &kp : keypoints) {
      coordinates.emplace_back(
          Point2f(kp.imageCoordinates.x, kp.imageCoordinates.y));
    }
    return coordinates;
  }
