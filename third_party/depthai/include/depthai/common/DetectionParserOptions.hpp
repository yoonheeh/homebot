...
struct DetectionParserOptions {
  ...
  YoloDecodingFamily decodingFamily =
      YoloDecodingFamily::TLBR;  // top left bottom right anchor free
  bool decodeKeypoints = false;
  bool decodeSegmentation = false;

  // TODO (aljazkonec1): instead of listing every parser property separately,
  // add serialization support to dai::nn_archive::v1::Head and pass the entire
  // head
  int classes;
...
DEPTHAI_SERIALIZE_EXT(DetectionParserOptions, nnFamily, subtype,
                      confidenceThreshold, decodingFamily, decodeKeypoints,
                      decodeSegmentation, classes, classNames, coordinates,
                      nKeypoints, strides, anchors, anchorMasks,
                      outputNamesToUse, anchorsV2, iouThreshold, keypointEdges,
                      keypointLabelNames);
