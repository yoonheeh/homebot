...
struct adl_serializer<std::optional<T>> {
  static void to_json(
      json &j,
      const std::optional<T> &opt) {  // NOLINT this is a specialization,
                                      // naming conventions don't apply
    if (opt == std::nullopt) {
      j = nullptr;
    } else {
  ...
  static void from_json(
      const json &j,
      std::optional<T> &opt) {  // NOLINT this is a specialization, naming
...
struct Encoding<std::optional<T>> : EncodingIO<std::optional<T>> {
  ...
  static constexpr std::size_t Size(const Type &value) {
    return value ? Encoding<T>::Size(*value)
  ...
  static constexpr Status<void> ReadPayload(EncodingByte prefix, Type *value,
    ...
    if (prefix == EncodingByte::Empty) {
      ...
      T temp;
      // T temp {} would be better but seems to trigger a bug in libnop or GCC
      // when reading std::optional<class enum>
