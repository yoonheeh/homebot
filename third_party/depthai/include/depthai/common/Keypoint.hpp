...
struct Keypoint {
  ...
  Keypoint() = default;
  explicit Keypoint(Point3f imageCoordinates, float conf = 0.f,
                    uint32_t label = 0, std::string labelName = "")
      : imageCoordinates(imageCoordinates),
        confidence(conf),
        label(label),
        labelName(std::move(labelName)) {}

  explicit Keypoint(Point2f imageCoordinates, float confidence = 0.f,
                    uint32_t label = 0, std::string labelName = "")
      : Keypoint(Point3f{imageCoordinates.x, imageCoordinates.y, 0.f},
                 confidence, label, std::move(labelName)) {}

  explicit Keypoint(float x, float y, float z, float confidence = 0.f,
                    uint32_t label = 0, std::string labelName = "")
  ...
  void transform(const ImgTransformation &source,
    ...
    // TODO (aljazkonec1) Possible issue: Keypoint does not store its own
    // normalization flag, so normalization is guessed based on if coordinates
    // are in range [0,1]. A keypoint can be normalized but outside the [0, 1]
    // range. Possible fix: Add hasNormalized and normalized fields to
    // KeypointsList and Keypoint.
    Point2f remappedPoint = source.remapPointTo(
        target, Point2f{imageCoordinates.x, imageCoordinates.y});
...
struct KeypointsList : KeypointsListT<Keypoint> {
 public:
  ...
  /**
   * Returns a new KeypointsList with the keypoints transformed into the target
   * image transformation under the assumption that the keypoints are defined in
   * the source image transformation.
   *
   * If the target transformation has a different source coordinate system (eg.
   * different camera socket) than the one the keypoints were originally
   ...
   */
  KeypointsList transformTo(const ImgTransformation &source,
                            const ImgTransformation &target) {
