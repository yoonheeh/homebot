...
struct Extrinsics {
 ...
 public:
  ...
  Extrinsics(std::vector<std::vector<float>> rotationMatrix,
             Point3f translation, CameraBoardSocket toCameraSocket,
             LengthUnit lengthUnit = LengthUnit::CENTIMETER);

  Extrinsics(const std::vector<std::vector<float>> &extrinsicsMatrix,
             CameraBoardSocket toCameraSocket,
             LengthUnit lengthUnit = LengthUnit::CENTIMETER);

  Extrinsics(std::array<std::array<float, 4>, 4> &extrinsicsMatrix,
             CameraBoardSocket toCameraSocket,
             LengthUnit lengthUnit = LengthUnit::CENTIMETER);

  std::vector<std::vector<float>> rotationMatrix;
  /**
   *  (x, y, z) pose of destCameraSocket w.r.t currentCameraSocket obtained
   * through calibration
   */
  Point3f translation;
  /**
   *  (x, y, z) pose of destCameraSocket w.r.t currentCameraSocket measured
   * through CAD design
   */
   ...
   * [  0   0   0  1 ]
   * ```
   * @note The full transformation matrix can only be obtained if both the
   * rotation matrix and the translation vector are set.
   */
  ...
  /**
   * Get the inverse of the extrinsic transformation matrix which is equal to
   * the transformation from the toCameraSocket to the current camera socket.
   * @param useSpecTranslation Set to true to force using spec translation
   * @param unit Units of the returned translation vector
   * @return a transformationMatrix which is 4x4 in homogeneous coordinate
   * system
   */
  std::array<std::array<float, 4>, 4> getInverseTransformationMatrix(
      bool useSpecTranslation = false,
      LengthUnit unit = LengthUnit::CENTIMETER) const;
   ...
   * ```
   */
  void setTransformationMatrix(const std::vector<std::vector<float>> &matrix,
   ...
   */
  void setTranslationVector(const dai::Point3f &translationVector,
                            LengthUnit unit = LengthUnit::CENTIMETER,
                            bool useSpecTranslation = false);
   ...
   * @param useSpecTranslation Set to true to force using spec translation
   * @param sourceUnit Units of the translation vector in the source Extrinsics
   * (this). Only relevant if useSpecTranslation is false.
   * @return a transformationMatrix which is 4x4 in homogeneous coordinate
   * system
   */
  std::array<std::array<float, 4>, 4> getExtrinsicsTransformationTo(
      const Extrinsics &to, bool useSpecTranslation = false,
      LengthUnit sourceUnit = LengthUnit::CENTIMETER) const;

  DEPTHAI_SERIALIZE_OPTIONAL(Extrinsics, rotationMatrix, translation,
                             specTranslation, toCameraSocket, lengthUnit);
};
