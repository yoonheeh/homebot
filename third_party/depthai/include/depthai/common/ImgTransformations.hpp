...
/**
 * ImgTransformation struct. Holds information of how a ImgFrame or related
 * message was transformed from their source. Useful for remapping from one
 * ImgFrame to another.
 */
struct ImgTransformation {
 private:
  std::array<std::array<float, 3>, 3> transformationMatrix = {
      {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}}};
  std::array<std::array<float, 3>, 3> transformationMatrixInv = {
      {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}}};  // Precomputed inverse matrix
  std::array<std::array<float, 3>, 3> sourceIntrinsicMatrix = {
      {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}}};
  std::array<std::array<float, 3>, 3> sourceIntrinsicMatrixInv = {
      {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}}};
 ...
 public:
  ...
  ImgTransformation(size_t width, size_t height)
      : srcWidth(width), srcHeight(height), width(width), height(height) {}
  ImgTransformation(size_t srcWidth, size_t srcHeight, size_t width,
                    size_t height)
      : srcWidth(srcWidth),
        srcHeight(srcHeight),
        width(width),
  ...
  /**
   * Retrieve the size of the frame. Should be equal to the size of the
   * corresponding ImgFrame message.
  ...
  /**
   * Retrieve the transformation matrix from the source frame to the current
   * frame.
   * @return Transformation matrix
   */
  std::array<std::array<float, 3>, 3> getMatrix() const;
  /**
   * Retrieve the inverse transformation matrix from the current frame to the
   * source frame.
   ...
   * matrices, distortion models, distortion coefficients, extrinsics, and sizes
   * are all equal.
   ...
   * @param source If true, the source field of view will be returned.
   * Otherwise, the current field of view will be returned.
   ...
   */
  ImgTransformation &addTransformation(
  ...
  ImgTransformation &setIntrinsicMatrix(
      std::array<std::array<float, 3>, 3> intrinsicMatrix);
   ...
   * different (e.g. different camera), the function will also use the
   * intrinsics to remap the point.
   * @param to Transformation to remap to
   * @param point Point to remap
   * @note This function assumes both transformations have the same source (eg.
   * same source camera socket). If they don't, remapping will be inaccurate.
   */
  dai::Point2f remapPointTo(const ImgTransformation &to,
                            dai::Point2f point) const;
  /**
   ...
   * @param point Point to remap
   */
  dai::Point2f remapPointFrom(const ImgTransformation &from,
                              dai::Point2f point) const;
  /**
   * Remap a rotated rect from this transformation to another. If the intrinsics
   * are different (e.g. different camera), the function will also use the
   * intrinsics to remap the rect.
   * @param to Transformation to remap to
   * @param rect RotatedRect to remap
   */
  dai::RotatedRect remapRectTo(const ImgTransformation &to,
                               dai::RotatedRect rect) const;
  /**
   * Remap a rotated rect to this transformation from another. If the intrinsics
   * are different (e.g. different camera), the function will also use the
   * intrinsics to remap the rect.
   * @param from Transformation to remap from
   * @param rect RotatedRect to remap
   */
  dai::RotatedRect remapRectFrom(const ImgTransformation &from,
                                 dai::RotatedRect rect) const;

  /**
   * Project a 3D spatial point into 2D point in the current frame defined by
   * this transformation.
   * @param point 3D point to project
   * @return Projected 2D point in the current frame
   * @note This function assumes that the point is in the coordinate system of
   * the current frame.
   */
  dai::Point2f project3DPoint(const dai::Point3f &point) const;

  /**
   * Project a 2D point from the source frame defined by this transformation
   * into a 2D point in the target frame defined by the to transformation. This
   * function will use the depth of the point to project it into 3D space and
   * then reproject it back to 2D in the target frame.
   ...
   */
  dai::Point2f projectPointTo(const ImgTransformation &to, dai::Point2f &point,
                              float depth) const;

  /**
   * Project a rotated rectangle from the source frame defined by this
   * transformation into the target frame defined by the to transformation. This
   * function assumes the rectangle is parallel to the image plane (i.e. the
   * entire rectangle is at the same depth away). The function fits the smallest
   * possible rectangle on to the projected corners of the original rectangle.
   * @param to Target transformation to project to
   * @param rect Source rotated rectangle in the current frame
   * @param depth (mm) Depth of the rectangle to project
   */
  dai::RotatedRect projectRectTo(const ImgTransformation &to, RotatedRect &rect,
                                 float depth) const;
   ...
   * @return Projected 2D point in the target frame (to transformation)
   * @note This function assumes that the point3f is in the coordinate system of
   * the current frame.
   */
  dai::Point2f project3DPointTo(const ImgTransformation &to,
                                const dai::Point3f &point) const;

  /**
   * Project a 3D point from the source frame (from transformation) into a 2D
   * point in the current frame (this transformation).
   ...
   * the source frame.
   */
  dai::Point2f project3DPointFrom(const ImgTransformation &from,
                                  const dai::Point3f &point) const;

  /**
   * Remap a 3D point from the source coordinate system of this transformation
   * to the coordinate system of the target transformation (to transformation).
   ...
   * the current frame.
   */
  dai::Point3f remap3DPointTo(const ImgTransformation &to,
                              const dai::Point3f &point) const;

  /**
   * Remap a 3D point to the coordinate system of this transformation from the
   * source coordinate system of the from transformation.
   * @param from Transformation to remap from
   * @param point 3D point to remap
   ...
   * the source frame.
   */
  dai::Point3f remap3DPointFrom(const ImgTransformation &from,
                                const dai::Point3f &point) const;

  /**
   * Get the extrinsic transformation matrix from the source coordinate system
   * of this transformation to the target coordinate system of the to
   * transformation.
   * @param to Target transformation to get extrinsics to
   * @param useSpecTranslation If true, the translation vector w.r.t. the CAD
   * design will be used instead of the translation vector obtained through
   * calibration.
   * @param sourceUnit The desired measurement unit in which to return the
   * transformation matrix in.
   * @return 4x4 homogeneous transformation matrix representing the extrinsics
   * from this transformation to the target transformation
   * @note Both transformations must have a common toCameraSocket. Otherwise
   * extrinsics cannot be calculated.
   */
  std::array<std::array<float, 4>, 4> getExtrinsicsTransformationMatrixTo(
      const ImgTransformation &to, bool useSpecTranslation = false,
      LengthUnit sourceUnit = LengthUnit::CENTIMETER) const;

  /**
   * Get the extrinsic rotation matrix from the source coordinate system of this
   * transformation to the target coordinate system of the to transformation.
   * @param to Transformation to get extrinsics to
   * @return 3x3 rotation matrix representing the extrinsic rotation from this
   * transformation to the target transformation
   */
  std::array<std::array<float, 3>, 3> getRotationMatrixTo(
      const ImgTransformation &to) const;

  /**
   * Get the extrinsic translation vector from the source coordinate system of
   * this transformation to the target coordinate system of the to
   * transformation.
   * @param to Transformation to get extrinsics to
   * @return 3x1 translation vector representing the extrinsic translation from
   * this transformation to the target transformation
   */
  std::array<float, 3> getTranslationVectorTo(
      const ImgTransformation &to, bool useSpecTranslation = false,
      LengthUnit sourceUnit = LengthUnit::CENTIMETER) const;
  ...
  /**
   * Check if the transformations are valid. The transformations are valid if
   * the source frame size and the current frame size are set.
   */
  bool isValid() const;

  DEPTHAI_SERIALIZE(ImgTransformation, extrinsics, transformationMatrix,
                    transformationMatrixInv, sourceIntrinsicMatrix,
                    sourceIntrinsicMatrixInv, distortionModel,
                    distortionCoefficients, srcWidth, srcHeight, width, height,
                    srcCrops);
};
