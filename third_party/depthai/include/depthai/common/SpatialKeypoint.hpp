...
struct SpatialKeypoint {
  ...
  explicit SpatialKeypoint(Point3f imageCoordinates,
                           Point3f spatialCoordinates = Point3f{},
                           float conf = 0.f, uint32_t label = 0,
                           std::string labelName = "")
  ...
  explicit SpatialKeypoint(Point2f imageCoordinates,
                           Point3f spatialCoordinates = Point3f{},
                           float conf = 0.f, uint32_t label = 0,
                           std::string labelName = "")
      : SpatialKeypoint(Point3f(imageCoordinates.x, imageCoordinates.y, 0.f),
                        spatialCoordinates, conf, label, labelName) {}
  explicit SpatialKeypoint(float x, float y, float z, float sx, float sy,
                           float sz, float conf = 0.f, uint32_t label = 0,
                           std::string labelName = "")
      : SpatialKeypoint(Point3f(x, y, z), Point3f{sx, sy, sz}, conf, label,
                        labelName) {}

  DEPTHAI_SERIALIZE(dai::SpatialKeypoint, imageCoordinates, confidence, label,
                    labelName, spatialCoordinates);
};
...
struct SpatialKeypointsList : KeypointsListT<SpatialKeypoint> {
 public:
  ...
  explicit SpatialKeypointsList(std::vector<SpatialKeypoint> keypoints,
  ...
  explicit SpatialKeypointsList(std::vector<SpatialKeypoint> keypoints,
                                LengthUnit unit = LengthUnit::MILLIMETER)
      : Base(std::move(keypoints)), unit(unit) {}
   ...
   *
   * @param spatialCoordinates vector of Point3f objects to set as spatial
   * coordinates.
   * @param spatialUnit The unit of the spatial coordinates. By default, spatial
   * coordinates are in millimeters.
   */
  void setSpatialCoordinates(const std::vector<Point3f> &spatialCoordinates,
                             LengthUnit spatialUnit = LengthUnit::MILLIMETER) {
    if (spatialCoordinates.size() != keypoints.size()) {
      throw std::invalid_argument(
          "Size of spatialCoordinates must match the number of keypoints.");
    }
  ...
  /**
   * Transforms the list of keypoints to the target image transformation under
   * the assumption that the keypoints are defined in the source image
   * transformation.
   *
   * If a keypoint does not have valid spatial coordinates
   * (`spatialCoordinates.z <= 0`), the depth-based 3D projection is skipped and
   ...
   */
  SpatialKeypointsList transformTo(const ImgTransformation &source,
                                   const ImgTransformation &target) const {
    SpatialKeypointsList transformed = *this;

    const auto transMatrix = source.getExtrinsicsTransformationMatrixTo(
        target, false, transformed.unit);
    float depthScale =
        getDistanceUnitScale(LengthUnit::MILLIMETER, transformed.unit);
    for (auto &kp : transformed.keypoints) {
      ...
      if (kp.spatialCoordinates.z > 0) {
        ...
        // TODO (aljazkonec1): projecting each point seperately is inefficient,
        // possible optimization by loading all the necessary matrixes once and
        // reusing them.
        Point2f remappedImgCoordinates =
            source.projectPointTo(target, imgCoordinates2f, depth);
        kp.imageCoordinates.x = remappedImgCoordinates.x;
        kp.imageCoordinates.y = remappedImgCoordinates.y;

        kp.spatialCoordinates =
            matrix::transformPoint3f(transMatrix, kp.spatialCoordinates);
      } else {
        Point2f remappedPoint = source.remapPointTo(
            target, Point2f{kp.imageCoordinates.x, kp.imageCoordinates.y});
