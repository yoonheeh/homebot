...
struct adl_serializer<std::variant<Args...>> {
  static void to_json(
      json &j,
      std::variant<Args...> const &v) {  // NOLINT this is a specialization,
                                         // naming conventi  ons don't apply
    std::visit(
        [&](auto &&value) {
          j["index"] = v.index();
  ...
  static void from_json(
      json const &j,
      std::variant<Args...> &v) {  // NOLINT this is a specialization,
                                   // naming conventi  ons don't apply
    auto const index = j.at("index").get<int>();
    ::dai::internal::VariantSwitch<sizeof...(Args) - 1>{}(index, j.at("value"),
                                                          v);
  }
};
}  // namespace nlohmann
...
struct VariantReadNop {
  ...
  void operator()(int index, Reader *reader, Variant &v) const {
    if (index == N) {
      // using Element = typename std::decay<std::variant_alternative_t<N,
      // Variant>>::type;
      ...
      if (!status) {
        throw std::runtime_error(
            "converting libnop object to variant failed: "
            "nop::Encoding::Read failed");
      }
      v = element;
    } else {
...
struct VariantReadNop<0> {
  ...
  void operator()(int index, Reader *reader, Variant &v) const {
    if (index == 0) {
      // using Element = typename std::decay<std::variant_alternative_t<0,
      // Variant>>::type;
      ...
      throw std::runtime_error(
          "converting libnop object to variant failed: invalid index");
    }
  }
};
}  // namespace internal
