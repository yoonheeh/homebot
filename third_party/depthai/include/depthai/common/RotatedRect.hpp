...
struct RotatedRect {
  ...
  RotatedRect() = default;
  /**
   * A rotated rectangle is specified by the center point, size, and rotation
   * angle in degrees.
   */
  RotatedRect(const Point2f &center, const Size2f &size, float angle)
    ...
    if (size.isNormalized() != center.isNormalized()) {
      throw std::runtime_error(
          "Cannot create RotatedRect with mixed normalization");
    }
  }
  /**
  ...
  bool isNormalized() const {
    if (size.isNormalized() != center.isNormalized()) {
      throw std::runtime_error(
          "Cannot determine normalization as size and "
          "center have mixed normalization.");
    }
    return size.isNormalized();
  }

  /**
   * Normalize the rotated rectangle. The normalized rectangle will have center
   * and size coordinates in range [0,1]
   * @return Normalized rotated rectangle
   */
  RotatedRect normalize(unsigned int width, unsigned int height) const {
    if (width == 0 || height == 0) {
      throw std::runtime_error(
          "Width and height must be non-zero to denormalize RotatedRect");
    }
    if (isNormalized()) return *this;
    RotatedRect normalized = *this;
    normalized.center = dai::Point2f(center.x / width, center.y / height, true);
    normalized.size =
        dai::Size2f(size.width / width, size.height / height, true);
    return normalized;
  }

  /**
   * Denormalize the rotated rectangle. The denormalized rectangle will have
   * center and size coordinates in range [0, width] and [0, height]
   * @return Denormalized rotated rectangle
   */
  RotatedRect denormalize(unsigned int width, unsigned int height,
    ...
    denormalized.center =
        dai::Point2f(center.x * width, center.y * height, false);
    denormalized.size =
        dai::Size2f(size.width * width, size.height * height, false);
    return denormalized;
  }
  ...
  /**
   * Returns the outer non-rotated rectangle in the COCO (xmin, ymin, width,
   * height) format.
   * @return (top-left point, size)
   */
  std::tuple<dai::Point2f, dai::Size2f> getOuterXYWH() const {
    auto [minx, miny, maxx, maxy] = getOuterRect();
    return {dai::Point2f{minx, miny, isNormalized()},
            dai::Size2f{maxx - minx, maxy - miny, isNormalized()}};
  }

  /**
   * Returns the outer non-rotated rectangle in the YOLO (xcenter, ycenter,
   * width, height) format.
   * @return (center point, size)
   */
  std::tuple<dai::Point2f, dai::Size2f> getOuterCXCYWH() const {
    auto [minx, miny, maxx, maxy] = getOuterRect();
    return {dai::Point2f{(minx + maxx) / 2.0f, (miny + maxy) / 2.0f,
                         isNormalized()},
            dai::Size2f{maxx - minx, maxy - miny, isNormalized()}};
  }
};
