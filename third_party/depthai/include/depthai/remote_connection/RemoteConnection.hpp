 ...
 * @brief Runs a websocket server exposing DepthAI messages as well as a static
 * frontend UI
 */
class RemoteConnection {
 public:
  static constexpr auto DEFAULT_WEBSOCKET_PORT =
  ...
  static constexpr auto DEFAULT_ADDRESS =
      "0.0.0.0";  ///< Default address to bind.
   ...
   * @param useVisualizationIfAvailable Whether to enable visualization on the
   * message if available or send message as is.
   */
  void addTopic(const std::string &topicName, Node::Output &output,
                const std::string &group = "",
                bool useVisualizationIfAvailable = true);
  ...
  std::shared_ptr<MessageQueue> addTopic(
      const std::string &topicName, const std::string &group = "",
      unsigned int maxSize = 16, bool blocking = false,
      bool useVisualizationIfAvailable = true);
   ...
   * @note After removing a topic any messages sent to it will cause an
   * exception to be called on the sender, since this closes the queue.
