...
struct NNArchiveOptions {
  // General parameters
  DEPTAHI_ARG_DEFAULT(NNArchiveEntry::Compression, compression,
...
class NNArchive {
 public:
  /**
   * @brief Construct a new NNArchive object - a container holding a model and
   * its configuration
   *
   * @param archivePath: Path to the archive file
   * @param options: Archive options such as compression, number of shaves, etc.
   * See NNArchiveOptions.
   */
  NNArchive(const std::filesystem::path &archivePath,
            NNArchiveOptions options = {});

  /**
   * @brief Return a SuperVINO::Blob from the archive if getModelType() returns
   * BLOB, nothing otherwise
   *
  ...
  /**
   * @brief Return a SuperVINO::SuperBlob from the archive if getModelType()
   * returns SUPERBLOB, nothing otherwise
   *
  ...
  /**
   * @brief Return a model from the archive if getModelType() returns OTHER or
   ...
   * @note this function is only valid for models with NCHW and NHWC input
   * formats
   * @return std::vector<std::pair<int, int>>: inputSize
   */
  std::optional<std::pair<uint32_t, uint32_t>> getInputSize(
 ...
 private:
  ...
  std::vector<uint8_t> readModelFromArchive(
      const std::filesystem::path &archivePath,
      const std::string &modelPathInArchive) const;
