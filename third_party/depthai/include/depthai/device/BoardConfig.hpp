...
struct BoardConfig {
  ...
  struct Network {
    ...
    uint16_t mtu = 0;
    /// Sets the `TCP_NODELAY` option for XLink TCP sockets (disable Nagle's
    /// algorithm), reducing latency at the expense of a small hit for max
    /// throughput. Default is `true`
    bool xlinkTcpNoDelay = true;
  };

  Network network;

  /// Optional list of FreeBSD sysctl parameters to be set (system, network,
  /// etc.). For example: "net.inet.tcp.delayed_ack=0" (this one is also set by
  /// default)
...
DEPTHAI_SERIALIZE_EXT(BoardConfig::USB, vid, pid, flashBootedVid,
                      flashBootedPid, maxSpeed, productName, manufacturer);
DEPTHAI_SERIALIZE_EXT(BoardConfig::Network, mtu, xlinkTcpNoDelay);
DEPTHAI_SERIALIZE_EXT(BoardConfig::GPIO, mode, direction, level, pull, drive,
                      schmitt, slewFast);
DEPTHAI_SERIALIZE_EXT(BoardConfig::UART, tmp);
DEPTHAI_SERIALIZE_EXT(BoardConfig::Camera, name, sensorType, orientation);
DEPTHAI_SERIALIZE_EXT(BoardConfig::IMU, bus, interrupt, wake, csGpio, boot,
                      reset);
DEPTHAI_SERIALIZE_EXT(BoardConfig::UVC, cameraName, width, height, frameType,
                      enable);
DEPTHAI_SERIALIZE_EXT(BoardConfig, usb, network, sysctl, watchdogTimeoutMs,
                      watchdogInitialDelayMs, gpio, uart, pcieInternalClock,
                      usb3PhyInternalClock, emmc, logPath, logSizeMax,
                      logVerbosity, logDevicePrints, nonExclusiveMode, camera,
                      imu, uvc, defaultImuExtr);
