 ...
 private:
  ...
  class GateImpl {
   public:
    ...
    virtual bool createSession(std::string version, bool exclusive,
                               XLinkPlatform_t platform, std::string &sessionId,
                               std::atomic_bool &sessionCreated) = 0;
    ...
    virtual std::optional<std::vector<uint8_t>> getFile(
        const std::string &fileUrl, std::string &filename) = 0;
  };
  ...
  class USBImpl : public GateImpl {
   public:
    ...
    bool createSession(std::string version, bool exclusive,
                       XLinkPlatform_t platform, std::string &sessionId,
                       std::atomic_bool &sessionCreated) override;
    ...
    std::optional<std::vector<uint8_t>> getFile(const std::string &fileUrl,
