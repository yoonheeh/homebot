...
class CrashDump {
 public:
  ...
  /**
   * @brief Serialize the crash dump to a byte array (bytes in filesystem after
   * calling toTar)
   * @return Byte array
   */
  ...
  /**
   * @brief Deserialize the crash dump from a byte array (bytes in filesystem
   * after calling toBytes)
   * @param bytes Byte array
   * @return Unique pointer to the appropriate CrashDump subclass
   */
  static std::unique_ptr<CrashDump> fromBytes(
...
class CrashDumpRVC2 final : public CrashDump {
 public:
  struct CrashReport {
    ...
    struct ThreadCallstack {
      ...
      struct CallstackContext {
        ...
        std::string context;
        DEPTHAI_SERIALIZE(CallstackContext, callSite, calledTarget,
                          framePointer, context);
      };

      std::vector<CallstackContext> callStack;

      DEPTHAI_SERIALIZE(ThreadCallstack, threadId, threadName, threadStatus,
                        stackBottom, stackTop, stackPointer, instructionPointer,
                        callStack);
    };
    ...
    DEPTHAI_SERIALIZE(CrashReport, processor, errorSource, crashedThreadId,
                      errorSourceInfo, threadCallstack, prints, uptimeNs,
  ...
  struct CrashReportCollection {
    ...
    std::string depthaiCommitHash;
    std::string deviceId;
    DEPTHAI_SERIALIZE(CrashReportCollection, crashReports, depthaiCommitHash,
                      deviceId);
  };
  ...
  Platform getPlatform() const override { return Platform::RVC2; }

  std::string getCrashDumpVersion() const override { return "1.0.0"; }
...
class CrashDumpRVC4 final : public CrashDump {
 public:
  ...
  Platform getPlatform() const override { return Platform::RVC4; }
