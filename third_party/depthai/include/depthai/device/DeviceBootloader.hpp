...
class DeviceBootloader {
 public:
   ...
   * @returns Tuple of boolean and DeviceInfo. If found boolean is true and
   * DeviceInfo describes the device. Otherwise false
   */
   ...
   * @param compress Optional boolean which specifies if contents should be
   * compressed
  ...
  static std::vector<uint8_t> createDepthaiApplicationPackage(
      const Pipeline &pipeline, const fs::path &pathToCmd = {},
      bool compress = false, std::string applicationName = "",
   ...
   */
  static std::vector<uint8_t> createDepthaiApplicationPackage(
      const Pipeline &pipeline, bool compress, std::string applicationName = "",
      bool checkChecksum = false);
   ...
   * Connects to or boots device in bootloader mode depending on devInfo state.
   * @param devInfo DeviceInfo of which to boot or connect to
   * @param allowFlashingBootloader (bool) Set to true to allow flashing the
   * devices bootloader
   */
  template <typename T,
   ...
   * possible. This constructor will automatically boot into specified
   * bootloader type if not already running
   * @param devInfo DeviceInfo of which to boot or connect to
   * @param type Type of bootloader to boot/connect to.
   * @param allowFlashingBootloader Set to true to allow flashing the devices
   * bootloader. Defaults to false
   */
  ...
  /**
   * Connects to or boots device in bootloader mode depending on devInfo state
   * with a custom bootloader firmware.
   * @param devInfo DeviceInfo of which to boot or connect to
   * @param pathToBootloader Custom bootloader firmware to boot
   * @param allowFlashingBootloader Set to true to allow flashing the devices
   * bootloader. Defaults to false
   */
  DeviceBootloader(const DeviceInfo &devInfo, const fs::path &pathToBootloader,
                   bool allowFlashingBootloader = false);
   ...
   * bootloader. Defaults to false
   */
  DeviceBootloader(std::string nameOrDeviceId,
                   bool allowFlashingBootloader = false);
   ...
   * @param progressCallback Callback that sends back a value between 0..1 which
   * signifies current flashing progress
  ...
  std::tuple<bool, std::string> flash(
      std::function<void(float)> progressCallback, const Pipeline &pipeline,
      bool compress = false, std::string applicationName = "",
      Memory memory = Memory::AUTO, bool checkChecksum = false);
   ...
   * @param applicationName Optional name the application that is flashed
   */
  std::tuple<bool, std::string> flash(const Pipeline &pipeline,
                                      bool compress = false,
                                      std::string applicationName = "",
   ...
   */
  std::tuple<bool, std::string> flashDepthaiApplicationPackage(
      std::function<void(float)> progressCallback, std::vector<uint8_t> package,
      Memory memory = Memory::AUTO);
   ...
   */
  std::tuple<bool, std::string> flashDepthaiApplicationPackage(
      std::vector<uint8_t> package, Memory memory = Memory::AUTO);
   ...
   */
  std::tuple<bool, std::string> flashBootloader(
      std::function<void(float)> progressCallback, const fs::path &path = {});
   ...
   */
  std::tuple<bool, std::string> flashBootloader(
      Memory memory, Type type, std::function<void(float)> progressCallback,
      const fs::path &path = {});

  /**
   * Flashes user bootloader to the current board. Available for NETWORK
   * bootloader type
   * @param progressCallback Callback that sends back a value between 0..1 which
   * signifies current flashing progress
   * @param path Optional parameter to custom bootloader to flash
   */
  std::tuple<bool, std::string> flashUserBootloader(
      std::function<void(float)> progressCallback, const fs::path &path = {});
   ...
   */
  std::tuple<bool, std::string> flashGpioModeBootHeader(Memory memory,
                                                        int gpioMode);
   ...
   * @param location Target location the header should boot to. Default to
   * location of bootloader
   * @param dummyCycles SPI specific parameter
   * @param offset Offset in memory to flash the header to. Defaults to offset
   * of boot header
   * @returns status as std::tuple<bool, std::string>
   */
  std::tuple<bool, std::string> flashBootHeader(Memory memory,
                                                int32_t frequency = -1,
                                                int64_t location = -1,
   ...
   * @returns status as std::tuple<bool, std::string>
   */
  std::tuple<bool, std::string> flashFastBootHeader(Memory memory,
                                                    int32_t frequency = -1,
                                                    int64_t location = -1,
                                                    int32_t dummyCycles = -1,
                                                    int64_t offset = -1);
  ...
  std::tuple<bool, std::string> flashCustom(
      Memory memory, size_t offset, const std::vector<uint8_t> &data,
      std::function<void(float)> progressCb = nullptr);
  std::tuple<bool, std::string> flashCustom(
      Memory memory, size_t offset, const uint8_t *data, size_t size,
      std::function<void(float)> progressCb = nullptr);
  std::tuple<bool, std::string> flashCustom(
      Memory memory, size_t offset, std::string filename,
      std::function<void(float)> progressCb = nullptr);
   ...
   * @param progressCallback Callback that sends back a value between 0..1 which
   * signifies current reading progress
   */
  std::tuple<bool, std::string> readCustom(
      Memory memory, size_t offset, size_t size, std::vector<uint8_t> &data,
      std::function<void(float)> progressCb = nullptr);
  std::tuple<bool, std::string> readCustom(
      Memory memory, size_t offset, size_t size, uint8_t *data,
      std::function<void(float)> progressCb = nullptr);
  std::tuple<bool, std::string> readCustom(
      Memory memory, size_t offset, size_t size, std::string filename,
      std::function<void(float)> progressCb = nullptr);
  std::tuple<bool, std::string, std::vector<uint8_t>> readCustom(
   ...
   * data
   */
  nlohmann::json readConfigData(Memory memory = Memory::AUTO,
                                Type type = Type::AUTO);
   ...
   */
  std::tuple<bool, std::string> flashConfigData(nlohmann::json configData,
                                                Memory memory = Memory::AUTO,
                                                Type type = Type::AUTO);
   ...
   * @param type Optional - for which type of bootloader to flash configuration
   */
  std::tuple<bool, std::string> flashConfigFile(const fs::path &configPath,
                                                Memory memory = Memory::AUTO,
                                                Type type = Type::AUTO);
   ...
   * @param type Optional - for which type of bootloader to clear configuration
   * data
   */
  std::tuple<bool, std::string> flashConfigClear(Memory memory = Memory::AUTO,
                                                 Type type = Type::AUTO);
   ...
   * @param type Optional - for which type of bootloader to flash configuration
   */
  std::tuple<bool, std::string> flashConfig(const Config &config,
                                            Memory memory = Memory::AUTO,
                                            Type type = Type::AUTO);
  ...
  /**
   * @returns True when bootloader was booted using latest bootloader integrated
   * in the library. False when bootloader is already running on the device and
   * just connected to.
   */
 ...
 private:
  ...
  // private methods
  void init(bool embeddedMvcmd, const fs::path &pathToMvcmd,
            std::optional<bootloader::Type> type, bool allowBlFlash);
  template <typename T>
  ...
  std::tuple<bool, std::string> flashCustom(
      Memory memory, size_t offset, const uint8_t *data, size_t size,
      std::string filename, std::function<void(float)> progressCb);
  std::tuple<bool, std::string> readCustom(
      Memory memory, size_t offset, size_t size, uint8_t *data,
      std::string filename, std::function<void(float)> progressCb);
...
inline std::ostream &operator<<(std::ostream &out,
                                const dai::DeviceBootloader::Type &type) {
  switch (type) {
...
inline std::ostream &operator<<(std::ostream &out,
                                const dai::DeviceBootloader::Memory &memory) {
  switch (memory) {
...
inline std::ostream &operator<<(std::ostream &out,
                                const dai::DeviceBootloader::Section &type) {
  switch (type) {
...
inline std::ostream &operator<<(std::ostream &out,
                                const dai::DeviceBootloader::Version &v) {
