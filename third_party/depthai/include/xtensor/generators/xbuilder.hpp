...
using integer_with_signed_integer =
    std::conjunction<both_integer<T, S>, xtl::is_signed<S>>;

template <class T, class S>
using integer_with_unsigned_integer =
    std::conjunction<both_integer<T, S>, std::is_unsigned<S>>;
...
class concatenate_access {
 public:
  ...
  template <class It>
  inline value_type access(const tuple_type &t, size_type axis, It first,
                           It last) const {
    ...
    auto get = [&](auto &arr) {
      ...
      for (size_t i = 0; i < end; i++) {
        ...
        const size_t stride = std::accumulate(
            shape.begin() + i + 1, shape.end(), 1, std::multiplies<size_t>());
        if (i == axis) {
