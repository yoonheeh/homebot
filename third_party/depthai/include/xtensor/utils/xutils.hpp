...
struct has_storage_type<
    T, void_t<typename xcontainer_inner_types<T>::storage_type>>
    : std::negation<
          std::is_same<typename std::remove_cv<typename xcontainer_inner_types<
                           T>::storage_type>::type,
                       invalid_type>> {};
...
struct has_assign_to<E1, E2,
                     void_t<decltype(std::declval<const E2 &>().assign_to(
                         std::declval<E1 &>()))>> : std::true_type {};
...
struct has_memory_address<
    T, void_t<decltype(std::addressof(*std::declval<T>().begin()))>>
    : std::true_type {};
...
struct overlapping_memory_checker_traits {
  static bool check_overlap(const E &, const memory_range &) { return true; }
};

template <class E>
struct overlapping_memory_checker_traits<
    E, std::enable_if_t<has_memory_address<E>::value>> {
  static bool check_overlap(const E &expr, const memory_range &dst_range) {
    if (expr.size() == 0) {
      return false;
    } else {
      return dst_range.overlaps(std::addressof(*expr.begin()),
                                std::addressof(*expr.rbegin()));
    }
  }
};
...
struct overlapping_memory_checker_base {
  ...
  bool check_overlap(const E &expr) const {
    if (!m_dst_range.m_first || !m_dst_range.m_last) {
      ...
      return overlapping_memory_checker_traits<E>::check_overlap(expr,
                                                                 m_dst_range);
    }
  }
};
...
struct overlapping_memory_checker<
    Dst, std::enable_if_t<has_memory_address<Dst>::value>>
    : overlapping_memory_checker_base {
  explicit overlapping_memory_checker(const Dst &aDst)
      : overlapping_memory_checker_base([&]() {
          if (aDst.size() == 0) {
            ...
            return memory_range(std::addressof(*aDst.begin()),
                                std::addressof(*aDst.rbegin()));
          }
        }()) {}
};
...
struct get_strides_type<xbuffer_adaptor<CP, O, A>> {
  // In bindings this mapping is called by reshape_view with an inner shape of
  // type xbuffer_adaptor. Since we cannot create a buffer adaptor holding data,
  // we map it to an std::vector.
  using type = std::vector<typename xbuffer_adaptor<CP, O, A>::value_type,
