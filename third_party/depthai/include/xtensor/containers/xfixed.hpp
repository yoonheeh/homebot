 ...
 * Note this function is only provided for homogeneity, and the shape & layout
 ...
 * for
 * ``xt::xshape<X, Y, Z>``. C arrays can be initialized with the initializer
...
inline xfixed_container<ET, S, L, SH, Tag>::xfixed_container(
  ...
  L == tmp ? nested_copy(m_storage.begin(), t)
           : nested_copy(this->template begin<tmp>(), t);
}
...
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class ST>
inline const auto &xfixed_container<ET, S, L, SH, Tag>::reshape(
...
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class ST>
inline const auto &xfixed_adaptor<ET, S, L, SH, Tag>::reshape(
    ST &&shape, layout_type layout) const {
  if (!(std::equal(shape.begin(), shape.end(), m_shape.begin()) &&
        shape.size() == m_shape.size() && layout == L)) {
    XTENSOR_THROW(
