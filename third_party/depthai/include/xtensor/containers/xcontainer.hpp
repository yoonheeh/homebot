...
/**
 * Returns a constant reference to the element at the specified position in the
 * container storage (as if it was one dimensional).
 * @param i index specifying the position in the storage.
 * Must be smaller than the number of elements in the container.
 */
template <class D>
...
template <class D>
inline auto xcontainer<D>::linear_begin() const noexcept
    -> const_linear_iterator {
  return storage().begin();
}

template <class D>
inline auto xcontainer<D>::linear_end() const noexcept
    -> const_linear_iterator {
  return storage().cend();
}

template <class D>
inline auto xcontainer<D>::linear_cbegin() const noexcept
    -> const_linear_iterator {
  return storage().cbegin();
}

template <class D>
inline auto xcontainer<D>::linear_cend() const noexcept
    -> const_linear_iterator {
  return storage().cend();
}
...
template <class D>
inline auto xcontainer<D>::linear_rbegin() const noexcept
    -> const_reverse_linear_iterator {
  return storage().rbegin();
}

template <class D>
inline auto xcontainer<D>::linear_rend() const noexcept
    -> const_reverse_linear_iterator {
  return storage().rend();
}

template <class D>
inline auto xcontainer<D>::linear_crbegin() const noexcept
    -> const_reverse_linear_iterator {
  return storage().crbegin();
}

template <class D>
inline auto xcontainer<D>::linear_crend() const noexcept
    -> const_reverse_linear_iterator {
...
inline bool xstrided_container<D>::is_contiguous() const noexcept {
  ...
  // Trailing zero strides are ignored because they indicate bradcasted
  // dimensions.
  if (m_layout == layout_type::row_major) {
    auto it = std::find_if_not(m_strides.rbegin(), m_strides.rend(), is_zero);
    // If the array has strides of zero, it is a constant, and therefore
    // contiguous.
...
inline void xstrided_container<D>::resize(S &&shape, layout_type l) {
  ...
  if (base_type::static_layout != layout_type::dynamic &&
    ...
    XTENSOR_THROW(std::runtime_error,
                  "Cannot change layout_type if template "
                  "parameter not layout_type::dynamic.");
  }
  m_layout = l;
...
inline void xstrided_container<D>::resize(S &&shape,
  ...
  if (base_type::static_layout != layout_type::dynamic) {
    XTENSOR_THROW(std::runtime_error,
                  "Cannot resize with custom strides when "
                  "layout() is != layout_type::dynamic.");
  }
  m_shape = xtl::forward_sequence<shape_type, S>(shape);
  m_strides = strides;
...
inline void xstrided_container<D>::reshape_impl(
  ...
  if (compute_size(shape) != this->size()) {
    XTENSOR_THROW(std::runtime_error,
                  "Cannot reshape with incorrect number of "
                  "elements. Do you mean to resize?");
  }
  if (D::static_layout == layout_type::dynamic &&
