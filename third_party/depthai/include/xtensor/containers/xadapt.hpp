...
/**
 * Constructs an xarray_adaptor of the given C array allocated on the stack,
 * with the specified shape and layout.
 *
 * @ingroup xt_xadapt
...
/**
 * Constructs an xtensor_adaptor of the given C array allocated on the stack,
 * with the specified shape and layout.
 *
 * @ingroup xt_xadapt
 * @param c_array the C array allocated on the stack
 ...
 *
 * from the given stl-like container or pointer, with the specified shape and
 * layout. If the adaptor is built from a pointer, it does not take its
 * ownership.
 *
 * @ingroup xt_xadapt
 ...
 * - an xtensor_adaptor if SC is an array type
 *
 * of the given dynamically allocated C array, with the specified shape and
 * layout.
 *
 * @ingroup xt_xadapt
 * @param pointer the pointer to the beginning of the dynamic array
...
template <layout_type L = XTENSOR_DEFAULT_LAYOUT, class P, class O, class SC,
          class A = detail::default_allocator_for_ptr_t<P>>
inline auto adapt(P &&pointer, typename A::size_type size, O ownership,
                  const SC &shape, layout_type l = L, const A &alloc = A());
...
template <class P, class O, class SC, class SS,
          class A = detail::default_allocator_for_ptr_t<P>>
inline auto adapt(P &&pointer, typename A::size_type size, O ownership,
                  SC &&shape, SS &&strides, const A &alloc = A());
 ...
 * - an xtensor_adaptor if SC is an array type
 *
 * of the given C array allocated on the stack, with the specified shape and
 * layout.
 *
 * @ingroup xt_xadapt
 * @param c_array the C array allocated on the stack
 ...
 *
 * xt::xtensor_pointer<double, 2> a = xt::adapt(data.data(), 4,
...
using xtensor_pointer =
    xtensor_adaptor<xbuffer_adaptor<xtl::closure_type_t<T *>, xt::no_ownership,
                                    detail::default_allocator_for_ptr_t<T>>,
                    N, L>;
 ...
 *
 * xt::xarray_pointer<int> a = xt::adapt(data.data(), data.size(),
 * xt::no_ownership(), shape);
 * @endcode
 *
 * @ingroup xt_xadapt
 ...
 * @tparam SC The shape container type (e.g. ``xt::svector<size_t>``). Default
 * matches xt::adapt(P&&, typename A::size_type, O, const SC&, layout_type,
 * const A& alloc)
 */
template <class T, layout_type L = XTENSOR_DEFAULT_LAYOUT,
          class SC = XTENSOR_DEFAULT_SHAPE_CONTAINER(
              T, std::allocator<std::size_t>, std::allocator<std::size_t>)>
using xarray_pointer =
    xarray_adaptor<xbuffer_adaptor<xtl::closure_type_t<T *>, xt::no_ownership,
                                   detail::default_allocator_for_ptr_t<T>>,
                   L, SC>;
}  // namespace xt
