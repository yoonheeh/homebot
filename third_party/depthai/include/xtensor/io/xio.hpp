  ...
  if (view.dimension() == 0) {
    ...
    for (; i != size_type(view.shape()[0] - 1); ++i) {
      if (edgeitems && size_type(view.shape()[0]) > (edgeitems * 2) &&
          i == edgeitems) {
        if (view.dimension() == 1 && line_lim != 0 &&
            elems_on_line >= line_lim) {
          out << " ...,";
      ...
      if ((view.dimension() == 1) &&
          !(line_lim != 0 && elems_on_line >= line_lim)) {
        out << ' ';
      } else if (view.dimension() > 1) {
        out << std::endl << indents;
      }
...
void recurser_run(F &fn, const E &e, xstrided_slice_vector &slices,
                  std::size_t lim = 0) {
  using size_type = typename E::size_type;
