 ...
 * ``xt::layout_type::any`` so that assigning a row major 1D container to
 * another row_major container becomes free.
 *
 * @ingroup xt_xshape
 */
template <layout_type L, class S>
struct select_layout {
  static constexpr std::ptrdiff_t static_dimension =
      xt::static_dimension<S>::value;
  static constexpr bool is_any = static_dimension != -1 &&
                                 static_dimension <= 1 &&
...
struct broadcast_fixed_shape_impl<std::index_sequence<JX...>, fixed_shape<I...>,
  ...
  static constexpr bool value = std::conjunction<broadcast_fixed_shape_cmp_impl<
...
template <class... S>
using only_array =
    std::conjunction<std::disjunction<is_array<S>, is_fixed<S>>...>;
...
using only_fixed = std::integral_constant<
    bool, std::disjunction<is_fixed<S>...>::value &&
              std::conjunction<
                  std::disjunction<is_fixed<S>, is_scalar_shape<S>>...>::value>;
