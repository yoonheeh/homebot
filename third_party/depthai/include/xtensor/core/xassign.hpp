...
struct use_strided_loop<xfunction<F, CT...>> {
  static constexpr bool value =
      std::conjunction<use_strided_loop<std::decay_t<CT>>...>::value;
};
 ...
 * cannot be implicitly instantiated from batches of scalar values. Making the
...
inline void xexpression_assigner_base<xtensor_expression_tag>::assign_data(
  ...
  if (linear_assign) {
    ...
    stepper_assigner<E1, E2, default_assignable_layout(E1::static_layout)>(de1,
                                                                           de2)
        .run();
  }
}
...
inline void linear_assigner<simd_assign>::run(E1 &e1, const E2 &e2) {
  ...
  constexpr bool needs_cast =
      has_assign_conversion<e2_value_type, e1_value_type>::value;
  ...
  if (size >= XTENSOR_TBB_THRESHOLD) {
    ...
    tbb::parallel_for(
        align_begin, align_end, simd_size,
        [&e1, &e2](size_t i) {
...
inline void linear_assigner<false>::run_impl(
  ...
  tbb::parallel_for(
      std::ptrdiff_t(0), static_cast<std::ptrdiff_t>(n),
      [&](std::ptrdiff_t i) {
...
#pragma omp parallel for default(none) shared(src, dst, n)
    for (std::ptrdiff_t i = std::ptrdiff_t(0);
...
struct idx_tools<layout_type::row_major> {
  ...
  static void nth_idx(size_t n, T &outer_index, const T &outer_shape) {
    ...
    for (size_type i = outer_shape.size(); i > 0; i--) {
      stride_sizes[i - 1] =
          (i == outer_shape.size()) ? 1 : stride_sizes[i] * outer_shape[i];
    }
...
template <bool possible = true, class E1, class E2,
          std::enable_if_t<!has_strides<E1>::value || !possible, bool> = true>
...
loop_sizes_t get_loop_sizes(const E1 &e1, const E2 &e2) {
  ...
  // Try to find a row-major scheme first, where the outer loop is on the first
  // N = `cut` dimensions, and the inner loop is
  is_row_major = true;
  ...
  return {inner_loop_size > 1, is_row_major, inner_loop_size,
          outer_loop_size,     cut,          e1.dimension()};
}
}  // namespace strided_assign_detail

template <bool simd>
template <class E1, class E2>
inline strided_assign_detail::loop_sizes_t
strided_loop_assigner<simd>::get_loop_sizes(E1 &e1, const E2 &e2) {
...
#pragma omp parallel for schedule(static) \
    firstprivate(first_step, fct_stepper, res_stepper, idx)
    for (std::size_t ox = 0; ox < outer_loop_size; ++ox) {
      if (first_step) {
        is_row_major
            ? strided_assign_detail::idx_tools<layout_type::row_major>::nth_idx(
                  ox, idx, max_shape)
            : strided_assign_detail::idx_tools<
                  layout_type::column_major>::nth_idx(ox, idx, max_shape);
      ...
      is_row_major
          ? strided_assign_detail::idx_tools<layout_type::row_major>::next_idx(
                idx, max_shape)
          : strided_assign_detail::idx_tools<
                layout_type::column_major>::next_idx(idx, max_shape);
  ...
  if (outer_loop_size > XTENSOR_TBB_THRESHOLD / inner_loop_size) {
    ...
    tbb::parallel_for(
        ...
        [&e1, &e2, is_row_major, step_dim, simd_size, simd_rest, &max_shape,
         &idx_ = idx](const tbb::blocked_range<size_t> &r) {
          auto idx = idx_;
          auto fct_stepper = e2.stepper_begin(e1.shape());
          auto res_stepper = e1.stepper_begin(e1.shape());
          std::size_t first_step = true;
          // #pragma omp parallel for schedule(static) firstprivate(first_step,
          // fct_stepper, res_stepper, idx)
          for (std::size_t ox = r.begin(); ox < r.end(); ++ox) {
            if (first_step) {
              is_row_major
                  ? strided_assign_detail::idx_tools<
                        layout_type::row_major>::nth_idx(ox, idx, max_shape)
                  : strided_assign_detail::idx_tools<
                        layout_type::column_major>::nth_idx(ox, idx, max_shape);
            ...
            is_row_major
                ? strided_assign_detail::idx_tools<
                      layout_type::row_major>::next_idx(idx, max_shape)
                : strided_assign_detail::idx_tools<
                      layout_type::column_major>::next_idx(idx, max_shape);
...
#if (defined(XTENSOR_USE_OPENMP) || defined(XTENSOR_USE_TBB)) && \
...
inline void strided_loop_assigner<true>::run(E1 &e1, const E2 &e2) {
  strided_assign_detail::loop_sizes_t loop_sizes =
      strided_loop_assigner<true>::get_loop_sizes(e1, e2);
  if (loop_sizes.can_do_strided_assign) {
    run(e1, e2, loop_sizes);
  } else {
    // trigger the fallback assigner
    stepper_assigner<E1, E2, default_assignable_layout(E1::static_layout)>(e1,
                                                                           e2)
        .run();
  }
}
...
inline void strided_loop_assigner<false>::run(E1 &e1, const E2 &e2) {
  // trigger the fallback assigner
  stepper_assigner<E1, E2, default_assignable_layout(E1::static_layout)>(e1, e2)
      .run();
}
}  // namespace xt
