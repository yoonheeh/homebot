...
template <class C>
struct has_linear_iterator<C,
                           void_t<decltype(std::declval<C>().linear_cbegin())>>
    : std::true_type {};
}  // namespace detail
...
void stepper_tools<layout_type::row_major>::increment_stepper(S &stepper,
  ...
  if (i == 0) {
    if (size != size_type(0)) {
      std::transform(shape.cbegin(), shape.cend() - 1, index.begin(),
...
void stepper_tools<layout_type::column_major>::increment_stepper(
  ...
  if (i == size) {
    if (size != size_type(0)) {
      std::transform(shape.cbegin() + 1, shape.cend(), index.begin() + 1,
                     [](const auto &v) { return v - 1; });
      index[0] = shape[0];
    }
...
inline void xindexed_stepper<C, is_const>::to_end(layout_type l) {
  ...
  std::transform(shape.cbegin(), shape.cend(), m_index.begin(),
                 [](const auto &v) { return v - 1; });
