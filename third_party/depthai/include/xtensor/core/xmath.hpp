 ...
 * @param es evaluation strategy of the reducer
 * @tparam T the value type used for internal computation. The default is
 * `E::value_type`. `T` is also used for determining the value type of the
 * result, which is the type of `T() * E::value_type()`. You can pass
...
template <class T = void, class E, class W, class X,
          class EVS = DEFAULT_STRATEGY_REDUCERS,
          XTL_REQUIRES(is_reducer_options<EVS>,
                       std::negation<xtl::is_integral<X>>)>
...
inline auto variance(E &&e, X &&axes, const D &ddof, EVS es = EVS()) {
  ...
  // fake keep_dims = 1
  // Since the inner_shape might have a reference semantic (e.g. xbuffer_adaptor
  // in bindings) We need to map it to another type before modifying it. We
  // pragmatically abuse `get_strides_t`
  using tmp_shape_t = get_strides_t<typename std::decay_t<E>::shape_type>;
  tmp_shape_t keep_dim_shape =
      xtl::forward_sequence<tmp_shape_t, decltype(e.shape())>(e.shape());
 ...
 * This is not the same as counting NaNs as zero, since excluding NaNs changes
 * the number of elements considered in the statistic.
 * @param e an \ref xexpression
 * @param axes the axes along which the mean is computed (optional)
 * @param es the evaluation strategy (optional)
 * @tparam T the result type. The default is `E::value_type`.
...
auto calculate_interval(
    E2 &&period,
    typename std::enable_if<std::is_floating_point<E1>::value, E1>::type * =
        0) {
  auto interval_high = 0.5 * period;
...
/**
 * @ingroup basic_functions
 * @brief Unwrap by taking the complement of large deltas with respect to the
 * period
 ...
 *     To have an effect different from the default, use `discontinuity > period
 * / 2`.
 * @param axis Axis along which unwrap will operate, default: the last axis.
 * @param period Size of the range over which the input wraps. Default: \f$ 2
 * \pi \f$.
 */

template <class E1, class E2 = xt::placeholders::xtuph, class E3 = double>
inline auto unwrap(E1 &&p, E2 discontinuity = xnone(), std::ptrdiff_t axis = -1,
  ...
  auto ddmod =
      xt::eval(xt::fmod(xt::fmod(dd - interval_low, period) + period, period) +
               interval_low);
  ...
  E1 up(p);
  strided_view(up, slice) =
      strided_view(p, slice) +
...
inline auto convolve(E1 &&a, E2 &&v, E3 mode) {
  ...
  if (a.size() < v.size()) {
    return detail::convolve_impl(std::forward<E2>(v), std::forward<E1>(a),
                                 mode);
  } else {
    return detail::convolve_impl(std::forward<E1>(a), std::forward<E2>(v),
                                 mode);
  }
}
}  // namespace xt
