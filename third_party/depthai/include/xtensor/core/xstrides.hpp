...
/**
 * @defgroup xt_xstrides Support functions swich between array indices and flat
 * indices
 */
 ...
 *
 * Given ``m`` arguments, and dimension ``n``of the array (``n ==
 * strides.size()``).
 *
 *  -   If ``m == n``, the index is
 *      ``strides[0] * index[0] + ... + strides[n - 1] * index[n - 1]``.
 *
 *  -   If ``m < n`` and the last argument is ``xt::missing`` the indices are
 * zero-padded at the end to match the dimension of the array. The index is then
 *      ``strides[0] * index[0] + ... + strides[m - 1] * index[m - 1]``.
 *
 *  -   If ``m < n`` (and the last argument is not ``xt::missing``), the index
 * is
 *      ``strides[n - m - 1] * index[0] + ... + strides[n - 1] * index[m - 1]``.
 *
 *  -   If ``m > n``, then the first ``m - n`` arguments are ignored. The index
 * is then
 ...
 * For example if the shape is ``(3, 4)`` and the index is ``(3, -4)`` the
 * result is ``(0, 0)``.
 *
 * @ingroup xt_xstrides
 * @param shape Shape of the array.
...
inline return_type compute_stride_impl(layout_type layout, const S &shape,
                                       T axis, D default_stride) {
  if (layout == layout_type::row_major) {
    return std::accumulate(shape.cbegin() + axis + 1, shape.cend(),
                           static_cast<return_type>(1),
                           std::multiplies<return_type>());
  }
  if (layout == layout_type::column_major) {
    return std::accumulate(shape.cbegin(), shape.cbegin() + axis,
                           static_cast<return_type>(1),
                           std::multiplies<return_type>());
  }
...
enum class stride_type {
  internal = 0,  ///< As used internally (with `stride(axis) == 0` if
                 ///< `shape(axis) == 1`)
 ...
 */
template <class E>
inline auto strides(const E &e,
                    stride_type type = stride_type::normal) noexcept {
  ...
  for (std::size_t i = 0; i < ret.size(); ++i) {
    if (shape[i] == 1) {
      ret[i] = detail::compute_stride_impl<return_type>(e.layout(), shape, i,
                                                        ret[i]);
    }
  }
 ...
 */
template <class E>
inline auto strides(const E &e, std::size_t axis,
                    stride_type type = stride_type::normal) noexcept {
  using strides_type = typename E::strides_type;
  using return_type = typename strides_type::value_type;
  ...
  if (ret == 0) {
    if (e.shape(axis) == 1) {
      ret = detail::compute_stride_impl<return_type>(e.layout(), e.shape(),
                                                     axis, ret);
    }
  }
...
inline auto raw_data_offset(const S &strides, Arg arg, Args... args) noexcept {
  return static_cast<std::ptrdiff_t>(arg) * strides[dim] +
         raw_data_offset<dim + 1>(strides, args...);
}
