...
class xmultiindex_iterator {
 public:
  ...
  xmultiindex_iterator(B &&begin, E &&end, C &&current,
  ...
  shape_type &operator*() { return m_current; }

  const shape_type &operator*() const { return m_current; }

  bool operator==(const self_type &rhs) const {
    return m_linear_index == rhs.m_linear_index;
  }

  bool operator!=(const self_type &rhs) const { return !this->operator==(rhs); }
...
auto multiindex_iterator_begin(B &&roi_begin, E &&roi_end) {
  ...
  return xmultiindex_iterator<S>(std::forward<B>(roi_begin),
                                 std::forward<E>(roi_end), std::move(current),
                                 0);
}
