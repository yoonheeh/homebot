 ...
 *
 * @return returns a ``xt::xchunked_array<xt::xarray<T>>`` with the given shape,
 * chunk shape and memory layout.
 */
 ...
 * Creates an in-memory chunked array.
 *
 * This function returns a ``xt::xchunked_array<xt::xarray<T>>`` initialized
 * from an expression.
 *
 * @ingroup xt_xchunked_array
 *
 ...
 * XTENSOR_DEFAULT_LAYOUT)
 *
 * @return returns a ``xt::xchunked_array<xt::xarray<T>>`` from the given
 * expression, with the given chunk shape and memory layout.
 */
 ...
 *
 * @return returns a ``xt::xchunked_array<xt::xarray<T>>`` from the given
 * expression, with the expression's chunk shape and the given memory layout.
 */
