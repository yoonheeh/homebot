...
struct sum_functor : public simple_functor_base {
  using value_type = typename std::decay_t<decltype(xt::sum<T_I>(
      std::declval<xarray<T_E>>()))>::value_type;
...
struct amin_functor : public simple_functor_base {
  using value_type = typename std::decay_t<decltype(xt::amin<T_I>(
      std::declval<xarray<T_E>>()))>::value_type;
...
struct amax_functor : public simple_functor_base {
  using value_type = typename std::decay_t<decltype(xt::amax<T_I>(
      std::declval<xarray<T_E>>()))>::value_type;
...
template <class T_E, class T_I = void>
struct mean_functor {
  using value_type = typename std::decay_t<decltype(xt::mean<T_I>(
      std::declval<xarray<T_E>>()))>::value_type;
  ...
  auto merge(const BR &block_result, bool first, E &result,
             empty_reduction_variable &) const {
    if (first) {
      xt::noalias(result) = block_result;
    } else {
      xt::noalias(result) += block_result;
  ...
  void finalize(const empty_reduction_variable &, E &results,
                const R &reducer) const {
...
struct variance_functor {
  using value_type = typename std::decay_t<decltype(xt::variance<T_I>(
      std::declval<xarray<T_E>>()))>::value_type;
  ...
  auto compute(const E &input, const A &axes, const O &options) const {
    ...
    return std::make_tuple(xt::variance<value_type>(input, axes, options),
                           xt::mean<value_type>(input, axes, options), weight);
  }
...
struct norm_l0_functor : public simple_functor_base {
  using value_type = typename std::decay_t<decltype(xt::norm_l0(
      std::declval<xarray<T_E>>()))>::value_type;

  template <class E, class A, class O>
  auto compute(const E &input, const A &axes, const O &options) const {
    return xt::sum<value_type>(
        xt::not_equal(input, xt::zeros<T_E>(input.shape())), axes, options);
  }
...
struct norm_l1_functor : public simple_functor_base {
  using value_type = typename std::decay_t<decltype(xt::norm_l1(
      std::declval<xarray<T_E>>()))>::value_type;
...
struct norm_l2_functor {
  using value_type = typename std::decay_t<decltype(xt::norm_l2(
      std::declval<xarray<T_E>>()))>::value_type;
...
struct norm_sq_functor : public simple_functor_base {
  using value_type = typename std::decay_t<decltype(xt::norm_sq(
      std::declval<xarray<T_E>>()))>::value_type;
...
struct norm_linf_functor : public simple_functor_base {
  using value_type = typename std::decay_t<decltype(xt::norm_linf(
      std::declval<xarray<T_E>>()))>::value_type;
...
class norm_lp_to_p_functor {
 public:
  using value_type = typename std::decay_t<decltype(xt::norm_lp_to_p(
      std::declval<xarray<T_E>>(), 1.0))>::value_type;

  norm_lp_to_p_functor(double p) : m_p(p) {}
...
class norm_lp_functor {
 public:
  norm_lp_functor(double p) : m_p(p) {}

  using value_type = typename std::decay_t<decltype(xt::norm_lp(
      std::declval<xarray<T_E>>(), 1.0))>::value_type;
