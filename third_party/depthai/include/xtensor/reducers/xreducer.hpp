...
struct reducer_options {
  ...
  reducer_options(const T &tpl) {
    if constexpr (initial_val_idx != std::tuple_size<T>::value) {
      initial_value = std::get < initial_val_idx != std::tuple_size<T>::value
                          ? initial_val_idx
                          : 0 > (tpl).value();
    }
  }
  ...
  static constexpr bool has_initial_value =
...
template <class F, class E, class R,
          XTL_REQUIRES(std::negation<std::is_convertible<
...
inline auto reduce_immediate(F &&f, E &&e, X &&axes, O &&raw_options) {
  ...
  // i pointing to the sequence and any non-negative integer n such that i + n
  // is a valid iterator pointing to an element of the sequence, comp(*(i + n),
  // *i) == false. Therefore less is required to detect duplicates.
...
class xreducer : public xsharable_expression<xreducer<F, CT, X, O>>,
 ...
 public:
  ...
  xreducer_functors_type functors() const {
    return xreducer_functors_type(m_reduce, m_init,
                                  m_merge);  // TODO: understand why
                                             // make_xreducer_functor is
                                             // throwing an error
  }
...
template <class F, class E, class X, class EVS = DEFAULT_STRATEGY_REDUCERS,
          XTL_REQUIRES(std::negation<is_reducer_options<X>>,
                       std::negation<detail::is_xreducer_functors<F>>)>
inline auto reduce(F &&f, E &&e, X &&axes, EVS &&options = EVS()) {
...
struct in {
  static constexpr bool value =
      std::disjunction<std::integral_constant<bool, X == I>...>::value;
};
...
inline auto xreducer_stepper<F, CT, X, O>::aggregate_impl(size_type dim,
  ...
  if (dim != m_reducer->m_axes.size() - 1) {
    ...
    res = m_reducer->m_reduce(static_cast<reference>(m_reducer->m_init()),
                              *m_stepper);
    for (size_type i = 1; i != size; ++i) {
      m_stepper.step(index);
