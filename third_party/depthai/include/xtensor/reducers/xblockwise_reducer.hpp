...
class xblockwise_reducer {
 ...
 private:
  ...
  using input_const_chunked_iterator_type =
      typename input_chunked_view_type::const_chunk_iterator;
  using input_chunk_range_type =
      std::array<xmultiindex_iterator<input_chunk_index_type>, 2>;
  ...
  input_const_chunked_iterator_type get_input_chunk_iter(
      input_chunk_index_type input_chunk_index) const;
  void init_shapes();
...
template <class CT, class F, class X, class O>
template <class E, class BS, class XX, class OO, class FF>
xblockwise_reducer<CT, F, X, O>::xblockwise_reducer(E &&e, BS &&block_shape,
                                                    XX &&axes, OO &&options,
                                                    FF &&functor)
    : m_e(std::forward<E>(e)),
...
template <class CT, class F, class X, class O>
inline auto xblockwise_reducer<CT, F, X, O>::input_shape() const
    -> const input_shape_type & {
  return m_e.shape();
}
...
template <class CT, class F, class X, class O>
inline auto xblockwise_reducer<CT, F, X, O>::shape() const
    -> const shape_type & {
  return m_result_shape;
}

template <class CT, class F, class X, class O>
inline auto xblockwise_reducer<CT, F, X, O>::chunk_shape() const
    -> const chunk_shape_type & {
...
inline void xblockwise_reducer<CT, F, X, O>::assign_to(R &result) const {
  ...
  for (auto chunk_iter = result_chunked_view.chunk_begin();
       chunk_iter != result_chunked_view.chunk_end(); ++chunk_iter) {
    assign_to_chunk(chunk_iter);
  }
}

template <class CT, class F, class X, class O>
auto xblockwise_reducer<CT, F, X, O>::get_input_chunk_iter(
    input_chunk_index_type input_chunk_index) const
  ...
  return input_const_chunked_iterator_type(
      m_e_chunked_view, std::move(input_chunk_index), chunk_linear_index);
}

template <class CT, class F, class X, class O>
template <class CI>
void xblockwise_reducer<CT, F, X, O>::assign_to_chunk(
    CI &result_chunk_iter) const {
  ...
  auto first = true;
  // std::for_each(std::get<0>(range), std::get<1>(range), [&](auto &&
  // input_chunk_index)
...
template <class CT, class F, class X, class O>
template <class CI>
auto xblockwise_reducer<CT, F, X, O>::compute_input_chunk_range(
    CI &result_chunk_iter) const -> input_chunk_range_type {
  auto input_chunks_begin = xtl::make_sequence<input_chunk_index_type>(
      m_e_chunked_view.dimension(), 0);
  auto input_chunks_end =
      xtl::make_sequence<input_chunk_index_type>(m_e_chunked_view.dimension());
  ...
  std::copy(m_e_chunked_view.grid_shape().begin(),
            m_e_chunked_view.grid_shape().end(), input_chunks_end.begin());

  const auto &chunk_index = result_chunk_iter.chunk_index();
  for (std::size_t result_ax_index = 0; result_ax_index < m_result_shape.size();
       ++result_ax_index) {
  ...
  return input_chunk_range_type{
      multiindex_iterator_begin<input_chunk_index_type>(input_chunks_begin,
                                                        input_chunks_end),
      multiindex_iterator_end<input_chunk_index_type>(input_chunks_begin,
                                                      input_chunks_end)};
}
...
inline auto blockwise_reducer(E &&e, CS &&chunk_shape, A &&axes,
                              O &&raw_options, FF &&functor) {
  ...
  return xblockwise_reducer<closure_type, functor_type, axes_type, O>(
      std::forward<E>(e), std::forward<CS>(chunk_shape), std::forward<A>(axes),
      std::forward<O>(raw_options), std::forward<FF>(functor));
}

namespace blockwise {

#define XTENSOR_BLOCKWISE_REDUCER_FUNC(FNAME, FUNCTOR)                       \
  template <class T = void, class E, class BS, class X,                      \
            class O = DEFAULT_STRATEGY_REDUCERS,                             \
            XTL_REQUIRES(std::negation<is_reducer_options<X>>,               \
                         std::negation<xtl::is_integral<std::decay_t<X>>>)>  \
  auto FNAME(E &&e, BS &&block_shape, X &&axes, O options = O()) {           \
    using input_expression_type = std::decay_t<E>;                           \
    using functor_type =                                                     \
        FUNCTOR<typename input_expression_type::value_type, T>;              \
    return blockwise_reducer(                                                \
        std::forward<E>(e), std::forward<BS>(block_shape),                   \
        std::forward<X>(axes), std::forward<O>(options), functor_type());    \
  }                                                                          \
  template <class T = void, class E, class BS, class X,                      \
            class O = DEFAULT_STRATEGY_REDUCERS,                             \
  ...
  auto FNAME(E &&e, BS &&block_shape, X axis, O options = O()) {             \
    ...
    using functor_type =                                                     \
        FUNCTOR<typename input_expression_type::value_type, T>;              \
    return blockwise_reducer(std::forward<E>(e),                             \
                             std::forward<BS>(block_shape), axes,            \
                             std::forward<O>(options), functor_type());      \
  }                                                                          \
  template <class T = void, class E, class BS,                               \
            class O = DEFAULT_STRATEGY_REDUCERS,                             \
            XTL_REQUIRES(is_reducer_options<O>,                              \
                         std::negation<xtl::is_integral<std::decay_t<O>>>)>  \
  auto FNAME(E &&e, BS &&block_shape, O options = O()) {                     \
    using input_expression_type = std::decay_t<E>;                           \
    using axes_type =                                                        \
        filter_fixed_shape_t<typename input_expression_type::shape_type>;    \
    ...
    using functor_type =                                                     \
        FUNCTOR<typename input_expression_type::value_type, T>;              \
    return blockwise_reducer(std::forward<E>(e),                             \
                             std::forward<BS>(block_shape), std::move(axes), \
                             std::forward<O>(options), functor_type());      \
  }                                                                          \
  template <class T = void, class E, class BS, class I, std::size_t N,       \
            class O = DEFAULT_STRATEGY_REDUCERS>                             \
  auto FNAME(E &&e, BS &&block_shape, const I(&axes)[N], O options = O()) {  \
    ...
    return blockwise_reducer(std::forward<E>(e),                             \
                             std::forward<BS>(block_shape), std::move(ax),   \
                             std::forward<O>(options), functor_type());      \
  }
...
XTENSOR_BLOCKWISE_REDUCER_FUNC(variance,
                               xt::detail::blockwise::variance_functor)
...
#define XTENSOR_BLOCKWISE_NORM_REDUCER_FUNC(FNAME, FUNCTOR)                   \
  ...
  template <class E, class BS, class I, std::size_t N,                        \
            class O = DEFAULT_STRATEGY_REDUCERS>                              \
  auto FNAME(E &&e, BS &&block_shape, const I(&axes)[N], O options = O()) {   \
    using input_expression_type = std::decay_t<E>;                            \
    using functor_type = FUNCTOR<typename input_expression_type::value_type>; \
...
XTENSOR_BLOCKWISE_NORM_REDUCER_FUNC(norm_l0,
                                    xt::detail::blockwise::norm_l0_functor)
XTENSOR_BLOCKWISE_NORM_REDUCER_FUNC(norm_l1,
                                    xt::detail::blockwise::norm_l1_functor)
XTENSOR_BLOCKWISE_NORM_REDUCER_FUNC(norm_l2,
                                    xt::detail::blockwise::norm_l2_functor)
XTENSOR_BLOCKWISE_NORM_REDUCER_FUNC(norm_sq,
                                    xt::detail::blockwise::norm_sq_functor)
XTENSOR_BLOCKWISE_NORM_REDUCER_FUNC(norm_linf,
                                    xt::detail::blockwise::norm_linf_functor)
...
#define XTENSOR_BLOCKWISE_NORM_REDUCER_FUNC(FNAME, FUNCTOR)                   \
  ...
  auto FNAME(E &&e, BS &&block_shape, double p, X &&axes, O options = O()) {  \
    ...
    return blockwise_reducer(                                                 \
        std::forward<E>(e), std::forward<BS>(block_shape),                    \
        std::forward<X>(axes), std::forward<O>(options), functor_type(p));    \
  }                                                                           \
  template <class E, class BS, class X, class O = DEFAULT_STRATEGY_REDUCERS,  \
            XTL_REQUIRES(xtl::is_integral<std::decay_t<X>>)>                  \
  auto FNAME(E &&e, BS &&block_shape, double p, X axis, O options = O()) {    \
    ...
    return blockwise_reducer(std::forward<E>(e),                              \
                             std::forward<BS>(block_shape), axes,             \
                             std::forward<O>(options), functor_type(p));      \
  }                                                                           \
  template <class E, class BS, class O = DEFAULT_STRATEGY_REDUCERS,           \
            XTL_REQUIRES(is_reducer_options<O>,                               \
                         std::negation<xtl::is_integral<std::decay_t<O>>>)>   \
  ...
  template <class E, class BS, class I, std::size_t N,                        \
            class O = DEFAULT_STRATEGY_REDUCERS>                              \
  auto FNAME(E &&e, BS &&block_shape, double p, const I(&axes)[N],            \
             O options = O()) {                                               \
    using input_expression_type = std::decay_t<E>;                            \
    using functor_type = FUNCTOR<typename input_expression_type::value_type>; \
    using axes_type = std::array<std::size_t, N>;                             \
...
XTENSOR_BLOCKWISE_NORM_REDUCER_FUNC(
    norm_lp_to_p, xt::detail::blockwise::norm_lp_to_p_functor);
XTENSOR_BLOCKWISE_NORM_REDUCER_FUNC(norm_lp,
                                    xt::detail::blockwise::norm_lp_functor);
