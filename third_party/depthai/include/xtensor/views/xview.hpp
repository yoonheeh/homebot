...
struct is_strided_view
    : std::integral_constant<
          bool,
          std::conjunction<has_data_interface<E>,
...
class xview
 ...
 public:
  ...
  static constexpr bool has_trivial_strides =
      is_contiguous_view && !std::disjunction<detail::is_xrange<S>...>::value;
...
inline layout_type xview<CT, S...>::layout() const noexcept {
  if constexpr (is_strided_view) {
    if (static_layout != layout_type::dynamic) {
      ...
      bool strides_match =
          do_strides_match(shape(), strides(), m_e.layout(), true);
...
inline auto xview<CT, S...>::operator&()
    & -> xtl::xclosure_pointer<self_type &> {
...
class row_impl {
 ...
 private:
  ...
  inline static void check_dimension(const std::array<T, N> &) {
    static_assert(N == 2,
                  "A row can only be accessed on an expression with "
                  "exact two dimensions");
  }
};
...
class column_impl {
 ...
 private:
  ...
  inline static void check_dimension(const std::array<T, N> &) {
    static_assert(N == 2,
                  "A column can only be accessed on an expression with "
                  "exact two dimensions");
  }
};
}  // namespace detail
