...
class xfunctor_applier_base : private xaccessible<D> {
 public:
  ...
  using reverse_linear_iterator =
      xfunctor_iterator<functor_type,
                        typename xexpression_type::reverse_linear_iterator>;
 ...
 *
 * The xt::xfunctor_view class is an expression addressing its elements by
 * applying a functor to the corresponding element of an underlying expression.
 ...
 * For a similar feature with a container semantics, one can use
 * xt::xfunctor_adaptor.
 *
 * xt::xfunctor_view is not meant to be used directly, but through helper
 * functions such as xt::real or xt::imag.
 *
...
/**
 * Adapt a container with a functor, forwarding methods such as resize /
 * reshape.
 *
 * xt::xfunctor_adaptor has a container semantics and can only be used with
 * containers. For a similar feature with a view semantics, one can use
 * xt::xfunctor_view.
 *
 * @ingroup xt_xfunctor_view
 * @tparam F the functor type to be applied to the elements of specified
...
/**
 * Constructs an xfunctor_applier_base expression wrappering the specified
 * xt::xexpression.
 *
...
inline auto xfunctor_applier_base<D>::crbegin() const noexcept {
  return xfunctor_iterator<const functor_type,
                           decltype(m_e.template crbegin<L>())>(
...
template <class D>
template <class S, layout_type L>
inline auto xfunctor_applier_base<D>::crbegin(const S & /*shape*/)
...
template <class D>
inline auto xfunctor_applier_base<D>::linear_begin() noexcept
    -> linear_iterator {
...
template <class D>
inline auto xfunctor_applier_base<D>::linear_begin() const noexcept
    -> const_linear_iterator {
  return const_linear_iterator(m_e.linear_begin(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::linear_end() const noexcept
    -> const_linear_iterator {
  return const_linear_iterator(m_e.linear_end(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::linear_cbegin() const noexcept
    -> const_linear_iterator {
  return const_linear_iterator(m_e.linear_cbegin(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::linear_cend() const noexcept
    -> const_linear_iterator {
  return const_linear_iterator(m_e.linear_cend(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::linear_rbegin() noexcept
    -> reverse_linear_iterator {
  return reverse_linear_iterator(m_e.linear_rbegin(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::linear_rend() noexcept
    -> reverse_linear_iterator {
  return reverse_linear_iterator(m_e.linear_rend(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::linear_rbegin() const noexcept
    -> const_reverse_linear_iterator {
  return const_reverse_linear_iterator(m_e.linear_rbegin(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::linear_rend() const noexcept
    -> const_reverse_linear_iterator {
  return const_reverse_linear_iterator(m_e.linear_rend(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::linear_crbegin() const noexcept
    -> const_reverse_linear_iterator {
  return const_reverse_linear_iterator(m_e.linear_crbegin(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::linear_crend() const noexcept
    -> const_reverse_linear_iterator {
