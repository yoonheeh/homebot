 ...
 *
 * Because sorting functions need to access the tensor data repeatedly, they
 * evaluate their input and may allocate temporaries.
 */
...
inline std::ptrdiff_t get_secondary_stride(const E &ev) {
  if (ev.layout() == layout_type::row_major) {
    return adjust_secondary_stride(ev.strides()[ev.dimension() - 2],
                                   *(ev.shape().end() - 1));
  }
...
inline std::size_t leading_axis_n_iters(const E &ev) {
  if (ev.layout() == layout_type::row_major) {
    return std::accumulate(ev.shape().begin(), ev.shape().end() - 1,
                           std::size_t(1), std::multiplies<>());
  }
  return std::accumulate(ev.shape().begin() + 1, ev.shape().end(),
                         std::size_t(1), std::multiplies<>());
}
...
inline void call_over_leading_axis(E1 &e1, E2 &e2, F &&fct) {
  ...
  for (; (iter1 != end1) && (iter2 != end2);
       iter1 += secondary_stride1, iter2 += secondary_stride2) {
...
inline R map_axis(const E &e, std::ptrdiff_t axis, F &&lambda) {
  ...
  std::tie(permutation, reverse_permutation) =
      get_permutations(e.dimension(), ax, e.layout());
...
enum class sorting_method {
  /**
   *  Faster method but with no guarantee on preservation of order of equal
   * elements https://en.cppreference.com/w/cpp/algorithm/sort.
   */
  quick,
  /**
   *  Slower method but with guarantee on preservation of order of equal
   * elements https://en.cppreference.com/w/cpp/algorithm/stable_sort.
   */
  stable,
};
...
inline void argsort_iter(ConstRandomIt data_begin, ConstRandomIt data_end,
                         RandomIt idx_begin, RandomIt idx_end, Compare comp,
                         Method method) {
  XTENSOR_ASSERT(std::distance(data_begin, data_end) >= 0);
  XTENSOR_ASSERT(std::distance(idx_begin, idx_end) ==
  ...
  switch (method) {
    case (sorting_method::quick): {
      std::sort(idx_begin, idx_end, [&](const auto i, const auto j) {
        return comp(*(data_begin + i), *(data_begin + j));
      });
    }
    case (sorting_method::stable): {
      std::stable_sort(idx_begin, idx_end, [&](const auto i, const auto j) {
        return comp(*(data_begin + i), *(data_begin + j));
      });
    }
  }
...
inline void argsort_iter(ConstRandomIt data_begin, ConstRandomIt data_end,
                         RandomIt idx_begin, RandomIt idx_end, Method method) {
  return argsort_iter(
      std::move(data_begin), std::move(data_end), std::move(idx_begin),
      std::move(idx_end),
      [](const auto &x, const auto &y) -> bool { return x < y; }, method);
}
...
inline auto argsort(const xexpression<E> &e, std::ptrdiff_t axis = -1,
  ...
  const auto argsort = [&method](auto res_begin, auto res_end, auto ev_begin,
                                 auto ev_end) {
...
inline void partition_iter(RandomIt data_begin, RandomIt data_end,
                           Iter kth_begin, Iter kth_end, Compare comp) {
  XTENSOR_ASSERT(std::distance(data_begin, data_end) >= 0);
  ...
  for (; kth_begin != kth_end; ++kth_begin) {
    std::nth_element(data_begin, data_begin + *kth_begin, data_begin + k_last,
                     std::move(comp));
    k_last = *kth_begin;
  }
}
...
inline void partition_iter(RandomIt data_begin, RandomIt data_end,
                           Iter kth_begin, Iter kth_end) {
  return partition_iter(
      std::move(data_begin), std::move(data_end), std::move(kth_begin),
      std::move(kth_end),
      [](const auto &x, const auto &y) -> bool { return x < y; });
}
}  // namespace detail
...
inline R partition(const xexpression<E> &e, C kth_container,
  ...
  std::copy(de.linear_cbegin(), de.linear_cend(),
            ev.linear_begin());  // flatten

  detail::partition_iter(ev.linear_begin(), ev.linear_end(),
                         kth_container.rbegin(), kth_container.rend());
...
template <class E, class C,
          class = std::enable_if_t<!xtl::is_integral<C>::value, int>>
inline auto partition(const xexpression<E> &e, C kth_container,
  ...
  return detail::map_axis<eval_type>(
      e.derived_cast(), axis, [&kth_container](auto begin, auto end) {
        detail::partition_iter(begin, end, kth_container.rbegin(),
                               kth_container.rend());
      });
}
...
template <class E, class C,
          ...
          class = std::enable_if_t<!xtl::is_integral<C>::value, int>>
inline R argpartition(const xexpression<E> &e, C kth_container,
                      placeholders::xtuph) {
  using eval_type = typename detail::sort_eval_type<E>::type;
  using result_type =
  ...
  detail::partition_iter(
      res.linear_begin(), res.linear_end(), kth_container.rbegin(),
      kth_container.rend(),
      [&de](std::size_t a, std::size_t b) { return de[a] < de[b]; });
...
template <class E, class C,
          class = std::enable_if_t<!xtl::is_integral<C>::value, int>>
inline auto argpartition(const xexpression<E> &e, C kth_container,
                         std::ptrdiff_t axis = -1) {
  using eval_type = typename detail::sort_eval_type<E>::type;
  using result_type = typename detail::argsort_result_type<eval_type>::type;
  ...
  if (de.dimension() == 1) {
    return argpartition<E, C, result_type>(e, std::forward<C>(kth_container),
                                           xnone());
  }

  std::sort(kth_container.begin(), kth_container.end());
  const auto argpartition_w_kth = [&kth_container](auto res_begin, auto res_end,
                                                   auto ev_begin,
                                                   auto /*ev_end*/) {
    std::iota(res_begin, res_end, 0);
    detail::partition_iter(res_begin, res_end, kth_container.rbegin(),
                           kth_container.rend(),
...
inline void select_indices_impl(const S &shape, const I &indices,
                                std::size_t axis, std::size_t current_dim,
                                const K &current_index, O &out) {
...
inline auto select_indices(const S &shape, const I &indices, std::size_t axis) {
  ...
  select_indices_impl(shape, indices, axis, 0,
                      xtl::make_sequence<index_type>(shape.size()), out);
  return out;
}
...
inline auto quantile_kth_gamma(std::size_t n, const P &probas, T alpha,
                               T beta) {
...
inline auto unsqueeze_shape(const S &shape, std::size_t axis) {
  ...
  auto new_shape =
      xtl::forward_sequence<xt::svector<std::size_t>, decltype(shape)>(shape);
 ...
 *
 * In a sorted array represneting a distribution of numbers, the quantile of a
 * probability ``p`` is the the cut value ``q`` such that a fraction ``p`` of
 * the distribution is lesser or equal to ``q``. When the cutpoint falls between
 * two elemnts of the sample distribution, a interpolation is computed using the
 * @p alpha and @p beta coefficients, as descripted in (Hyndman and Fan, 1996).
 *
 * The algorithm partially sorts entries in a copy along the @p axis axis.
 *
 * @ingroup xt_xsort
 * @param e Expression containing the distribution over which the quantiles are
 * computed.
 * @param probas An list of probability associated with each desired quantiles.
 *        All elements must be in the range ``[0, 1]``.
 * @param axis The dimension in which to compute the quantiles, *i.e* the axis
...
inline auto quantile(E &&e, const P &probas, std::ptrdiff_t axis, T alpha,
                     T beta) {
  ...
  auto kth_gamma =
      detail::quantile_kth_gamma<T, id_t, P>(n, probas, alpha, beta);
  ...
  auto e_kth = detail::fancy_indexing(std::move(e_partition),
                                      std::move(kth_gamma.first), ax);
  ...
  auto gm1_g_reshaped =
      reshape_view(std::move(kth_gamma.second), std::move(gm1_g_shape));
...
inline auto quantile(E &&e, const T (&probas)[N], std::ptrdiff_t axis, T alpha,
                     T beta) {
 ...
 *
 * The quantiles are computed over the whole expression, as if flatten in a
 * one-dimensional expression.
 *
 * @ingroup xt_xsort
 * @see xt::quantile(E&& e, P const& probas, std::ptrdiff_t axis, T alpha, T
 * beta)
 */
template <class T = double, class E, class P>
 ...
 *
 * Predefined methods for interpolating quantiles, as defined in (Hyndman and
 * Fan, 1996).
 *
 * @ingroup xt_xsort
 * @see (Hyndman and Fan, 1996) R. J. Hyndman and Y. Fan,
 *      "Sample quantiles in statistical packages", The American Statistician,
 *      50(4), pp. 361-365, 1996
 * @see xt::quantile(E&& e, P const& probas, std::ptrdiff_t axis,
 * xt::quantile_method method)
 */
 ...
 * Compute quantiles over the given axis.
 *
 * The function takes the name of a predefined method to compute to interpolate
 * between values.
 *
 * @ingroup xt_xsort
 ...
 */
template <class T = double, class E, class P>
inline auto quantile(E &&e, const P &probas, std::ptrdiff_t axis,
...
// Static proba array overload
template <class T = double, class E, std::size_t N>
inline auto quantile(E &&e, const T (&probas)[N], std::ptrdiff_t axis,
                     quantile_method method = quantile_method::linear) {
 ...
 *
 * The quantiles are computed over the whole expression, as if flatten in a
 * one-dimensional expression. The function takes the name of a predefined
 * method to compute to interpolate between values.
 *
 * @ingroup xt_xsort
 * @see xt::quantile_method
 * @see xt::quantile(E&& e, P const& probas, std::ptrdiff_t axis,
 ...
 */
template <class T = double, class E, class P>
inline auto quantile(E &&e, const P &probas,
                     quantile_method method = quantile_method::linear) {
...
// Static proba array overload
template <class T = double, class E, std::size_t N>
inline auto quantile(E &&e, const T (&probas)[N],
                     quantile_method method = quantile_method::linear) {
