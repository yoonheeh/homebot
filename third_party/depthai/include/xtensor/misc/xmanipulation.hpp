...
namespace detail {
template <class S>
inline S swapaxes_perm(std::size_t dim, std::ptrdiff_t axis1,
                       std::ptrdiff_t axis2) {
...
inline auto swapaxes(E &&e, std::ptrdiff_t axis1, std::ptrdiff_t axis2) {
  ...
  return transpose(std::forward<E>(e),
                   detail::swapaxes_perm<strides_t>(dim, axis1, axis2));
}
...
namespace detail {
template <class S>
inline S moveaxis_perm(std::size_t dim, std::ptrdiff_t src,
                       std::ptrdiff_t dest) {
  using id_t = typename S::value_type;
...
inline auto moveaxis(E &&e, std::ptrdiff_t src, std::ptrdiff_t dest) {
  ...
  using strides_t = get_strides_t<typename std::decay_t<E>::shape_type>;
  return xt::transpose(std::forward<E>(e), detail::moveaxis_perm<strides_t>(
                                               e.dimension(), src, dest));
}
 ...
 *
 * No copy is made.
 * This method is equivalent to ravel and is provided for API sameness with
 * NumPy.
 *
 * @ingroup xt_xmanipulation
 * @param e the input expression
