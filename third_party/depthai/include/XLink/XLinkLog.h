 ...
 * MVLOG_ERROR = 3
 * MVLOG_FATAL = 4
 * Before including header, a unit name can be set, otherwise defaults to
 * global. eg:
 *
...
#else
FUNCATTR_WEAK mvLog_t __attribute__((weak))
MVLOGLEVEL(MVLOG_UNIT_NAME) = MVLOG_LAST;
#endif
...
#define mvLog(lvl, format, ...)                                           \
  logprintf(MVLOGLEVEL(MVLOG_UNIT_NAME), lvl, __func__, __LINE__, format, \
            ##__VA_ARGS__)

// Set log level for the current unit. Note that the level must be smaller than
// the global default
...
// Set the global log level. Can be used to prevent modules from hiding messages
// (enable all of them with a single change) This should be an application
// setting, not a per module one
