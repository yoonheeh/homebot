...
#define XLINK_RET_ERR_IF(condition, err)                      \
  do {                                                        \
    if ((condition)) {                                        \
...
#define XLINK_RET_IF_FAIL(call)                                             \
  do {                                                                      \
    int rc;                                                                 \
    if ((rc = (call))) {                                                    \
      mvLog(MVLOG_ERROR, " %s method call failed with an error: %d", #call, \
            rc);                                                            \
      return rc;                                                            \
    }                                                                       \
  } while (0)
...
#define XLINK_OUT_WITH_LOG_IF(condition, log) \
  do {                                        \
    if ((condition)) {                        \
      (log);                                  \
      goto XLINK_OUT;                         \
...
#ifndef XLINK_OUT_IF
#define XLINK_OUT_IF(condition)                                     \
  do {                                                              \
    XLINK_OUT_WITH_LOG_IF(                                          \
        (condition),                                                \
