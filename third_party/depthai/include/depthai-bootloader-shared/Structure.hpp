...
struct NetworkBootloaderStructure : Structure {
  ...
  constexpr static long USER_BOOTLOADER_SIZE =
      8 * 1024 * 1024 - BOOTLOADER_SIZE - CONFIG_SIZE - HEADER_SIZE;
  constexpr static long CONFIG_OFFSET =
      USER_BOOTLOADER_OFFSET + USER_BOOTLOADER_SIZE;
...
struct UsbBootloaderStructure : Structure {
  ...
  constexpr static long BOOTLOADER_SIZE =
      1 * 1024 * 1024 - CONFIG_SIZE - HEADER_SIZE;
...
inline const Structure getStructure(Type type) {
  switch (type) {
    case Type::AUTO:
      throw std::invalid_argument("Invalid argument to getStructure function");
    case Type::USB:
      return UsbBootloaderStructure();
    case Type::NETWORK:
      return NetworkBootloaderStructure();
  }
  // Default
  return UsbBootloaderStructure();
}
