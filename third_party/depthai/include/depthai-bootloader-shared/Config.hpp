  ...
  inline void to_json(nlohmann::json &nlohmann_json_j,                       \
    ...
    DEPTHAI_BOOTLOADER_NLOHMANN_JSON_EXPAND(                                 \
        DEPTHAI_BOOTLOADER_NLOHMANN_JSON_PASTE(                              \
            DEPTHAI_BOOTLOADER_NLOHMANN_JSON_OPTIONAL_TO, __VA_ARGS__))      \
  }                                                                          \
  inline void from_json(const nlohmann::json &nlohmann_json_j,               \
                        Type &nlohmann_json_t) {                             \
    DEPTHAI_BOOTLOADER_NLOHMANN_JSON_EXPAND(                                 \
        DEPTHAI_BOOTLOADER_NLOHMANN_JSON_PASTE(                              \
            DEPTHAI_BOOTLOADER_NLOHMANN_JSON_OPTIONAL_FROM, __VA_ARGS__))    \
  }
...
DEPTHAI_BOOTLOADER_NLOHMANN_DEFINE_TYPE_OPTIONAL_NON_INTRUSIVE(
    NetworkConfig, timeoutMs, ipv4, ipv4Mask, ipv4Gateway, ipv4Dns, ipv4DnsAlt,
    staticIpv4, ipv6, ipv6Prefix, ipv6Gateway, ipv6Dns, ipv6DnsAlt, staticIpv6,
    mac);
...
DEPTHAI_BOOTLOADER_NLOHMANN_DEFINE_TYPE_OPTIONAL_NON_INTRUSIVE(UsbConfig,
                                                               timeoutMs,
                                                               maxUsbSpeed, vid,
                                                               pid);
