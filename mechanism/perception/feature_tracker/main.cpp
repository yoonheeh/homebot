...
class FeatureTrackerDrawer {
 public:
  ...
  FeatureTrackerDrawer(const std::string &trackbarName,
  ...
  void drawFeatures(cv::Mat &img) {
    cv::setTrackbarPos(trackbarName.c_str(), windowName.c_str(),
    ...
    for (const auto &[id, path] : trackedFeaturesPath) {
      for (size_t j = 0; j < path.size() - 1; j++) {
        ...
        cv::Point dst(static_cast<int>(path[j + 1].x),
                      static_cast<int>(path[j + 1].y));
        cv::line(img, src, dst, lineColor, 1, cv::LINE_AA, 0);
      }

      if (!path.empty()) {
        size_t j = path.size() - 1;
        cv::Point point(static_cast<int>(path[j].x),
                        static_cast<int>(path[j].y));
...
void onTrackbar(int val, void *userdata) {
  auto *inputConfigQueue =
      static_cast<std::shared_ptr<dai::InputQueue> *>(userdata);
  try {
    auto cfg = std::make_shared<dai::FeatureTrackerConfig>();
    auto cornerDetector = dai::FeatureTrackerConfig::CornerDetector();
    cornerDetector.numMaxFeatures =
        cv::getTrackbarPos("numMaxFeatures", "Features");
...
int main() {
  ...
  auto camOutput = camera->requestOutput(std::make_pair(640, 640),
  ...
  cv::createTrackbar("harrisScore", "Features", nullptr, 25000, onTrackbar,
                     &inputConfigQueue);
  cv::createTrackbar("numMaxFeatures", "Features", nullptr, 1024, onTrackbar,
                     &inputConfigQueue);
  ...
  FeatureTrackerDrawer leftFeatureDrawer("Feature tracking duration (frames)",
                                         leftWindowName);
  ...
  while (true) {
    auto outputPassthroughImage =
        outputFeaturePassthroughQueue->get<dai::ImgFrame>();
    if (outputPassthroughImage == nullptr) continue;

    cv::Mat passthroughImage = outputPassthroughImage->getCvFrame();
    auto trackedFeaturesLeft =
        outputFeatureQueue->get<dai::TrackedFeatures>()->trackedFeatures;
