...
const uint PIN_ENC_FL_A = 2;  // Front-Left Encoder: GP2 (Phase A) & GP3 (Phase B)
const uint PIN_ENC_RL_A = 4;  // Rear-Left Encoder:  GP4 (Phase A) & GP5 (Phase B)
const uint PIN_ENC_FR_A = 6;  // Front-Right Encoder: GP6 (Phase A) & GP7 (Phase B)
const uint PIN_ENC_RR_A = 8;  // Rear-Right Encoder:  GP8 (Phase A) & GP9 (Phase B)

// Left Motor Driver Control (Board L)
const uint PIN_PWM_LEFT = 12; // Left PWM Speed Control: GP12 (PWMA bridged to PWMB)
const uint PIN_DIR_L1   = 10; // Left Direction Pin 1:   GP10 (AIN1 bridged to BIN1)
const uint PIN_DIR_L2   = 11; // Left Direction Pin 2:   GP11 (AIN2 bridged to BIN2)

// Right Motor Driver Control (Board R)
const uint PIN_PWM_RIGHT = 13; // Right PWM Speed Control: GP13 (PWMA bridged to PWMB)
const uint PIN_DIR_R1    = 14; // Right Direction Pin 1:   GP14 (AIN1 bridged to BIN1)
const uint PIN_DIR_R2    = 15; // Right Direction Pin 2:   GP15 (AIN2 bridged to BIN2)
...
// 4. COBS Encoding Algorithm (removes all 0x00 bytes and appends 0x00 delimiter)
...
void send_telemetry(const EncoderIMUTelemetry &telemetry) {
    ...
    size_t encoded_len = cobs_encode(raw_buffer, sizeof(EncoderIMUTelemetry), encoded_buffer);
...
int main() {
    ...
    gpio_init(PIN_DIR_L1); gpio_set_dir(PIN_DIR_L1, GPIO_OUT);
    gpio_init(PIN_DIR_L2); gpio_set_dir(PIN_DIR_L2, GPIO_OUT);
    gpio_init(PIN_DIR_R1); gpio_set_dir(PIN_DIR_R1, GPIO_OUT);
    gpio_init(PIN_DIR_R2); gpio_set_dir(PIN_DIR_R2, GPIO_OUT);
    ...
    while (true) {
        ...
        while ((c = getchar_timeout_us(0)) != PICO_ERROR_TIMEOUT) {
            if (c == 0x00) {
                ...
                if (decoded_len == sizeof(VelocityTarget)) {
                    ...
                    uint16_t check_crc = calculate_crc16((uint8_t*)&incoming, sizeof(float) * 2);
        ...
        // Background auto-detection: if IMU is not present, try to initialize it once per second (every 50 loops at 50Hz)
        ...
        if (imu_present) {
            ...
            if (i2c_write_blocking(i2c0, active_imu_addr, &reg, 1, true) >= 0) {
                if (i2c_read_blocking(i2c0, active_imu_addr, raw_data, 14, false) >= 0) {
                    ...
                    telemetry.gyro_x = ((float)raw_gyro_x / 131.0f) * (3.14159265f / 180.0f); // rad/sec scale
                    telemetry.gyro_y = ((float)raw_gyro_y / 131.0f) * (3.14159265f / 180.0f);
                    telemetry.gyro_z = ((float)raw_gyro_z / 131.0f) * (3.14159265f / 180.0f);
                }
            }
        }

        // Calculate and append CRC16 check code to telemetry packet
        telemetry.crc16 = calculate_crc16((uint8_t*)&telemetry, sizeof(EncoderIMUTelemetry) - sizeof(uint16_t));
